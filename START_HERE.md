...
2. **Read the standing orchestrator prompt** — the NEWEST file matching `nexsys-hivemind/context/handoff/*_PM-mission-control_v*_orchestrator_session_prompt.md` that is NOT in `archive/`. Since v67 (2026-09-06) this is the STABLE prompt — one file, the laws by reference; it boots on the newest `context/handoff/OPERATOR-BRIEF_for-Nick.md` (§HELD = the operator's wait-state ledger and Nick's entry point) + the newest spine beat. Lane agents instead read the dispatch Nick names (in `context/instructions/`). **The archive convention (2026-09-07):** an executed prompt/instruction/packet lives in its directory's `archive/` with an `EXECUTED`/`SUPERSEDED` status prefix; a cited path not found is looked up in `<dir>/archive/` (`context/canonical-paths.md` § The archive convention).
3. **Run your skill's freshness preflight before any work.** The PM's 12-check preflight re-derives all volatile state (HEADs, milestone status, watermark, counts) from source. Never trust a masthead's state claims — including this file's.
