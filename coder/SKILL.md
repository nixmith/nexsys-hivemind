...
status: CURRENT
last-verified: 2026-06-22 (v5 hub — the M7.2b-GREEN / action-model-FROZEN point). **M7.2b GREEN** (core `1b0b6c9`, `./gradlew check` 149 tasks, first pass, zero gate-fix rounds) — **THE ACTION MODEL IS FROZEN** (`ComputedValue` sealed / 3 permits resolved at run-init against the captured AMD-03 snapshot before the frozen `ActionExecutor`; run-coupled fail-closed-read terminal; no-engine-retry AMD-90-INV-01; counts 71/41/53; module-info UNCHANGED). The engine **ACTS end-to-end** (trigger → condition → run → action → dispatch → events): M6 (4-of-4) · M7.1 · AB-1/AB-2/AB-3 · M7.2a-1 · M7.2a-2 · M7.2b all DONE + GREEN. **Next Core slot = M7.3 (PendingCommandLedger)** — §3.11.2 FSM (`PendingStatus` 5 states), `Expectation`→`state_confirmed` correlation, deadline/timeout, idempotency + EXPIRED-on-restart, coalescing DISABLED; builds on M7.2a-2's `CommandDispatchService`; **it ADDS event types — re-derive the new counts from source (71/41/53)** — then the thin causal-read-API → AB-4 → M9 Zigbee. **Build discipline (bake into every WU):** the **record component / static-factory name collision** STOP-check (a component `X` already defines accessor `X()` — never add a method/factory `X()`; M7.2a-2 shipped this TWICE past a clean LLM self-review — scan every new record before handoff) + a targeted `:module:compileJava` + `spotlessApply` self-step BEFORE handoff; **the compile/CI gate is the only GREEN** (CI-as-gate-of-record — an in-session 'verification: clean' is not a gate). docs `e47f01e` (Doc 16 LOCKED; AMD-94; 169/49). **NEW:** a parallel hardware-bench / device-characterization thread (Wave 1 ~2026-06-23; corpus `project-knowledge/device-corpus/`) feeds M9's acceptance ground-truth. Prior baseline: 2026-06-19 against `60d50ce` (AB-3/Seam-1 arc).
-->
