...
status: CURRENT
last-verified: 2026-07-30 (skills currency pass 2 — masthead only: **+the v40-arc conventions line** (the format #20/#21 wire-pin pointers · operator-block hygiene ×4 · instrument-first-on-repeat-failure) and **the north-star line amended to the TWO-document strategy pointer with the language law**; `references/freshness-preflight.md` gained the spelled lock-free-porcelain line in the same pass. No body change; census + honesty evidence: `../context/audits/2026-07-30_skills-pass-2_return.md`.) Prior: 2026-07-28 (the post-v39 skills-currency pass — masthead only: **+the v39-arc conventions line** (Kotlin-DSL `java`-extension shadowing · `--rerun` is per-task and its evidence discipline · git-invisible empty dirs flipping NO-SOURCE states) and **+the north-star pointer line**; `references/freshness-preflight.md` Check 6 gained the third-location/remote-cache refinement in the same pass. No reference-file content beyond that; no body change). Prior: 2026-07-26 (v38 hub, beat 3 — the skills currency pass: the §3.9 silent-skip convention line updated to the authored SKIP-VIS truth (READ-side honesty; skip events stay unemitted; payload counts = the only disclosure); the NO_DIRECT_TIME_ACCESS test-reach claim corrected here + java-patterns §11 + testing-standards §8 (convention-everywhere, enforcement = app test classpath — the 2026-06-13 correction finally folded at its carriers); the web-ui/dashboard MODULE_CONTEXT no-longer-a-stub correction). Prior: 2026-07-18 (v32 hub, close-out — +the v32-arc conventions line below: fixture-paired asserts STANDING; OR-group observation anchors; the sect-3.9 silent-skip composition gotcha; deploy-state at the instrument; terminal-materialization). Prior: 2026-07-08 (v25 hub, beat 6 — +the DUR-arc conventions below; mental-model §3 gains registries-as-projections; java-patterns gains §14). Prior: 2026-07-07 (v23 hub, beat 7 — the M9.4 bench-arc conventions).
v40-arc conventions (2026-07-30, the B2 silicon arc — detail: `../context/audits/2026-07-28_B2_suite-port_return.md` §§12–13 + pm-handoff v40 beats 1–5; all STANDING): (1) **wire pins ride the PM format #20/#21 laws** — a pin on any wire path/shape cites a filed measurement or orders one first (measure-then-pin), a RE-pin checks the FILED MEASUREMENT CORPUS before any source read (an endpoint-feed source pin never outranks a wire measurement), and a wire-dialect fix carries the FULL-CORPUS sweep for the class in the same beat. (2) **operator-block hygiene when authoring paste-blocks** — STOP-gates get their OWN block · WHERE-labels ride INSIDE as a leading comment · FULL PATHS, never PATH assumptions · every named verb/flag verified to exist before it ships (the F-1/F-5/F-10/F-11 quartet; playbook §8 addenda). (3) **instrument-first on a repeat failure** — the same leg failing twice under two theories buys the bundle/evidence read BEFORE any third theory, per-hypothesis predictions stated first; never retune blind.
v39-arc conventions (2026-07-27, the DASH-SERVE arc — detail: coder-lessons 2026-07-27 ×2 + pm-handoff v39 beats 1–2; both entries STANDING): (1) **Kotlin-DSL `java`-extension shadowing** — inside a build script the identifier `java` resolves to the `java {}` extension accessor, not the package, so an inline `java.util.zip.ZipFile(...)` FQN fails script compilation ("Unresolved reference: util"): **hoist an `import`; never inline-FQN a `java.*` type in a build script.** (2) **`--rerun` is a PER-TASK option** — it binds only to the task it FOLLOWS on the command line (`gradlew a:test b:test --rerun` forces `b:test` alone), so a "forced-fresh ×2" claim behind such a run is silently vacuous: force EVERY task explicitly, and prove forced-freshness from the executed-state lines + result-file mtimes, NEVER from the green summary. (3) **Git-invisible empty dirs flip NO-SOURCE states machine-to-machine** — an untracked empty dir under `src/main/resources` makes `processResources` EXECUTE on one tree and report NO-SOURCE on another, moving the actionable-task count: reconcile ANY gate-state variance to its mechanism before quoting the gate.
v32-arc conventions (2026-07-18, the B1-REV2→I3a arc — detail: coder-lessons + `context/process/2026-07-18_compounding-testing-doctrine.md` + pm-handoff v32 beats 1–3): (1) **Fixture-paired assert semantics are a STANDING RULE** — every new assert/scenario mechanic ships with fixtures proving its PASS AND its false-verdict boundary, MUTATION-VERIFIED (a mutant deleting the condition must flip the verdict; the checked tool restored byte-identical). Field-proven: the REV2 assert caught a four-day invisible condition on its first live rep. (2) **OR-group observation anchors:** a `log_any`/OR-group's satisfaction NEVER stamps per-member observations — record only actually-matched tokens (the REV2 false-PASS defect class, killed at lint AND stamping). (3) **The §3.9 UnavailablePolicy composition gotcha:** `CommandAction`s silently SKIP UNAVAILABLE targets per-target (the action completes "success"; ZERO events at the log layer — per-target skips emit nothing, so the terminal payload's `actionCount`/`commandCount` arithmetic is the ONLY disclosure; the SKIP-VIS WU [authored 2026-07-26] makes the READ side honest — verdict + additive wire keys — while skip events stay unemitted by design) and availability REHYDRATES FROM THE LOG — state HISTORY governs behavior; price both in any automation/bench/scenario work, and prefer history-seeded test setups for composition behavior. (4) **Deploy-state is re-derived at the instrument** — version-verify the checkout/binary you are actually testing (never assume an ordered step ran); instruments self-identify (version banners). (5) **Terminal materialization:** runs-surface rows AND run-lifecycle events materialize at/near terminal (`triggered.eventTime == completed.eventTime` — field-confirmed); a type-filtered subscriber's checkpoint is NOT a log-head cursor. Price both in every runs/checkpoint assert.
dur-arc conventions (2026-07-08, the AMD-99/M9.5 arc — detail in mental-model §3 + both MODULE_CONTEXTs + coder-lessons): (1) **registries are projections** — never call a registry mutator in production code (single apply path; write-ahead publish-then-apply; idempotent re-emit is the ONLY update path; boot replays from position 0 — in-memory projections never resume from a checkpoint). (2) **Mirrors track schema** — a device-model record component change needs its event-mirror twin or an explicit exclusion ruling. (3) **Number-bearing serde tests pin ACTUAL types** (java-patterns §14 — the `?:` promotion re-box class). (4) **Push back pre-implementation on instruction pins that contradict the instruction's OWN settled decisions** — the M9.5-DUR zero-module-info pin vs its DP-2 exported surface; the evidence-based pushback was ruled ACCEPT and beat a silent workaround.
bench-arc conventions (2026-07-07, the M9.4 silicon arc — detail in `context/process/bench-troubleshooting-playbook.md` + coder-lessons): (1) **BENCH-VERIFY constants blocks** — every wire fact the silicon could contradict (frame/cluster/command/status ids, timeouts, key bitmasks) lives in an isolated, labeled constants block that BOTH production code and tests bind, so a silicon correction is a one-constant edit fixing code + tests together (proven 4×). (2) **The anti-vacuous success INFO** — any arm that can succeed silently ships ONE positive-evidence INFO with counts (`…: device={} …=N`); absence-of-WARNs from a never-run mechanism reads as success and has shipped gaps. (3) **Typed results across ops seams** — hardware-outcome seams return typed results (TIMEOUT is a RESULT), never throw; a dedicated catch-and-WARN backstop at the drive site keeps hardware failures from gating unrelated lifecycles. (4) **Log-only observability arms carry a test-enforced PIN** (never a device/interview/event/registry change) — the 0x0024/0x0023/0x009B pattern. (5) Count assertions frame-id-scoped, never totals (session-start arithmetic grows); within a scripted frame list, riders precede the matching response (the receive loop returns at the match).
