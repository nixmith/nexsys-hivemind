...
state-type: comms
status: SUPERSEDED 2026-09-07 — by: context/handoff/2026-09-06_v66_OPERATOR-BRIEF_for-Nick.md §HELD (the wait-state ledger) · was: CLOSED-CHANNEL (historical register; content preserved verba
...
## OQ (2026-06-11) — `NO_DIRECT_TIME_ACCESS` does not reach non-app test source sets (from M6.2 Coder feedback)

**Fact (Coder-verified, review-agent-confirmed):** `HomeSynapseCoreTest` uses `Clock.systemUTC()` ×9 across GREEN gates — the ArchUnit rule runs from app's test classpath only and structurally cannot scan lifecycle's (or any other module's) test classes. The PM skill §4c reminder's premise ("the rule scans test classes in non-whitelisted packages") is therefore WRONG for every module except app itself; the prescription (inject `Clock.fixed`) remains correct practice and M6.2 followed it.
**Dispositions:** (a) extend the rule's reach (import sibling test classes into the app ArchUnit run), or (b) explicitly whitelist module test source sets + correct the §4c text to "convention, enforced only for app-visible classes."
**Owner:** PM — correct the §4c wording at the next skill-source touch; the reach decision rides the next app-module-touching WU (do NOT fold into M6.3 — no scope creep). Until resolved, coding instructions must not assert the rule reaches non-app test code.
**Update 2026-06-13 (Cowork PM):** disposition (b) **wording-half DONE** — PM `SKILL.md` §4c corrected to scope enforcement to app-visible classes only (Clock-injection framed as a self-enforced convention elsewhere; production-code reach clarified). The **reach decision** (whitelist module test source sets vs. extend the app ArchUnit run) **remains OPEN**, still riding the next app-module-touching WU. Skill-mirror sync (Check 9) pending so the read-only mirror picks up the corrected §4c.
