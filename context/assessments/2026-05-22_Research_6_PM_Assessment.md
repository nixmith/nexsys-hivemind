...
> **✅ NQ-1..6 RESOLVED — 2026-06-04 (Nick, W23 Goal 2).** Nick adopted **all six PM recommendations as stated above**: NQ-1 `SecurityServices` aggregator; NQ-2 keep-both + rename `schemaVersion`→`descriptorSchemaVersion` + add `configSchemaMajor`/`configSchemaMinor` (one cohesive AMD per P2 §8 — resolve the hooks-vs-schema split as two full integers if it splits); NQ-3 sealed `Capability` permit class + existing `CapabilityInstance` as identity (no new `CapabilityId`); NQ-4 no new SQLite table — capability events project into `Entity.capabilities` (entity-registry projection); NQ-5 REC-49 REJECTED (use existing `HealthParameters.maxRestarts`/`restartWindow`; document the 1/60s embedded override); NQ-6 keep 1/60s global default + per-descriptor override, with a pre-M9 empirical spike on real Zigbee/Matter restart frequency. **NQ-3/NQ-4 are pre-decisions** — they finalize automatically once B Stage 2 (M4.B-S2 EntityRole) lands, per `context/planning/2026-05-31_Workstream-C_gate-status.md` §1 (the capability surface freezes once, against the post-B device model). Recorded in OQ-05-05 (RESOLVED) and the gate-status note. Workstream C Gate A is **cleared pending B-S2 landing**; the AMD integers re-base to 54+ at C-briefing per the renumber notice above.
