...
| G1 | M | surface ✅ BUILT + LANDED (v35 beat 6, `f3bfd5c`, frontend.yml green); the gate-day ≤10-min demo = the remaining evidence act. **⚠ WATCH 2026-07-27 (v39 beat 5):** the redux found the run-detail causal chain rendering EMPTY live on EVERY run (finding F4) + the why-not surface hanging (F2) — demo legs (a)/(c) are BLOCKED until the detail-read cluster is diagnosed/fixed; WCAP wire capture ordered (`context/handoff/2026-07-27_WCAP_detail-read-wire-capture_operator-block.md`) |
