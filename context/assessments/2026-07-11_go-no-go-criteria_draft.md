...
| G1 | M | surface ✅ BUILT + LANDED (v35 beat 6, `f3bfd5c`, frontend.yml green); the gate-day ≤10-min demo = the remaining evidence act. **⚠ WATCH re-narrowed 2026-07-27 (v39 beat 8, WCAP-2):** **the demo's leg-(a)/(c) WIRE is PROVEN** — fresh-run chains hydrate server-side UNANIMOUSLY (3/3 populated at 1693–1717 B vs the 507 B skeleton, incl. a post-deploy-stamped run with `automationName` + `trigger.type` resolving; the live five-modes vocabulary observed raw: UNCONFIRMED-timed-out · DISPATCHED-superseded · the honest no-report register, `settled: true`). The Jul-19-era skeleton = a HISTORY artifact (the honest "no detail recorded" state rides FE-LIVE-V112 (i); the era-boundary bounded core look = P3). **The one residual: the SPA's render of populated payloads** — the redux's blanks on recent runs remain unexplained; ONE dev-tools glance closes it (F12 → Network → open the freshest run → ⏺ the chain response status/size + what renders). WCAP-2 return: `context/audits/2026-07-27_WCAP2_recent-run-chain_return.md` |
