...
| G1 | M | surface ✅ BUILT + LANDED (v35 beat 6, `f3bfd5c`, frontend.yml green); the gate-day ≤10-min demo = the remaining evidence act. **⚠ WATCH 2026-07-27, NARROWED at v39 beat 6 (WCAP adjudicated):** the demo blocker = **WU-CHAIN-HYDRATE** only — the live `/causal-chain` serves a self-contradicting skeleton (`actionCount: 9`, `actions: []`; server-side, correlation-join seam; WCAP return §3-F4). The why-not "hang" dissolved (the endpoint answers a 404 in 6 ms — the id had rotated; the spinner is an FE posture note) and `/state`'s dialect drift (STATE-DIALECT, DRIFT-2-class) does not touch the demo legs. Legs (a)/(c) unblock when CHAIN-HYDRATE lands + deploys; the demo scripts against a FRESH run (identity rotation is demo-compatible) |
