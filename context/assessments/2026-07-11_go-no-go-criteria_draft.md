...
|---|---|---|---|---|---|
| A1 | The 72 h soak completes on the certified build `04f5f70`: zero crashes, zero event loss, no unbounded memory/log growth, zero un-triaged anomalies (any health-glance anomaly interrupts everything — Nick's rail) | [M] | Soak logs (`bench-2026-07-10-075035.log`+) + the close-out intake block appended to the bench acceptance record | IN PROGRESS — entered 07:50:35 Pi-local 2026-07-10; exit ≥ 2026-07-13 ~07:50; a restart resets the clock | Nick (glances) + hub (close-out intake) |
...
**Why N = 200 (the proposal Nick was asked for):** at N = 200 with zero false CONFIRMs, a true false-CONFIRM rate of 1% would have been seen with ~87% probability (1 − 0.99²⁰⁰); at N = 500, ~99.3%. 200 is reachable from ~15–20 §51-class runs plus nightly suite repetitions inside the window even if B3 lands late; 500 requires B3 nightly running by ~end of July. Hence 200 MUST / 500 SHOULD. The count is cumulative-recorded (positions citable), never sampled.
...
|---|---|---|---|---|---|
| J1 | The gate is read against THIS ratified checklist and its §2 non-criteria — no criterion added at the table, no Wave-3 item smuggled in as a blocker | [M] | The ratified version of this file, frozen pre-gate | This draft | Nick (ratifies) + hub (freezes) |
...
- H3's "clean Pi image" rep is the one criterion with no scheduled carrier as of 2026-07-10 — it needs ~an hour of operator time and an SD card; flagging so it gets a slot rather than a gate-day surprise.
