...
date: 2026-07-10 (authored + hub-audit corrections applied same day)
...
status: RETURNED 2026-07-10 (filename per the dispatch §4) → hub-audited ACCEPT-WITH-CORRECTIONS same day; the five corrections are APPLIED in place (audit trail: §6.6). Awaiting re-audit; commit is the hub's.
...
- **CONFLICT (both quoted, report-both):** the SmartThings DTH fingerprint hand-writes `profileId: "0104"` for the same cluster set. The machine capture (0xC05E) is the stronger evidence and is independently corroborated by behavior: the S31 Lite zb joins Philips Hue v1 bridges as a "light" (HA carries a model-string special case — `self.is_s31litezb = light.modelid == "S31 Lite zb"`, `homeassistant/components/hue/v1/light.py:363`; changelog "Fix Hue SONOFF S31 Lite zb plug", core-2022.6; both via the grep.app index — https://grep.app/api/search?q=S31%20Lite%20zb) — ZLL-profile behavior. Firmware variance across units is possible. **The profile field is `[CONFIRM-ON-BENCH]` before any fingerprint ships.**
...
- **Basic `powerSource` (attr `0x0007`) raw byte: NOT CACHED** in the zha capture (only manufacturer/model sit in the Basic attribute cache — re-checked at hub audit); the node descriptor renders "Battery or Unknown". Read the byte at interview `[CONFIRM-ON-BENCH]`; the P-family carries documented Basic-powerSource misreports (§3.5/§4.6) — never key platform logic on it (§6.2-g).
...
- **Basic-cluster identity → MatchCriteria:** manufacturerName=**`eWeLink`**, modelIdentifier=**`SNZB-04P`** — the ZHA quirk keys on exactly this pair: `QuirkBuilder("eWeLink", "SNZB-04P")` (`zhaquirks/sonoff/snzb04p.py` — https://raw.githubusercontent.com/zigpy/zha-device-handlers/dev/zhaquirks/sonoff/snzb04p.py); z2m matches `zigbeeModel: ["SNZB-04P"]` / `model: "SNZB-04P"` (`src/devices/sonoff.ts` via grep.app index of Koenkk/zigbee-herdsman-converters). Cross-ecosystem corroboration: SmartThings Edge fingerprint `{ mfr = "eWeLink", model = "SNZB-04P" }` (via the grep.app cross-repo index — https://grep.app/api/search?q=SNZB-04P&filter[repo][0]=Koenkk/zigbee-herdsman-converters). Profile key = **`("eWeLink", "SNZB-04P")`** `[REF]` — same manufacturer string as the bench-captured SNZB-03P, so HIGH confidence, but the exact-model match still verifies at interview `[CONFIRM-ON-BENCH]`.
...
- **Power:** battery sleepy end device (PollControl-bearing battery sensor — §3.2). **Battery TYPE unverified** (SONOFF product page unfetched this session — §6.3-d; do not assume CR2032). **Basic `powerSource` (attr `0x0007`) raw byte: NOT CACHED** in the zha capture (hub-audit re-check; node descriptor renders "Battery or Unknown"), and this exact device has a documented powerSource misreport class (#24025, §3.5) — byte `[CONFIRM-ON-BENCH]`, flagged §6.3-h. manufacturerCode + firmware/dateCode `[CONFIRM-ON-BENCH]`.
...
Decoded (standard ZCL hex; decode is INFERENCE on the quoted dump): **EP1**, profile `0x0104`, device type `0x0402` — **in:** `genBasic 0x0000`, `genPowerCfg 0x0001`, `genIdentify 0x0003`, **`genPollCtrl 0x0020`**, **`ssIasZone 0x0500`**, **`0xFC11`** (SONOFF custom, 64529), and a second custom cluster the comment writes as `64567` — **which decodes to `0xFC37`**, not 0xFC57 (0xFC57 = 64599) · **out:** `genIdentify 0x0003`, **`genOnOff 0x0006` (client!)**, `genOta 0x0019`. `[REF]` *(Hub-audit correction: an earlier draft of this return mis-decoded 64567 as 0xFC57 and called the two sources byte-identical — both fixed; the conflict is stated below.)*

**Second, independent machine capture — corroborates the map EXCEPT ONE cluster id, where the two sources CONFLICT** (parent lane's gap-fill pass): the zigpy/zha test-corpus dump at firmware `0x2200` records EP1, profile 260, device type IAS_ZONE 1026, in `[0x0000, 0x0001, 0x0003, 0x0020, 0x0500, 0xfc11, 0xfc57]`, out `[0x0003, 0x0006, 0x0019]` — the capture's second custom cluster is **`0xfc57` (64599)** where the quirk's hand-written SimpleDescriptor comment says `64567` (= `0xFC37`). One of the two carries a transcription slip; **the machine capture LEADS** (hand comments are the weaker evidence class), so `0xFC57` stands in the fingerprint stub — conflict flagged §6.3-g; the interview's own read adjudicates `[CONFIRM-ON-BENCH]`. The capture also records quirk applied `zhaquirks.sonoff.snzb04p:(eWeLink / SNZB-04P)`, **cached IAS `zoneType = 21`** (the ZCL Contact-Switch class — the §3.2 expectation now evidence-backed), battery cache 3.0 V / 100 % — https://raw.githubusercontent.com/zigpy/zha/dev/tests/data/devices/ewelink-snzb-04p-0x00002200.json `[REF]`. Raw dump on OUR silicon `[CONFIRM-ON-BENCH]`.
...
```
(fingerprint = the zha MACHINE capture — incl. `0xFC57`, the capture-led side of the §6.3-g cluster-id conflict; re-derive from OUR interview before shipping — the Hue LCA006→LCA017 lesson.)
...
- **Basic-cluster identity → MatchCriteria:** manufacturerName=**`eWeLink`**, modelIdentifier=**`SNZB-01P`** — ZHA: `QuirkBuilder("eWeLink", "WB01") … .also_applies_to("eWeLink", "SNZB-01P")` (`zhaquirks/sonoff/button.py` — https://raw.githubusercontent.com/zigpy/zha-device-handlers/dev/zhaquirks/sonoff/button.py); z2m: `zigbeeModel: ["SNZB-01P"], model: "SNZB-01P", vendor: "SONOFF", description: "Wireless button"` (sonoff.ts ~6410–6414 via grep.app); SmartThings Edge `drivers/SmartThings/zigbee-button/src/ewelink/fingerprints.lua`: `{ mfr = "eWeLink", model = "SNZB-01P" }` (via the grep.app cross-repo index — https://grep.app/api/search?q=SNZB-01P&filter[repo][0]=Koenkk/zigbee-herdsman-converters). Profile key = **`("eWeLink", "SNZB-01P")`** `[REF]`; verify at interview `[CONFIRM-ON-BENCH]`.
...
- **Basic `powerSource` (attr `0x0007`) raw byte: NOT CACHED** in either zha capture (hub-audit re-check; node descriptor renders "Battery or Unknown"), and this exact device has the documented powerSource-"Unknown" misreport class (#22032/#24007, §4.6) — byte `[CONFIRM-ON-BENCH]`, flagged §6.4-h.
...
- **ZCL device type: `0x0000` (ON_OFF_SWITCH)** — per the two §4.2 machine captures `[REF]` (hub-audit correction of a stale pre-gap-fill line); re-verify at interview `[CONFIRM-ON-BENCH]`.
...
- **Health + identity**: the driver's health read is `Sys.GetStatus` → `uptime`, `restart_required`, `available_updates` (https://shelly-api-docs.shelly.cloud/gen2/ComponentsAndServices/Sys/) — log `uptime` with every poll so an unexpected watchdog reboot is visible in the evidence bundle; identity is `Shelly.GetDeviceInfo` per §5.1. **`Shelly.GetStatus`** — "This method returns the status of all the components of the device." (response = status objects keyed per component instance: `switch:0`, `sys`, `wifi`, `cloud`, `ble`, `mqtt`, … — https://shelly-api-docs.shelly.cloud/gen2/ComponentsAndServices/Shelly/, re-verified at hub audit) — is the one-call all-components snapshot. **Declared substitution (§6.5-i):** the per-verb contract DELIBERATELY prefers the narrower `Switch.GetStatus` (assertion) + `Sys.GetStatus` (health); `Shelly.GetStatus` is reserved for the on-FAIL evidence-bundle snapshot. `[REF]`
...
- **(g)** Basic `powerSource` (attr `0x0007`) raw byte NOT CACHED in the zha capture (hub-audit re-check); node descriptor "Battery or Unknown" only. Byte `[CONFIRM-ON-BENCH]`.
...
- **(g) CLUSTER-ID CONFLICT (hub-audit find — the lane's own gloss originally mis-decoded it):** the quirk comment's `64567` = **`0xFC37`**, while the zha machine capture records **`0xfc57`** (= 64599) for the same slot. Capture leads (0xFC57 in the stub); the quirk comment likely carries a hand-transcription slip; OUR interview read settles it `[CONFIRM-ON-BENCH]`.
- **(h)** Basic `powerSource` (attr `0x0007`) raw byte: NOT CACHED in the zha capture (re-checked at hub audit — only manufacturer/model sit in the Basic cache); node descriptor renders "Battery or Unknown"; family misreport documented (#24025). Byte `[CONFIRM-ON-BENCH]`; never key logic on it.
...
- **(h)** Basic `powerSource` (attr `0x0007`) raw byte NOT CACHED in either zha capture (hub-audit re-check); the #22032/#24007 class makes the byte suspect even once read. `[CONFIRM-ON-BENCH]`.
...
- **(i)** The dispatch named `Shelly.GetStatus` among the endpoints to verify; the driver contract substitutes the narrower `Switch.GetStatus` (assert) + `Sys.GetStatus` (health) per verb — a deliberate, DECLARED substitution (hub-audit correction), with `Shelly.GetStatus` now quoted in §5.2 and reserved for the on-FAIL snapshot.
...
**Hub-audit corrections applied (2026-07-10, verdict ACCEPT-WITH-CORRECTIONS; edited in place by this lane; commit remains the hub's):**
1. §3.2/§3.4: the quirk comment's `64567` re-decoded correctly as `0xFC37` (was mis-glossed 0xFC57); the false "corroborates byte-for-byte" claim replaced with the explicit quirk-vs-capture cluster-id CONFLICT; capture-led `0xFC57` retained in the stub; new flag §6.3-g.
2. §5.2: `Shelly.GetStatus` quoted from the official page (re-fetched) + the driver's narrower-call substitution DECLARED; new flag §6.5-i.
3. §4.1: stale "ZCL device type: NOT FOUND pre-silicon" corrected to the captured `0x0000` ON_OFF_SWITCH.
4. §2.1/§3.1/§4.1: all three zha captures re-fetched for Basic `powerSource` (attr 0x0007) — NOT CACHED in any (only manufacturer/model); absence + the family misreport class now stated per device; new flags §6.2-g/§6.3-h/§6.4-h.
5. §1.2/§3.1/§4.1: the SmartThings and HA-light.py corroborations now carry their banked grep.app index URLs.
