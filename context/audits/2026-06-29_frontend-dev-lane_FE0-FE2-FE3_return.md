...
## Addendum (same session) — D-FE-10 RULED + implemented: self-hosted subset Inter
Nick ruled **self-host the ~25 KB subset Inter-variable** (cross-surface type coherence; affordable; stays local-first). Landed this session:
- **Asset:** `src/styles/fonts/inter-variable-subset.woff2` — **25,388 B (~25 KB)**, Inter variable **`wght`** axis retained (one file = 400/500/600), tight UI-glyph subset; git-tracked (not ignored). Reproducible subset command: `src/styles/fonts/README.md`.
- **Wiring:** `src/styles/fonts.css` `@font-face` (`font-display: swap`), imported by `global.css`; `--hs-font-sans` flipped in the DTCG source to lead with `"Inter Variable"`, system stack as fallback. Local-first held — ships in-image, served from loopback, **no CDN** (INV-LF-01). Decorative chain markers (`●✓✕◆`) aren't in Inter's Latin set and fall back per-glyph (intentional; they're `aria-hidden`).
- **`tokens.css` note (env limit):** the mount truncated the just-edited DTCG source for the shell, so the generator couldn't run in-sandbox. I applied the single regenerated `--hs-font-sans` line to the committed `tokens.css` by hand to match the source exactly; **CI `prebuild` / `npm run tokens` regenerates it from source authoritatively** (idempotent — identical bytes), and `tokens:check` will pass on a clean checkout. Re-confirm the real asset size + that `--hs-font-sans` resolves to Inter when the gate runs (the woff2 is a separate `@font-face` asset, not the JS/CSS bundle the 100 KB budget measures).

**axe-core gate — STAGED, deliberately NOT added here.** Wiring it needs a new devDep → a `package-lock.json` update → a working `npm install`, which this sandbox's mount cannot do reliably (the ENOTEMPTY rename failure). Adding the dep without a clean lockfile update would break `npm ci` — the opposite of a green gate. Staged for the clean-checkout gate-reconciliation as the first step of FE-4: `npm i -D vitest-axe` (or `@axe-core/playwright`), add an a11y test rendering the hero + key views in jsdom asserting no violations, add the step to `ci/frontend.yml`. Land + verify it there.

**Path (unchanged):** clean-checkout `npm ci && npm run verify` green (incl. the regenerated `tokens.css` + the new font asset) → add axe-core → **FE-4** (hero state matrix · WCAG 2.2 AA in BOTH themes · mobile-first) → FE-PWA → FE-5. **FE-1 still needs Core up on loopback** — run it where Core runs, not here.
