...
## §5 Audit correction (R3) — `settledAt != null ⇔ settled` (applied 2026-09-12, 23:36–23:39Z)
- STOP gates at boot: HEAD `72efb42`, porcelain 13 ✓. `date -u` 2026-09-12T23:36:41Z ⇒ 18:36 CDT, still 2026-09-12 CT. Rows 1–4 applied exactly; **row 5 TAKEN** (`buildTrigger`: `Instant matchedAt = instantOf(triggered);` — one rule, one place; wire byte-identical).
- **Tests first:** T7 rewritten (`settledAt_confirmedAt_forDispatched_followSettled`: bare + acknowledged → `settled` false, both instants null; superseded → `settled` true, `settledAt` = the superseded `command_result` envelope's instant, `confirmedAt` null) + T7c (`settledAt_presentIffSettled_overEveryFixture`: `assertThat(action.settledAt() != null).isEqualTo(action.settled())` over 17 fixtures — every `seedRun` kind, every `seedRunWithResult` outcome incl. superseded+timeout and acked+confirmed, the timed fixtures, the SKIPPED run).
- **RED 23:38:35–:39Z** (`:core:automation:test --tests StandardExplanationServiceTest --rerun --offline`): class 37 run / 2 fail — T7 `expected: 2026-01-01T00:00:02Z but was: null` (the superseded case); T7c `[DISPATCHED / superseded] expected: true but was: false`.
- **Fix:** `deriveOutcome` keeps `lastResultEnv` beside `lastResult`; the DISPATCHED return is the §2 block verbatim (`settledByResult = resultOutcome != null && !"acknowledged".equals(resultOutcome)` — on that branch only `superseded` reaches it); the javadoc sentence per row 2; `core/automation/MODULE_CONTEXT.md` per row 4 — the v1.1.4 paragraph's DISPATCHED sentence AND the Gotcha's matching sentence in the same file, pins now T4–T7c.
- **GREEN 23:38:57–:39:01Z**, the gate line verbatim `./gradlew :core:automation:test :api:rest-api:test :lifecycle:lifecycle:compileJava spotlessCheck --offline` → BUILD SUCCESSFUL: automation **228 → 229** (T7c; T7 replaced in place), 229/229 · rest-api 156/156 (3 skip); `compileJava` fresh, zero `warning:` lines; `spotlessJavaCheck` executed; lifecycle `compileJava` UP-TO-DATE (its ABI input unchanged by the edit); both test tasks executed fresh (inputs changed), XML 23:38:59Z / 23:39:01Z.
- **Census still 13 = 13 M + 0 A + 0 D** (the same files; diff now +969/−106); nothing staged or committed; `StandardRunManager`/`StandardActionExecutor` untouched.
- Left as delivered, for the hub: `api/rest-api/MODULE_CONTEXT.md` §EXPLAIN-114a's "JSON null when" cell for `settledAt` still reads "`DISPATCHED` (no classifying event; a superseded DISPATCHED included — flagged)" — outside §2's `core/automation` scope, so not edited; the correct cell is "a bare or acknowledged `DISPATCHED`" (one phrase for the landing). §1/§3 R3 above stand as the record of the pre-correction rule.

RETURNED nexsys-hivemind/context/audits/2026-09-12_EXPLAIN-114a_return.md 14748
