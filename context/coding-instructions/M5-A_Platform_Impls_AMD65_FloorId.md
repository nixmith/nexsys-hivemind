<!--
file: context/coding-instructions/M5-A_Platform_Impls_AMD65_FloorId.md
purpose: M5-A coding instruction — platform deployment impls (PlatformPaths/HealthReporter) + AMD-65 Expectation codec + FloorId Jackson registration. The small-Core lane of the M5 window (decisions D1–D8; charter weeks/2026-W24).
audience: Coder (Claude Code), PM
status: ISSUE-READY for the platform + FloorId parts; the AMD-65 codec part is ⛔ GATED on AMD-65 lightweight ratification (un-gate when AMD-65 RATIFIED).
baseline: homesynapse-core HEAD `8ef9e9f` (M4 COMPLETE; build GREEN 145 tasks). Re-verify at issue.
-->

# Coding Task: M5-A — Platform Deployment Impls + AMD-65 Expectation Codec + FloorId Registration

**Subsystem:** platform (platform-api impls) + persistence (codec + typed-ID registration)
**Design Doc:** Doc 12 §8.2/§8.3 (HealthReporter/PlatformPaths, Locked); AMD-65 (Expectation codec, PROPOSED — gates Part 2); AMD-44 (FloorId, RATIFIED).
**Phase:** 3-Implementation
**Task Brief Reference:** M5 window charter (`context/planning/weeks/2026-W24_jun08-jun14.md`), lane M5-A; decisions D1–D8.

## What This Implements

The small, dependency-free Core lane of the M5 window. Three independent pieces: **(1)** the deployment-tier implementations of the two already-frozen `platform-api` interfaces — `PlatformPaths` (filesystem layout) and `HealthReporter` (supervisor health reporting) — which exist today only as interfaces (Doc 12 §8.2/§8.3); **(2)** the **AMD-65** `Expectation` persisted codec so a command-bearing `CapabilityAdded` round-trips (clears the M9 prerequisite; gated on AMD-65 ratification); **(3)** the one-line `FloorId` Jackson registration (the B-S1 carry-item). These do not depend on the crypto design (Doc 15) and run in parallel with its DOCS review. **Deliberately small** — do not expand it.
...
| `core/persistence/src/test/java/com/homesynapse/persistence/EventPayloadCodecTest.java` | the `@Disabled("AMD-65 pending")` `capabilityAdded_onOff_roundTrips` (the acceptance to un-disable) + `TestEventSamples.capabilityAddedOnOff()` |
| `design/amendments/AMD-65_Expectation_Persisted_Codec.md` | the codec spec (Part 2) — implement to it |
...
// Part 2 (AMD-65) PROPOSED DIFF:  + requires com.homesynapse.device;
```
...
| MODIFY ⛔ | `core/persistence/src/main/java/module-info.java` | **+ `requires com.homesynapse.device;`** (AMD-65 — the JPMS STOP-gate) |
| CREATE ⛔ | `core/persistence/.../persistence/ExpectationSerializer.java` + `ExpectationDeserializer.java` | package-private; AMD-65 tagged-union codec over the 4 permits |
| MODIFY ⛔ | `core/persistence/.../persistence/PersistenceJacksonModule.java` | + `addSerializer(Expectation.class, …)` / `addDeserializer(Expectation.class, …)` (AMD-65) **and** + `registerTypedWrapper(FloorId.class, FloorId::toString, FloorId::parse)` (FloorId, NOT gated) |
| MODIFY ⛔ | `core/persistence/.../persistence/EventPayloadCodecTest.java` | remove `@Disabled` from `capabilityAdded_onOff_roundTrips` (AMD-65 acceptance) |
| CREATE | `core/persistence/.../persistence/...Test` | FloorId round-trip test (NOT gated) |

⛔ = the AMD-65 codec + persistence→device edge are **gated on AMD-65 lightweight ratification**. The platform impls (Part 1) and the FloorId registration + its test (Part 3) are **NOT gated** — they may land first. (FloorId registration is in `PersistenceJacksonModule` but is independent of the device edge.)
...
- **AMD-65-INV-01:** every `Expectation` permit round-trips losslessly; `WithinTolerance` doubles are bit-anchored (AMD-52). Test: the un-`@Disabled` acceptance + a per-permit round-trip suite.
...
None produced. The AMD-65 codec round-trips `capability.added` / `capability.removed` payloads (already-registered event types) — it adds no event type. **P2 survey result (below): no event-type set, count-pin, or manifest aggregator is touched by M5-A.**
...
- **persistence (⛔ AMD-65):** per-permit `Expectation` round-trip (all 4); `WithinTolerance` bit-identity (e.g. `0.1`, `NaN`, `−0.0`); the un-`@Disabled` `capabilityAdded_onOff_roundTrips` passes.
...
- **`WithinTolerance.evaluate()` still throws** — that's correct; AMD-65 is the *codec*, not the evaluator (automation, M7/M8). Do not implement `evaluate()`.
