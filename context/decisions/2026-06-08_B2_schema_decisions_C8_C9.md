...
state-type: decision
status: RATIFIED 2026-06-12 (Nick) — independent DOCS review returned 2026-06-12 in the AMD-88..93 bundle: C8 RATIFY-WITH-EDITS (EC8-1 folded — Glossary §5.4 superseded-in-part, INV-MU-01 check PASS [no user_ref wording present in the invariants register]; EC8-2 folded source-corrected), C9 RATIFY-AS-IS, coupling SAFE-TOGETHER verified. Doc 01 banner + Glossary edits applied 2026-06-12. The conventions govern from the next event written.
...
- **Owner-doc:** Doc 01 (Event Model) — the actor-attribution section currently says "person or actor by convention"; it gains the four-kind closed set + the typed-ID-provenance + the API-client mapping. INV-MU-01 cites it. **Implementing path:** a Doc 01 amendment authored at the milestone that first enforces it (M10 / multi-user); this record + a Doc 01 currency note land now so the convention governs from the next event written. **(EC8-1, review-folded 2026-06-12): the ratification currency edits ALSO cover Glossary §5.4 (:926 — "The Actor always references a `user_ref`, not a `person_ref`" conflicts with rules 2/5 and is superseded: the closed set is four-kind, automation-originated events stamp the `AutomationId`, and the originating human remains recoverable via the correlation chain to the user-actored root event — the causal-reference semantic restated as chain-recoverable) + an INV-MU-01 wording check** (the who-caused-this intent survives through the trace, not through field duplication).
- **Origin-consistency rule (EC8-2, review-suggested; folded with source-corrected enum values — the return's "{DEVICE, USER, AUTOMATION, SYSTEM}" misquoted the real 7-value `EventOrigin {PHYSICAL, USER_COMMAND, AUTOMATION, DEVICE_AUTONOMOUS, INTEGRATION, SYSTEM, UNKNOWN}`):** when `actorRef` is non-null, its kind and the envelope's `origin` must agree — kind `AUTOMATION` ⟺ `origin = AUTOMATION`; kind `PERSON` or `API_CLIENT` ⟺ `origin = USER_COMMAND`; kind `SYSTEM` ⟺ `origin = SYSTEM`. This makes the stamping tests doubly enforceable and gives the pre-registry log a coarse kind discriminator for free. (Device/integration-originated events — `PHYSICAL`, `DEVICE_AUTONOMOUS`, `INTEGRATION` — carry `actorRef = null` per rule 5's existing no-attributable-actor clause.)
...
- **At-rest encrypted-scope membership** of energy is **refined by the Pi-4 microbench (OQ-15-2)** — Doc 15 §3.4 currently keeps energy **plaintext-at-rest at MVP** (high-volume, not PII under INV-PD-03), while the data-readiness §6 scope alignment treats energy as a shred-scope; **the microbench resolves whether the volume/perf supports encrypting it.** This is a **list-tuning of the Doc 15 §9 default, NOT a Doc 15 re-open** — flagged so the two are reconciled when OQ-15-2 returns. (The shape decision here is independent of and unaffected by that tuning — the attributes/aggregates are the same whether or not the scope is encrypted-at-rest.) **→ RECONCILED 2026-06-12 (OQ-15-2 RESOLVED — `context/assessments/2026-06-12_OQ-15-2_disposition_encrypted-scopes-CONFIRMED.md` item 4):** the measured cost (3.5–19 µs/event at aggregate payload sizes vs energy's ~0.33 ev/s Path-1 rate) shows perf would NOT force energy out — but MVP membership stays **plaintext-at-rest on Doc 15 §3.4's PII-classification grounds** (the data-readiness §6 shred-scope framing is superseded by Locked Doc 15's D2 line); extending encryption to energy is the Doc 15 §14 post-MVP item, now with its cost input measured. The MVP encrypted set is FIXED `[identity, presence_personal]`. This staging item is CLOSED (the interview-staged breadth item above remains OPEN).
...
- **REVIEW RETURNED 2026-06-12 (the bundled dispatch with the M7 AMD block):** **C8 RATIFY-WITH-EDITS** (EC8-1 required — folded above; EC8-2 suggested — folded with corrected enum values) · **C9 RATIFY-AS-IS** (gate rows verified at `7c73c91`; the Doc 15 §3.4 staging confirmed list-tuning-not-re-open; disjointness from AMD-92 confirmed) · **C8 ⊕ AMD-92 coupling: SAFE TO RATIFY TOGETHER — YES** (all 19 payload rows actor-field-free; envelope-seam-only stamping verified both directions). Source claims re-verified semantically at `7c73c91`; line anchors re-derived: `EventEnvelope:112` HOLDS; the `SqliteEventStore` write site is now `:329–332` (the `:330` setNull line holds; the `:672`-era hydrate anchor re-anchors in the Doc 01 currency edit). The pre-C8 historical-refs tolerance note rides the M10 design notes. Return: `context/audits/2026-06-12_AMD-88-93_C8-C9_DOCS_Review_Return.md` §B.
- **Nick ratifies** C8 + C9. On ratification: Doc 01 currency note (C8) **+ the Glossary §5.4 supersession + INV-MU-01 wording check (EC8-1)** + Doc 02 currency note (C9); the enforcing Doc 01/Doc 02 amendments are authored at the implementing milestones (M10 for C8; M6/M7/energy-feature for C9). The conventions govern from the next event written regardless.
