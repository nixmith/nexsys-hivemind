...
status: READY — authored 2026-06-19. AMD-94 is a queued governance item OFF the AB-3 critical path (it gates AB-1/AB-2/AB-4, which sit behind AB-3). Run as its own fresh Cowork conversation (independent of the Doc 16 review session — see the note in the reads).
baseline: core `beb4bc3` / docs `32afb3f` / hivemind (current) ; watermark AMD-93 → this amendment mints AMD-94 ; Doc 15 LOCKED (2026-06-07, AMD-86/87) ; M6 COMPLETE. Confirm at preflight.
reads (in order):
