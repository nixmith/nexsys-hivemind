...
**I-1 Fetch (desktop, Git Bash + the browser; do this any day before Saturday — `gh` is not assumed on the desktop and is ABSENT on the Pi).** The run page: **`https://github.com/nexsys-io/homesynapse-core/actions/runs/32672999145`** (the install-smoke run on commit `dec35be`; banked v57 beat 2). Open the **`Build image + .deb, install-smoke on a clean arm64 machine`** job (NOT the amd64 one — each leg builds its own `.deb` with its own hash) → the step "Version-grammar echo (hs_version is tag-shaped; the .deb carries it)" → ⏺ its `version-grammar echo green: … sha256 <64 hex>  homesynapse_0.1.0+git20260823.231355.gdec35be_arm64.deb` line whole (the ORIGIN hash). The amd64 leg's line (`hs_version=0.1.0+git20260823.231355.gdec35be`, sha256 `f68e31d5…_amd64.deb`) is already banked and is NOT the hash to compare against.
...
# expect: ONE .deb named homesynapse_0.1.0+git20260823.231355.gdec35be_arm64.deb (≈61.8 MB) + one .tar.gz (+ logs). ⏺ the .deb hash — it must EQUAL the `sha256` on the ARM64 job's `version-grammar echo green:` line (R-7b prints it; no hash on the step = not the R-7b artifact → STOP). (If `gh` IS installed: `gh run download 32672999145 --repo nexsys-io/homesynapse-core -n distribution-artifacts-arm64 -D ~/r3-artifact` — then the same sha256sum.)
```
