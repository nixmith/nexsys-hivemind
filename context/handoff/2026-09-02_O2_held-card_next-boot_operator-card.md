...
status: CLOSED — MEASURED ON THE HELD CARD (Nick, 2026-09-03 03:10Z = Wed 22:10 CT, `hs-fresh`; the bench card was powered down for the swap — restore it after). RESULT: Block 1 healthy (`ActiveState=active` · `SubState=running` · `Result=success` · `ExecMainStatus=0` · `NRestarts=0` · `adoption_maps_rehydrated` count 1 · `network_resumed` count 1); Block 2 after ONE clean stop: `ActiveState=failed` · `SubState=failed` · `Result=exit-code` · `ExecMainCode=1` (= exited) · `ExecMainStatus=143` — THE PRE-FILED HYPOTHESIS CONFIRMED ON THE SHIPPED UNIT; Block 3: `reset-failed` + `start` → `active`/`running`, `network_resumed` count 1. Row 6's refutable-by is DISCHARGED; the FAILCHAN charter authors on this measurement; R-4b step 0 is NO LONGER OWED. Intake: v61 beat 9 (`context/audits/2026-09-02_F-R4-1_intake_two-layer-audit_v61-beat-9.md` §4 names it).
prior-status (v61 beat 8): DEFERRED TO R-4b (its packet's STEP 0). **The 09-02 run (Nick, ~21:00 CT) executed on `hs-dev-1` = THE BENCH CARD (`ssh pi`), where `homesynapse.service` does not exist (the bench runs via `~/bench.sh`); every command failed harmlessly ("Unit … not loaded/not found"); the bench is untouched.** The hub's defect: this card said "the HELD card" without its ssh target. THE HELD CARD IS `nick@hs-fresh.local` (`ssh -i ~/.ssh/id_ed25519_pi nick@hs-fresh.local`, per the R-4 packet §2) and it is a SEPARATE SD CARD physically swapped into the same Pi — powering it means taking the bench card out (a bench-down act; the nightly fires ~03:32 CT). The §6-B premise (exit 143 on SIGTERM) is ALREADY MEASURED at R-3a §6-B and mechanism-explained at source (the JVM's default SIGTERM exit = 143; the unit has no `SuccessExitStatus=`), so the FAILCHAN charter no longer waits on this card; Block 2 becomes a CONFIRMATION at R-4b. Do not swap cards for this alone.
-->

# O-2 — the held card's next boot (~5 min)

> **RESULT — pasted 2026-09-03 03:10Z (Wed 22:10 CT), the HELD card (`nick@hs-fresh.local`, hostname `hs-fresh`), verbatim values:** Block 1 `Result=success NRestarts=0 ExecMainStatus=0 ActiveState=active SubState=running`, rehydrated `1`, resumed `1` · Block 2 `Result=exit-code ExecMainCode=1 ExecMainStatus=143 ActiveState=failed SubState=failed` · Block 3 `ActiveState=active SubState=running`, resumed `1`. **O-2 is CLOSED.** The blocks below stay as the record of what was run.
