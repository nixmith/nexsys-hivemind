...
```
**Your one line back (to v66):** `HERO-0: dispatched <hh:mm CT>`. v66 then hands **Act 6: the FE-113 LANDING** (one command — the audit's §5 card; `frontend.yml` = the gate), then Act 7 (RS-12-F: the program §5 paste with your two Job-A forms inserted where it says `JOB A:`), then Act 8 (TR-1), then — at ~14:50 CT — **Act 9: the H8-a navigator paste** (the packet's §N); BLOCK 6 after H8-a.
...
| Lane | Window | State at v66 beat 1 (16:18Z) | The paste lives at | Returns to |
|---|---|---|---|---|
| FE-113 (the v1.1.3 mirror; the one lane on the core tree; baseline `093d5b4`) | — | **RETURNED 16:12Z · AUDITED ACCEPT** (`context/audits/2026-09-06_FE-113_intake_two-layer-audit_v66-b5.md`); 16 files sit unstaged on your core tree — **Act 6 = the landing** (one command, the audit §5; msg `../_scratch/2026-09-06_core_FE-113_commit-msg.txt`); report `FE-113: landed <sha> — frontend <green|red> · ci <green|red>` | the audit §5 | v66 |
...
| RS-12-F the fanciful-domus sprint | fresh Cowork | Act 7 — **you insert your two Job-A forms with the story each carries where the paste says `JOB A:`** | the program §5 "RS-12-F" (edited b7) | v66 |
| TR-1 the position census | fresh Cowork | Act 8 | the program §5 "TR-1" | v66 |
| W-SKILLS-7 (the ledgers) | fresh Cowork | state UNKNOWN (no card on disk at 16:18Z; skills `f9c0bf4` clean) — tell v66 in one line when you know | the v65 brief Act 4 | v66 |
| **H8-a (the rig; EXCLUSIVE)** | a fresh Cowork window (the navigator) + Git Bash + the two cards | **PACKET ON DISK** — `context/instructions/2026-09-06_H8a_real-wire_v113-keys_and_failchan-proof_navigator-packet.md` (+ the record scaffold `context/audits/2026-09-06_H8a_real-wire_operator-record.md`). **Act 9 at ~14:50 CT:** paste its §N into a fresh Cowork window; §0 + §1 are desktop work the navigator walks you through before the rig; ≤60 min at the rig; nothing else opens and nothing is asked of you while you are there | the packet §N | the navigator files the record; you paste ONE line to v66: `H8-a: RETURNED <path> <bytes>` |
| F-R4-1b (coder; the core tree) | host-side Claude Code | **INSTRUCTION ON DISK** (`context/instructions/2026-09-06_coder-lane_F-R4-1b_zdo-ieee-addr-req_second-surface_coding-instruction.md`); ⛔ dispatches ONLY after FE-113 LANDS (Act 6 + `frontend.yml` green) — v66 hands the line with the sha filled; your `R4C:` word is due before that hand-off | the instruction's DISPATCH LINE | v66 |
...
- `FE-113: landed <sha> — frontend <green|red> · ci <green|red>` (after Act 6).
- Each remaining lane's `RETURNED <path> <bytes>` line as it lands (HERO-0 · RS-12-F · TR-1).
