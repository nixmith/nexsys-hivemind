...
status: LIVE — edited at v67 beat 7, THE CLOSE (Wed 2026-09-09 ~07:3x CT; instrument 2026-09-09T12:27:21Z). §NEXT = Act 1 the ear test, then Act 2 the close card (push · the AMD-53 docs commit · HONESTY-1's coder paste · dispatch v68). v68's ONE deliverable = the PALOKI commissioning packet.
-->

# Operator brief (one act at a time)

## §NEXT — Act 1: the five-minute ear test on PALOKI (the S4 gate; the one input no lane can give)
Cold, to your phone's assistant or dictation: **"Paloki"** ×5, then *"Open Paloki."* · *"Enable the Paloki skill."* · *"Ask Paloki to lock the door."* · *"Paloki, good night."* Count the DISTINCT spellings; note any mishear to *OKI* / *penalty* / *low key*.
**Done looks like:** nine tries, a count. **One line back:** `EAR-PALOKI: pass <n spellings> | fail <what it heard>` (pass = ≤3 spellings, no systematic collapse). The ruling of record — proceed with PALOKI; commission the comprehensive search + opinion, do NOT skip it — is `context/audits/2026-09-09_RS-13_PALOKI_intake_two-layer-audit_and_RULING_v67-b6.md` §2; it is final on this line.
**Act 2 — the close card (after your `EAR-PALOKI:` line; run top to bottom in Git Bash; stop at the first surprise and paste me its LAST line only):**
```bash
# 1. hivemind — push the seven v67 hub commits (a2d4eb5 … 7e64806 + the close; the tree is clean)
...
```
**3. HONESTY-1's coder paste** — a host-side Claude Code session in `~/Desktop/Code/ClaudeFolder/homesynapse-core` (the tree at `39c8dd3`, clean); the paste is the DISPATCH LINE at the end of `nexsys-hivemind/context/instructions/2026-09-07_coder-lane_HONESTY-1_lastreported-origin-commandeventid-cipaths_coding-instruction.md` (the last fenced block). It runs unattended; its `RETURNED` line is the report.
**4. Dispatch v68** — a FRESH Cowork conversation with `ClaudeFolder` connected; the paste is §3 of `nexsys-hivemind/context/handoff/2026-09-06_PM-mission-control_v67_orchestrator_session_prompt.md` with `<N>` = 68 and `STATE AT DISPATCH:` = `pushed: hivemind <the sha step 1 printed> · docs <the sha step 2 printed> (AMD-53 §1.5) · HONESTY-1: dispatched <hh:mm> · EAR-PALOKI: <your line> · SEARCH-NAME: PALOKI`. v68's ONE deliverable: the PALOKI commissioning packet for Erik (PDF + DOCX + a short body); it hands you the send.
**Done looks like:** `git push` printed `d5bce6e..<sha>` (hivemind) and `8262a3c..<sha>` (docs); the coder session acknowledged; v68 booted. **One line back:** in v68's dispatch line — nothing here.


## §HELD-BY-THE-HUB (edited every beat; you read it, you never remember it)
**Lanes (the cap: TWO + your hands; hardware exclusive):**
| Lane | State (v67 beat 1, 23:2xZ) | Next |
|---|---|---|
...
**v68's blocks, in order (the hub's list, not yours):** **the PALOKI commissioning packet** (the 09-07 packet's shape re-cut: §1 the engagement · §2 the embedded-strings question first (OKI Electric to price; the LOKI wording the identification avoids; PALO) · §3 the read order · Appendix = RS-13 §A + the DuPont walk + the ★ list + the two unscreened axes as asks; one sentence on the change of candidate) → HONESTY-1's landing audit (after your push; CI + install-smoke = the sample; Act 12 retires) → TR-1b's charter (the census ITs + the driver) → FE-NULL-1 (web-ui domain; D4) → R-4c's instruction (Sat) → the v1.1.4 EXPLAIN batch → the B-7 ADR word → W-SKILLS-9 (+ the RETURNED-line template sentence · THE WHOLE-PASTE LAW as arc 52 (v)). **RETIRED this beat:** Block 4 / the VERDOMO packet (UNSENT) · NAMING-B (overtaken).
...
- `EAR-PALOKI: pass <n spellings> | fail <what it heard>` — Act 1 above; `SEARCH-NAME: PALOKI` is final on it.
- `H8: <day> <hh:mm>` — the rig session for H8-a (≤60 min; the packet on disk); Tue passed with no word — name the evening.
- `EXPEDITE: yes | no` — when Erik answers the days + price on the PALOKI packet (the hub recommends in one line against 09-18).
