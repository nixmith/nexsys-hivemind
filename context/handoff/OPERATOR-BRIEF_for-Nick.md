...
status: LIVE — edited at v68 beat 1 (Wed 2026-09-09 ~19:0x CT; instrument 2026-09-10T00:24:42Z). §NEXT = Act 1 the ear test → on pass, the Erik send (the packet is on disk). Act 2 (queued) = the close card's steps 1–3, found NOT RUN at the instrument.
-->

# Operator brief (one act at a time)

## §NEXT — Act 1: the ear test, then the Erik send (one act; ≤10 minutes)
**Step 1 — the ear test (the S4 gate; five minutes).** Cold, to your phone's assistant or dictation: **"Paloki"** ×5, then *"Open Paloki."* · *"Enable the Paloki skill."* · *"Ask Paloki to lock the door."* · *"Paloki, good night."* Count the DISTINCT spellings; note any mishear to *OKI* / *penalty* / *low key*. Pass = ≤3 spellings and no systematic collapse. **On fail: stop; the one line back is `EAR-PALOKI: fail <what it heard>` and nothing is sent.**
**Step 2 — on pass, the send.** Open `nexsys-hivemind/context/strategy/brand-program/2026-09-09_ERIK-EMAIL_PALOKI_short-body_with-attachments.txt`; reply in Erik's thread (his Saturday message is the last in it); paste the body whole; attach the two files beside it — `PALOKI_Search_Packet_2026-09-09.pdf` and `PALOKI_Search_Packet_2026-09-09.docx`; send. The body carries the turnaround rule (standard; the opinion wanted by Fri 09-18; Erik takes the expedite himself if his queue misses it). Say `REVERT EXPEDITE` before sending only if you want the paid expedite outright — I swap one sentence.
**Done looks like:** nine tries, a count; one email sent with two attachments. **One line back:** `EAR-PALOKI: pass <n> · SENT <hh:mm>` (or the fail line).
**Act 2 — handed on your line, not before:** the close card's steps 1–3, unchanged from v67 — push hivemind (8 commits after this beat) · the AMD-53 docs commit (`git add -u` + the msg file on disk) · HONESTY-1's coder paste. If the coder session is already running, say `HONESTY-1: running <hh:mm>` and step 3 drops.

## §HELD-BY-THE-HUB (edited every beat; you read it, you never remember it)
**Lanes (the cap: TWO + your hands; hardware exclusive):**
| Lane | State (v68 beat 1, 00:04Z) | Next |
|---|---|---|
...
- `REVERT EXPEDITE` — only if you refuse the date rule in the body (standard turnaround; the opinion wanted by 09-18; Erik takes the expedite himself if his queue misses it — the v68 b1 intake §6); silence = it stands. `EXPEDITE: yes` = the certainty option, one sentence swap before the send.
