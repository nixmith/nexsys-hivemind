...
GOAL. Wave 1 (the full hero set) is on the desk; Wave 2 + the M9/M7.4 software are not here yet — so this is the optimal window to **FRONT-LOAD the hardware ground-truth**. Bring up the MG24/EZSP coordinator and characterize the curated device set, building the durable device corpus, **saving real captured device data as durable TEST FIXTURES** (which become M9's acceptance ground-truth AND M7.4's E2E-test inputs), and validating the design docs against real silicon — so M9 is built against ground truth and the repeatable **characterize → corpus → fixture → validate → integrate** onboarding pipeline is hardened (so Wave 2 and every future device/integration flow through it seamlessly). The MG24/EZSP runs the hero (motion→light) on its own — do NOT wait on the ZNP stick.
