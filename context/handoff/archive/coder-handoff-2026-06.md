# ROTATED 2026-06-11 (hygiene pass A1, M6.2-return session) — from `context/handoff/coder-handoff.md`: the M4-Readiness section + gate records M6.4 → M3.7 (everything older than the live `### DEFERRED — M6.2`)
...
### DEFERRED — M6.4: Hot-Reload Atomic Swap + UI/API Write Path (AMD-66/67/70/71 + R1/R2/R4/R5; 2026-06-11) — RESOLVED-GREEN, COMMITTED `62a81e6`

**⟳ Gate-fix round 1 (2026-06-11):** Nick's first run failed ONE test — `ConfigLayoutTest.composeAfterMerge` (AssertionError at :264; 169 config tests / 1 failed; both runs halted at `:config:configuration:test`, so nothing downstream executed). **PM diagnosis (confirmed): stale M6.1a-era pin, not an implementation bug** — the test asserts `rootDrafts.hasSize(1)` for a load whose fixture carries one ERROR issue (zigbee `channel: 5` vs `minimum: 11`), and the ratified R1/DP-10 obligation now adds one `config_error` per ERROR to the same pass → 2 drafts. **GF-1 (the only change, 1 file):** `ConfigLayoutTest.composeAfterMerge` — model assertions kept; `hasSize(1)`→`hasSize(2)`; order-agnostic type-filtering (exactly one `ConfigErrorEvent`: path `endsWith("channel")`, severity `"ERROR"`, appliedDefault `"15"`; exactly one `ConfigValidationCompletedEvent`: `severityCounts containsEntry("ERROR", 1)` — the pre-existing assertion); dated pin-bump comment; `+import ConfigErrorEvent`/`EventDraft`. Implementation untouched. **R-B condition CONFIRMED (no fix needed):** backup-copy failure skips the rename — `copyBackup`/`writeAtomically` are sequential statements in one try (`StandardConfigurationService.java:888-889`, catch :895); the original is never replaced without its on-disk backup. Defensive module-wide `rootDrafts` sweep: no other stale pin (consistent with the 169/1 run). Rulings recorded: **R-A/R-C/R-D ACCEPTED as implemented; R-B ACCEPTED with the confirmed condition.**

**Build gate:** **RE-DEFERRED to Nick after ⟳ round 1.** Files produced only (CLAUDE.md build discipline). The instruction-encouraged P5 shift-left targeted `compileJava` was attempted and is **permission-blocked in this environment** (same as M6.1a); mitigation per the 2026-06-10 lesson — the NEW third-party API surface (snakeyaml-engine Dump side: `Dump(DumpSettings)`, `dumpToString(Object)`, `DumpSettings.builder().setDefaultFlowStyle(FlowStyle.BLOCK)`, `FlowStyle.BLOCK`) is **javap-verified from the cached 2.9 jar**, and the module's `exports org.snakeyaml.engine.v2.common` + `…v2.api` confirmed from `META-INF/versions/11/module-info.class`. No other new third-party API is used (Jackson tree walks reuse M6.1a patterns). Round-1 caveat stands: the halted runs mean event-model/persistence tests, `-Werror` compilation of the new files, spotless, and ArchUnit are still unexercised — the re-run must be the FULL gate.

**Commit the gate must run against:** the M6.4 working tree (incl. ⟳ GF-1) layered on HEAD `7e0bce8` (the M6.1-complete docs commit). **NO `module-info.java`/`build.gradle.kts`/`projectionVersion`/`libs.versions.toml` change in ANY module; NO event-store migration.**

**Exact command (success = GREEN; per Nick's round-1 note, run the FULL gate — expect 147 tasks):**
```
./gradlew check          # full gate — all module tests + -Werror compile + ArchUnit (NO_DIRECT_TIME_ACCESS + NO_JACKSON_IN_DOMAIN_MODEL + Rule 5 config exemption) + spotless + modules-graph-assert
```

**Files CREATED (7):** config main `SchemaAnnotationIndex.java`, `ConfigModelDiff.java`, `AtomicYamlWriter.java` (all package-private final); config test `StandardConfigurationServiceReloadTest.java`, `StandardConfigurationServiceWriteTest.java`; event-model main `ConfigSectionReloadedEvent.java`; event-model test `ConfigSectionReloadedEventTest.java`.

**Files MODIFIED (15 source + 3 MODULE_CONTEXT):** config main `StandardConfigurationService.java` (reload/write un-staged; DP-1..12; DP-10/DP-11), `ConfigChangeSet.java` (+3 filters); config test `StandardConfigurationServiceTest.java` (staging tests removed; +DP-10/DP-11 suites; type-filtering helper), `ConfigLayoutTest.java` (⟳ GF-1 — the `composeAfterMerge` draft-count pin 1→2 per R1/DP-10); event-model main `EventTypes.java` (+constant, manifest 23→24, Javadoc counts ×3); event-model test `EventTypesTest.java` (54→55), `EventTypeAnnotationTest.java` (+record, 23→24, method renamed `exactlyTwentyFourAnnotatedRecords`); persistence main `EventCategoryMapping.java` (+1 `[SYSTEM]` entry); persistence test `EventCategoryMappingTest.java` (+1 test, 35→36), `EventTypeRegistryTest.java` (23→24 ×3, 35→36 ×2), `JacksonWarmupTest.java` (35→36, 23→24 ×2), `AllEventClasses.java` (Javadoc ×3), `TestEventSamples.java` (+sample +import), `EventPayloadCodecTest.java` (Javadoc, header, +round-trip); integration-tests `IntegrationTestHarness.java` (Javadoc count only). Plus MODULE_CONTEXT.md ×3 (config 28→31 types + M6.4 rows/gotchas/notes; event-model 48→49 + manifest counts + new row; persistence EventCategoryMapping row 35→36 + M6.4 section).

**Known environment-sensitive points for the gate:** (1) the torn-read concurrency test spins one reader thread for ~50 reload iterations — wall-clock-bounded by `join(30_000)`, no sleeps; (2) the §6.8/crash-window tests inject failure by pre-creating a DIRECTORY named `homesynapse.yaml.tmp` (portable — `FileChannel.open(CREATE,WRITE)` on a directory fails on both NTFS and POSIX); (3) `Files.move(…, ATOMIC_MOVE)` over an existing target: POSIX `rename(2)` and Windows `MoveFileEx(MOVEFILE_REPLACE_EXISTING)` both replace — if the gate surfaces an `AtomicMoveNotSupportedException` on an exotic filesystem, the fallback is adding `REPLACE_EXISTING` to the move options in `AtomicYamlWriter.writeAtomically` (one line).

**Next work unit (refuse-to-close):** **M6.2 — SecretStore + key management (AMD-68 `setAll`; crypto-gated per `context/planning/2026-06-08_M6-charter.md`)** upon (a) this gate GREEN + commit + PM WUCP Phase 2, and (b) the PM's M6.2 coding instruction. M6.2 picks up `!secret`/`!env` stage-3 tag resolution (the `YamlLoader` tag-constructor map is the seam; both tags are FATAL-unknown today) and now REBASES its `StandardConfigurationService` touches on the M6.4 delta (the PM's sequencing note anticipated exactly this — M6.2's tag-resolution is the more rebasable delta). R1 (config_error) is now DONE here — M6.2's instruction should drop that item. M6.3 (at-rest encryption) remains triple-gated behind M6.2.

### DEFERRED — M6.1a: Config Pipeline, Load/Validate Half (AMD-66/67/70/71 behavior; 2026-06-10)

**⟳ Gate-fix round 1 (2026-06-10):** Nick's first gate run FAILED twice — `YamlLoaderTest.nullValuedKeysDropped` (`:config:configuration:test`, AssertionError at YamlLoaderTest.java:167) and `HomeSynapseArchRulesTest.noDirectFilesystemInCore` (`:app:homesynapse-app:test`). Both PM-diagnosed and ruled; exactly the two fixes applied, nothing else. **(1) Implementation bug — YamlLoader scalar schema:** snakeyaml-engine's DEFAULT `LoadSettings` resolves scalars with the JSON schema, where `~` is the string `"~"`, not null; LTD-09 mandates YAML 1.2 → `.setSchema(new CoreSchema())` added to BOTH `rootSettings()` AND `includedSettings(...)` (they must match, or root and included files resolve the same literal differently — the match constraint is now stated in the `includedSettings` Javadoc) + `import org.snakeyaml.engine.v2.schema.CoreSchema` + the class-Javadoc safety-properties list now records Core-schema resolution (`~`/`null`/empty → null = unset). `CoreSchema` javap-verified in the cached 2.9 jar BEFORE the edit (public no-arg ctor; extends `JsonSchema` implements `Schema`; `LoadSettingsBuilder.setSchema(Schema)` confirmed; the `…v2.schema` package IS exported by the explicit module — no `module-info` change needed). The failing test was NOT changed (it is correct). **(2) Stale arch rule vs ratified AMD-71 (PM RULING approved):** `HomeSynapseArchRules` Rule 5 `NO_DIRECT_FILESYSTEM_IN_CORE` package list drops `com.homesynapse.config..` ONLY (event/device/state/automation stay; Rule 4 `NO_REVERSE_DEPENDENCIES` still lists config — untouched) — ratified AMD-71/[AMD-71-A] makes config the designated filesystem consumer rooted at the composition-root-injected configDir `Path`; the compensating control is AMD-71-INV-01 canonicalization-containment, test-pinned by `ConfigLayoutTest`. Ruling recorded, dated, in the rule's banner comment + field Javadoc; the `.as(...)` description string does not enumerate packages — untouched. **Rule-5-scope pin survey (ruled grep, report-only — no docs edited):** the ONLY doc enumerating Rule 5's package scope incl. config is `homesynapse-core-docs/research/homesynapse-core-cloud-scalability-analysis.md:56` ("…the core event model, device model, state store, automation, and configuration modules are already filesystem-agnostic") — now stale, docs lane. Name-only mentions (scope not enumerated — unaffected): `nexsys-hivemind/project-knowledge/HomeSynapse_Knowledge_Primer.md:385`, `…/CLAUDE_PROJECT_CORE_custom-instructions.md:35`, `nexsys-hivemind/context/relay/2026-05-28_codebase-investigation-for-rust-deliberation.md:400`, `homesynapse-core-docs/research/2026-05-16_…Top_Down_Architectural_Analysis.md:305/692`, archived `homesynapse-core/docs/archive/project-state-reports/…2026-04-08.md:235`. `HomeSynapseArchRulesTest` references the constant only → goes green with the rule fix. **Gate-fix files touched (2 source):** `config/configuration/src/main/java/com/homesynapse/config/YamlLoader.java`, `app/homesynapse-app/src/test/java/com/homesynapse/app/HomeSynapseArchRules.java` (+ the config MODULE_CONTEXT Core-schema gotcha, this handoff, a cross-agent note). Re-deferred to Nick; expected GREEN.

**Build gate:** **RESOLVED-GREEN 2026-06-10** — after ⟳ gate-fix round 1, Nick ran `./scripts/clean.sh && ./gradlew :config:configuration:check` (27 tasks) + full `./gradlew check` (147 tasks) **GREEN**; **COMMITTED `9035110`** (Nick host-git; sha reconciled by the PM 2026-06-10 against `git log` HEAD). PM WUCP Phase 2 → APPROVE. _Original record:_ Files produced only; no `./gradlew`/`javac`/`git` run in-session (the targeted P5 shift-left compile was attempted and is permission-blocked in this sandbox; third-party API risk was mitigated instead by `javap` verification of snakeyaml-engine 2.9 + networknt 1.5.6 jars from the Gradle cache — exception hierarchies, builder signatures, and the ValidationMessage instanceLocation/property semantics are bytecode-confirmed).

**Commit the gate must run against:** the M6.1a working tree layered on HEAD `b7bc65c` (the M6.1b commit). **Changes: config module + (⟳ gate-fix r1) ONE file in app/homesynapse-app TEST sources (`HomeSynapseArchRules.java` — Rule 5 package list)** — `module-info.java` +5 third-party non-transitive `requires` (Nick ruling 2026-06-10; HomeSynapse edge set unchanged), `build.gradle.kts` +2 `implementation` lines (jackson-databind, slf4j-api — catalog entries already existed), NO `libs.versions.toml`/`projectionVersion`/event-model/persistence change, NO P2 manifest change (survey CLOSED at M6.1b).

**Exact commands (success = GREEN):**
```
./gradlew :config:configuration:check
./gradlew check          # full gate — ArchUnit (NO_DIRECT_TIME_ACCESS + NO_JACKSON_IN_DOMAIN_MODEL) + spotless + modules-graph-assert
```

**Files CREATED (9):** config main `YamlLoader.java`, `JsonSchemaCompositeValidator.java`, `StandardSchemaRegistry.java`, `StandardConfigurationService.java` (all package-private final); config test `YamlLoaderTest.java`, `ConfigLayoutTest.java`, `JsonSchemaCompositeValidatorTest.java`, `StandardConfigurationServiceTest.java`, `RecordingEventPublisher.java` (test helper).

**Files MODIFIED (3):** `config/configuration/src/main/java/module-info.java` (+5 ruled `requires`), `config/configuration/build.gradle.kts` (+2 lockstep lines), `ConfigurationChangeListener.java` (Javadoc registration-list wording per the DP-4 ruling). Plus `config/configuration/MODULE_CONTEXT.md` (24→28 types, JPMS/Gradle sections, gotchas, Phase-3 notes).

**Known environment-sensitive test:** `ConfigLayoutTest.symlinkEscapeRejected` is `Assumptions`-guarded — it SKIPS (not fails) where symlink creation needs privileges (default non-developer-mode Windows). It should RUN on Nick's setup if Developer Mode is on, and on any Linux gate.

**Next work unit (refuse-to-close):** **M6.2 — SecretStore + key management (AMD-68 `setAll`; crypto-gated per `context/planning/2026-06-08_M6-charter.md`)** upon (a) this gate GREEN + commit, and (b) the PM's M6.2 coding instruction. M6.2 should also pick up `!secret`/`!env` stage-3 tag resolution (the YamlLoader seam is the tag-constructor map; both tags are FATAL-unknown today) and decide R1 (per-ERROR `config_error` publication). M6.4 (hot-reload) trails M6.2 and MUST re-run the P2 consumer/pin survey for `config.section_reloaded` (AMD-70 §9 checklist item).

### DEFERRED — M6.1b: Config Pipeline, Access/Events Half (AMD-66/67/70 shapes; 2026-06-09)

**Build gate:** **RESOLVED-GREEN 2026-06-10, COMMITTED `b7bc65c`** — Nick ran both commands below GREEN (57 + 147 tasks); sha reconciled against `git log` HEAD at the M6.1a session start. _Original record:_ DEFERRED. The Coder produced files only (CLAUDE.md build discipline — no `./gradlew`/`javac`/`git`); the in-session sandbox did not run the P5 shift-left compile.

**Commit the gate must run against:** the M6.1b working tree layered on HEAD `6c6dd33`. No commit made by the Coder. **NO `module-info`/`build.gradle.kts`/`projectionVersion`/`libs.versions.toml` change** — `config/configuration/src/main/java/module-info.java` is byte-identical to `6c6dd33` (the M6.1 STOP-gate property, verified before and after).

**Exact commands (success = GREEN):**
```
./gradlew :config:configuration:check :core:event-model:check :core:persistence:check
./gradlew check          # full gate — ArchUnit (NO_DIRECT_TIME_ACCESS + NO_JACKSON_IN_DOMAIN_MODEL) + spotless + modules-graph-assert
```

**Files CREATED (7):** config `ConfigurationChangeListener.java` (public, AMD-66 §2.1 verbatim shape) + `ScopedConfigurationAccess.java` (package-private); event-model `ConfigValidationCompletedEvent.java` (AMD-70 flattened payload, sum==issueCount guard); tests `ConfigModelTest`, `ConfigMigratorChainTest`, `ScopedConfigurationAccessTest`, `ConfigurationChangeListenerTest` (config), `ConfigValidationCompletedEventTest` (event-model).

**Files MODIFIED (16):** config main `ConfigModel.java` (5→6 components, AMD-67 guards + AMD-54 cross-ref Javadoc), `ConfigMigrator.java` (3→5 methods), `MigrationPreview.java` (4→6 components + guards); config testFixtures `TestConfigFactory.java` (×5 ctor sites, E67-2); config test `InMemoryConfigAccessTest.java` (accessor + DisplayName); event-model main `EventTypes.java` (+`CONFIG_VALIDATION_COMPLETED`, roster 22→23, Javadoc counts); event-model test `EventTypesTest.java` (53→54), `EventTypeAnnotationTest.java` (+record, 22→23, method renamed `exactlyTwentyThreeAnnotatedRecords`); persistence main `EventCategoryMapping.java` (+1 `[SYSTEM]` entry, sibling-placed); persistence test `EventCategoryMappingTest.java` (+1 test, 34→35), `EventTypeRegistryTest.java` (22→23 ×2, 34→35), `JacksonWarmupTest.java` (34→35, 22→23), `AllEventClasses.java` (Javadoc counts), `TestEventSamples.java` (+sample), `EventPayloadCodecTest.java` (+round-trip, header counts); integration-tests `IntegrationTestHarness.java` (Javadoc count only). Plus MODULE_CONTEXT.md ×3 (config/event-model/persistence).

**`[BLOCKING]` ESCALATION (M6.1a held):** see the Last-updated header block — the load/validate classes need third-party `requires` directives the instruction forbids; awaiting PM/Nick ruling. M6.1a files were deliberately NOT written.

**Next work unit (refuse-to-close):** ~~M6.1a~~ **DONE 2026-06-10 — delivered per the RULING ADDENDUM; see `### DEFERRED — M6.1a` above for its open gate.** _Original pointer (executed):_ M6.1a — config load/validate (YamlLoader + JsonSchemaCompositeValidator + StandardSchemaRegistry + StandardConfigurationService + AMD-71 behavior + the `config.validation_completed` publish path + AMD-66 registration) — UNBLOCKED by the Nick 2026-06-10 ruling (5 third-party non-transitive `requires` + 2 Gradle lockstep lines; DP-4 → `List` ctor; DIAGNOSTIC publish metadata). M6.2/M6.4 remained gated on M6.1 landing whole — M6.1 is now delivered whole pending the M6.1a gate.
...
- `core/persistence/src/test/java/com/homesynapse/persistence/SqliteViewCheckpointStoreTest.java` (MODIFIED) — same substitution at both call sites (default `dbE