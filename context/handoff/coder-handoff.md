...
**Scope delivered:** M5-A **Part 1** (platform-systemd impls) + **Part 3** (FloorId registration) ONLY. **Part 2 (AMD-87 `Expectation` persisted codec) was NOT implemented** — it is ⛔ gated on AMD-87 lightweight ratification per the instruction. The persistence `module-info` was therefore NOT given `requires com.homesynapse.device`, no `ExpectationSerializer`/`ExpectationDeserializer` were created, the `Expectation` serde was NOT added to `PersistenceJacksonModule`, and `EventPayloadCodecTest.capabilityAdded_onOff_roundTrips` remains `@Disabled("AMD-65 pending")` (untouched).
...
**Next work unit (refuse-to-close):** **M5-A Part 2 — AMD-87 `Expectation` persisted codec** (⛔ currently GATED on AMD-87 lightweight ratification). When AMD-87 is RATIFIED: add `requires com.homesynapse.device` to persistence `module-info` (STOP-gate G3 — verify the graph stays acyclic), create `ExpectationSerializer`/`ExpectationDeserializer` (tagged-union over the 4 permits; `WithinTolerance` gets AMD-52 bit-anchored-float treatment), register the `Expectation` pair in `PersistenceJacksonModule`, and delete the `@Disabled` from `EventPayloadCodecTest.capabilityAdded_onOff_roundTrips`. If AMD-87 is not yet ratified, the Coder is BLOCKED on either ratification or the PM's next M5 lane (e.g. M5 C4 verification-foundation). See `context/coding-instructions/M5-A_Platform_Impls_AMD87_FloorId.md` §Part 2 + `design/amendments/AMD-87_Expectation_Persisted_Codec.md`.
