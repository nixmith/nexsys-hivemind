...
**Status:** **RESOLVED — no open deferred build gates.** **M4.0b-2** (AMD-50 backfill + `projectionVersion` 1→2) GREEN and committed `7610296` — Nick ran `./gradlew :core:state-store:check` + `:lifecycle:lifecycle:check` + `./gradlew check` (139 tasks) GREEN + `:testing:integration-tests:test -PpiProfile=throttled` passing, 2026-05-29; PM WUCP Phase 2 closed. Prior: **M4.0b-1** GREEN `cf1a97e`; **M4.0a + D-1 correction** RESOLVED (`a441fdf`); all M3.7 gates RESOLVED 2026-05-27.
