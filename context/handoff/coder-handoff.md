...
**Status:** **RESOLVED — no open deferred build gates.** **M4.B3 (AMD-47)** GREEN — Nick ran `./gradlew :core:device-model:check` (24 tasks) + `./gradlew check` (139 tasks, full regression) GREEN, 2026-05-30, after a clean build; **PM WUCP Phase 2 closed** (every changed file read vs source; AMD-47 §2.1–§2.6 + INV-01..05 confirmed; the two `[REVIEW]` items adjudicated ACCEPT). Commit pending Nick (DEC-M3-12). Prior: **M4.0b-2** (AMD-50 backfill + `projectionVersion` 1→2) GREEN and committed `7610296` — Nick ran `./gradlew :core:state-store:check` + `:lifecycle:lifecycle:check` + `./gradlew check` (139 tasks) GREEN + `:testing:integration-tests:test -PpiProfile=throttled` passing, 2026-05-29; PM WUCP Phase 2 closed. **M4.0b-1** GREEN `cf1a97e`; **M4.0a + D-1 correction** RESOLVED (`a441fdf`); all M3.7 gates RESOLVED 2026-05-27.
