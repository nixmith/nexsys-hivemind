...
state-type: current
status: CURRENT
last-verified: 2026-05-29 against M4.0b-2 closeout working tree (M4.0b-2 COMPLETE, committed `7610296`, build GREEN)
-->

# PM Session Handoff

**Last updated:** 2026-05-29 (M4.0b-2 COMPLETE — AMD-50 version-transition backfill + `projectionVersion` 1→2 committed `7610296`, build GREEN; WUCP Phase 2 closed. Next: M4.B3 → M4.0b-3.)
...
**Next: M4.B3 → M4.0b-3.** **M4.B3** = device-model `AttributeValue` expansion (`QuantityValue`/`ArrayValue`/`DegradedAttributeValue`/`AttributeValueUpcaster`, AMD-47), **gated on P4 doc currency**. **M4.0b-3** = typed comparator (AMD-51) + typed `StateChangedEvent` (AMD-52), gated on M4.B3 — a clean rule-swap that reuses AMD-50's backfill path unchanged for the 2→3 transition (the supersession test is the standing N→M regression guard for that reuse). **Open prereqs / doc-currency follow-ups (none block the M4.0b-2 commit):** P3 (Research 6 NQ-1..6, Nick's calls) → Workstream C; P4 (Doc 02/05 currency) → Workstream B/C briefs; propagate the M4.0b-2 re-scope + M4.0b-3 row into PLAN-M4-CONSOLIDATED-v2 §3; KB currency (`HomeSynapse_Current_State.md` / `Knowledge_Primer`); the timestamp-model unifier WU. See Next Tasks for the ordered list.
...
0a. **DOC-CURRENCY CLEANUP PUNCH-LIST (do before / alongside M4.B3 — these are the "gaps in understanding" that can mislead a future Claude Code/Cowork session).** None are in the hivemind repo; all are in `homesynapse-core-docs` (Nick already committed the M4.0b-2 docs, so these are a fresh, separate docs commit). PM can execute on request:
    ...
    - **(iii) P4 — IN PROGRESS (2026-05-30, Doc 02 half DONE).** **AMD-47** (device `AttributeValue` expansion — `QuantityValue`/`ArrayValue`/`DegradedAttributeValue` + `AttributeValueUpcaster` SPI + 3 `AttributeType` constants `QUANTITY`/`ARRAY`/`DEGRADED` + AMD-47-INV-01..05) authored **PROPOSED** at `design/amendments/AMD-47_AttributeValue_Expansion_and_Upcaster.md`; Doc 02 §3.7/§8.2 carry inline "PENDING AMD-47" delta blocks + the JSR 385 `Unit<?>`→`String` unit-field currency fix (REC-93 makes hand-rolled units permanent). **AMD-47 confirmed against P2 §3** (device block 46–49; "AMD-47-equiv withdrawn" was an unauthored `EntityState.category` placeholder — `EntityCategory` is now AMD-46). **Awaiting Nick ratification** (AMD-47 §9; one fork: add `AttributeType.QUANTITY` [PM-rec] vs. reuse `FLOAT`). **M4.B3 becomes authorable the moment Nick ratifies AMD-47** — its other gate (Doc 02 currency) is done. On ratification: PM records AMD-47 alongside the AMD-50 watermark, registers AMD-47-INV-01..05 into `Architecture_Invariants_v1.md`, folds the Doc 02 PENDING blocks into the body, logs AMD-47 in the KB ledger, briefs M4.B3. **M4.B3 NOT started** (it is the implementation WU — three records + SPI + 3 AttributeType constants + §5 contract tests + upcaster wiring per AMD-47-INV-02; no `projectionVersion` bump). **Doc 05 half STILL OPEN + P3-gated:** Doc 05 (Integration Runtime) integration-api freeze content (Research 6 REC-41–51) cannot be brought current until Nick resolves Research 6 NQ-1..6 (task #1); integration AMD numbers are assign-at-milestone (P2 §4). Doc 05 gates **Workstream C**, not M4.B3 — not authored this session.
