...
as-built-erratum: 2026-06-19 — AB-3 landed GREEN (./gradlew check 149 tasks); PM WUCP Phase 2 APPROVE. Full record in pm-handoff (2026-06-19 AB-3 entry). The issued instruction had 5 defects the Coder corrected at build (none changed the milestone outcome — the FIX-07 precedent): (a) the AutomationEngineAssembly seam returns event.bus.Subscriber on the exported API → forces `requires transitive com.homesynapse.event.bus` + Gradle `api(...)` (the api<->requires-transitive lockstep; this instruction's "automation module-info UNCHANGED" was WRONG). (b) lifecycle also needs `requires com.homesynapse.platform.systemd` for the HealthReporter impls (missing from the §module-info diff; gate-allowed). (c) the automation core-schema must register in Phase 1 BEFORE config.load() validates the `automation:` section (this instruction's §3b had register+load adjacent, not phase-ordered). (d) the loader takes the `automation` SECTION content: `loader.load(rawMap().get("automation"))`, NOT `loader.load(rawMap())` — the loader reads top-level `automations`, so passing the whole rawMap silently dropped every automation (this instruction's §3b verbatim AND the AutomationSchema + loader Javadocs all carry this latent bug; doc-currency to fold). (e) SystemdHealthReporter throws on JDK-21 (OR-M13-SDNOTIFY) → the reporter selector falls back to NoOp on construction failure (§1d didn't carry the risk). PM rulings folded at WUCP Phase 2: SD-9 = surface-and-continue, NOT FATAL-boot (the loader contract surfaces a bad def as config_error + loads valid siblings; Doc 12 §4 FATAL = subsystem-init, not a per-entry error); HomeId made durable (read-or-create configDir/home_id, AMD-34).
-->
...
- **PD-3 — HTTP-test reconciliation + the `start()` signature ripple (RESOLVED 2026-06-19 — PM ruling on Coder pushback; ACCEPTED. The consumer/pin survey missed the `.start().join()` callers + the HTTP-test fallout; corrected here).** The `void start()` reconciliation (PD-1) breaks every `core.start().join()` caller, and gating production HTTP closed (C1) collides with the existing HTTP E2E tests. **Ruling: the explicit test-only HTTP-bring-up seam (Coder Option 1) — NOT mothballing (Option 2 rejected: coverage regression + remove-now/re-add-at-AB-1 churn + it over-reads C1).**
  - **Refactor the inline HTTP bring-up** (current `start()` steps 12–14: Javalin + `installReadinessGate` + entity-query + admin endpoints + `app.start(port)`) **into an explicit, non-default method on HSC** (e.g., package-private/test-visible `openExternalInterfaces()`), and **do NOT call it from the production phase progression.** Production `start()` runs Phases 0–6 with the External-Interfaces phase (openHAB 70) **gated CLOSED** → production binds no port (C1 honored; `start_doesNotOpenHttpSurface` is the regression guard). **This explicit method IS the AB-1 seam** — AB-1 wires `AuthMiddleware`/`RateLimiter` into it, makes the production progression invoke it behind auth + the loopback bind posture, and the test path becomes the production path behind auth. (Resolves the "do not register endpoints" tension: the boundary is "production does not OPEN HTTP," not "the HTTP code must not exist.")
  - **`start()` reconciliation shape:** public `void start() throws Exception` (the SLM contract) blocks internally — a private async bootstrap `.join()`-ed with `CompletionException` unwrapped to its cause, or a synchronous body. Existing callers migrate `core.start().join()` → `core.start()`.
  - **Keep the HTTP E2E coverage GREEN — do NOT `@Disable`.** Migrate the named test files (below) to the new contract: call `start()`, then `openExternalInterfaces()` where HTTP is exercised (unauthenticated, as today — AB-1 hardens it). **Do NOT remove HTTP code from HSC.** Blast radius = the two named test files, not the production REST layer.
...
| MODIFY | `lifecycle/lifecycle/src/test/java/com/homesynapse/lifecycle/HomeSynapseCoreTest.java` | **(PD-3)** Migrate `start().join()` → `start()` (the void/blocking contract); any HTTP assertion moves to the explicit `openExternalInterfaces()` path. Add/keep the C1 guard (production `start()` binds no port). |
| MODIFY | `testing/integration-tests/src/test/java/com/homesynapse/it/HomeSynapseE2eHarness.java` | **(PD-3)** The shared E2E/IT harness — migrate to `void start()` + call `openExternalInterfaces()` for the HTTP-dependent ITs (e.g., `InFlightRequestShutdownIT`), keeping them GREEN. Re-verify the exact `.start().join()` call sites at the issue HEAD before editing. |
...
- **`start()` caller migration (the `void`-vs-`CompletableFuture` ripple — the survey miss the Coder caught, PD-3):** grep `\.start()\.join()` / `\.start()\.get(` across ALL source. At `beb4bc3` the callers are `lifecycle/.../HomeSynapseCoreTest.java` + `testing/integration-tests/.../HomeSynapseE2eHarness.java` (the shared IT harness that the HTTP ITs ride). Migrate each to the blocking `void start()`. Production `Main` is rewritten anyway. Re-grep at the issue HEAD — do not assume the list is exhaustive.
