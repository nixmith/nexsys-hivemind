...
- **✅ STATUS: DISPATCHED + ASSESSED 2026-06-15 (grade A; NEW-1 folded in).** Disposition: `context/assessments/2026-06-15_Research_R-alpha_PM_Assessment.md`. Outcome: rotate-DEK-on-restore is the BINDING restore contract (carry-high-water-mark = cross-check only) → closes OR-M6-NONCE restore-half via a Doc 15 §6 currency amendment (Nick ratifies); the cause-discriminated read contract is adopted (CASE-a recorded-shred → degrade-to-DegradedEvent / CASE-b unintended → fail-closed; new primitive = chain-covered shred tombstone) = the design answer to converge C2. REC-216..235 routed to the crypto-shred WU + the app-bootstrap milestone; GCM-SIV / committing-AEAD / SP 800-38D Rev.1 parked Tier-2. **Next side-research dispatch = R-β.**
