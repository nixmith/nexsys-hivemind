...
10. **The `sys_*` namespace question (from the FE-HONEST-1 eyeball, ruled non-blocking 2026-08-31):** the registry-census resolver flags `sys_sun` "(not in this hub's registry)" on complete-census logic — truthful at the letter; whether system-provided refs should be census-resolved at all (or carry a system-namespace posture) is adjudicated AT the H8 real-wire exercise (the R-4 clone rig, foreign refs), which also discharges FE-HONEST-1's LIVE-VERIFICATION-PENDING register. Companion note: `Available`+`Not-found` pairs on real wire would be a finding (in mock they are unseeded fixtures).
