...
state-type: current
status: CURRENT
last-verified: 2026-06-05 against HEAD `e76b925` — M4.B-S2 (AMD-44 Stage 2: EntityRole) DONE `e76b925`, PM Phase 2 → APPROVE; **Workstream B COMPLETE**. **M4.C row added (PLANNED — GATED on AMD-54..64 ratification; block authored at the 2026-06-05 C-briefing).** SPIKE-DC scheduled (~Aug). Prior this chain: M4.B-S1 DONE `e73e199`; M4.0b-5 (AMD-53) DONE `c99b425` (`projectionVersion`=5, **Workstream A complete — typed end-to-end AND event-time-deterministic**); M4.0b-4b (AMD-52) DONE `72596cb`; M4.0b-4a (relocation) DONE `971cfa1`; M4.0b-3 (AMD-51) DONE `98f705b`.
-->
...
| **M4.C** | **Workstream C — integration-api interface freeze (AMD-54..64):** descriptor config-schema versioning + `schemaVersion`→`descriptorSchemaVersion` rename (54); four `default` adapter lifecycle hooks + `ConfigUpdateOutcome`/`MigrationOutcome` (55); `ExceptionClassification.AUTH_FAILED` (56); `HealthDetail` 12-value enum + `IntegrationHealthRecord` 13→14 (57); five new `IntegrationLifecycleEvent` permits, 5→10, dot-namespaced `integration.*` strings + three-way registration lockstep (58); `CapabilityEvent`/`CapabilityAdded`/`CapabilityRemoved` + `CapabilityPublisher` + `DiscoveryServices` per NQ-3/4 (59); `SecurityServices`/`CredentialRotator` per NQ-1 (60); `softDependencies` (61); `BackoffParameters` + the NQ-5/6 restart-intensity Javadoc fold (62); `IsolationLevel` reservation (63); `plannedRestartTimeout` (64). `IntegrationDescriptor` 8→14, `IntegrationContext` 10→12, `RequiredService` 3→5 — all with back-compat convenience ctors. **Contract-only — supervisor impl = M9. NO `module-info`/Gradle/`projectionVersion` change.** | integration/integration-api + integration/integration-runtime + core/event-model (string constants) + core/persistence (serde tests) | **PLANNED — GATED on AMD-54..64 ratification** | Block authored 2026-06-05 vs `e76b925` (all PROPOSED). DOCS-Project review prompt: `context/instructions/2026-06-05_AMD-54-64_DOCS_Project_Review_Prompt.md` (review-flags R1–R7). Gated instruction drafted: `context/coding-instructions/M4.C_Integration_API_Freeze.md` — **do not issue until RATIFIED + deltas folded + shapes re-verified vs HEAD**. Ratification mechanics owe: invariant registration (§24+), watermark AMD-53→AMD-64, Doc-05 body-fold, Doc 02 §11.2/§3.10/§4.2 Stage-2 currency fold, nav-index rows. |
