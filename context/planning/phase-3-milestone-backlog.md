...
| **M3.7** | E2E HTTP integration tests + abandon() crash-recovery contract + checkpoint key fix + TESTING checkpoint policy. 5 Cowork fix rounds + 2 Claude Code briefs. Eighteenth CC WU (counting the final checkpoint-fix brief). M3 COMPLETE. | testing/integration-tests + persistence + event-bus + state-store + lifecycle + rest-api | **DONE 2026-05-27** | — (pending commit) | Full `./gradlew check` GREEN (139 tasks). 18 modified + 3 new files. CrashRecoveryHttpIT passes — rehydration path exercised for first time. AMD-45 drafted (deferred to M4.0). |
