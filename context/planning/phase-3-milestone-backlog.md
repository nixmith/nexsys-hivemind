...
| **M4** | **Foundation: device-model expansion + projection/derivation + integration-api interface freeze** | event-bus (M3), persistence (M2), device-model | **SCOPED 2026-05-28 (Canonical)** | **Re-baselined.** `StateQueryService` already shipped in M3.6e.1. M4 now = **Workstream A** (M4.0a `AtomicCheckpointWriter`/AMD-45 → M4.0b `DispatchingProjectionAdvancer` REC-28 + real `DerivationRule` + one-shot backfill on the `projectionVersion` 1→2 replay), **Workstream B** (Research 8 REC-23–30 + AMD-44 Floor/EntityRole impl), **Workstream C** (Research 6 REC-41–51 integration-api interface freeze; supervisor impl = M9). Plan: `homesynapse-core-docs/design/HomeSynapse_Core_M4_Implementation_Plan_PLAN-M4-CONSOLIDATED.md`. Open: P2 AMD renumbering, P3 Research 6 NQ-1..6. Note: config=M6, automation=M7/M8, integration-runtime impl=M9 (rows below) are unchanged and align with the plan. |
