...
state-type: current
status: CURRENT
last-verified: 2026-05-31 against HEAD 98f705b — M4.0b-3 (AMD-51) DONE; M4.0b-4 (AMD-52 typed payload) added PLANNED-GATED after the OQ-05-08 design beat
-->
...
| **M4.0b-4** *(proposed id — confirm)* | AMD-52 typed `StateChangedEvent` payload: swap `StateChangedEvent.oldValue/newValue` from `String` to typed `AttributeValue`; materialize typed in `EntityState.attributes` (ends the String-only-materialized regime, AMD-51 §1.2/§2.6-erratum); custom `AttributeValue` Jackson (de)serializer in `core/persistence` (explicit `AttributeType` discriminator, **no `@JsonTypeInfo`** — ArchUnit Rule 7), deterministic float/NaN/±Inf encoding; extend `CheckpointSerializer` to a typed envelope per entry; bump `projectionVersion` **3→4** on AMD-50's frozen backfill (the 3→4 reconstructs typed from the immutable `state_reported` log — Path A). No event-store / `view_checkpoints` row migration (typed payload stays in-BLOB; per-event `events.schema_version` is the string↔typed discriminator). | core/event-model + core/persistence + core/state-store + lifecycle | **PLANNED — GATED** | — | **Design-gated, NOT authored.** Milestone id **M4.0b-4 proposed** under the M4.0b-x / projection-block-50–52 scheme (M4.0b-1/-2/-3 = no-bump slice / AMD-50 1→2 / AMD-51 2→3; AMD-52 is the natural next, AMD-50→51→**52**) — **confirm with Nick before use; do not treat as locked.** Gate = AMD-52 go/no-go (design beat §9: G1 serializer + G2 replay determinism OPEN; G5 row-shape DECIDED). Chain to GO: **OQ-05-08 design beat DONE (`design/2026-05-31_AMD-52_Typed_Payload_Serializer_Replay_Design_Beat.md`) → Research 11 dispatched (`context/instructions/Research_11_Typed_Event_Payload_Persistence_Brief.md`) → PM assessment → Nick adjudicates → author AMD-52 → this WU.** String payload frozen until AMD-52 ratifies (AMD-51 §2.7). |
