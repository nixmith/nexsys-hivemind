...
- **RETURNED + ASSESSED (2026-06-10/11):** Research 13 → **A−** (`context/assessments/2026-06-10_Research_13_PM_Assessment.md`; raw return archived per §2 Step 0 → `homesynapse-core-docs/research/returns/2026-06-11_Research_13_Config_System_Market_Superiority.md`). Quote-back gate HONORED (module-info character-exact); 11 RECs = REC-130..140: 9 ACCEPT (131/135 narrowed by Locked-doc text the researcher had no access to — Doc 06 §3.5 already documents comment-loss + mandates the backup mitigation), **REC-134 RE-BUCKETED → ALREADY-COVERED** (Doc 06 §3.6 `additionalProperties:false`@WARNING, shipped `9035110` — the lone disposition-table defect), REC-138 → FUTURE-AMD with the now-or-never claim softened (versioned-payload evolution exists; §4.4 already sketches `trigger`). REJECT bucket genuinely empty — endorsed. **Folded:** REC-130/131/132/133/135 → the M6.4 instruction; REC-140 → the M6.2 instruction; REC-136 + REC-138 (+ the PM-surfaced `ConfigChangedEvent`-vs-§12.4 reconciliation) → the FUTURE-AMD queue; REC-137/139 → M10/M11/Doc 13 parking. Strategic: the flagship INV-CE-01 split-brain-immunity claim is endorsed, citation-backed, M5-C website-grade.
...
## Addendum — the Research 14/15 cycle (added 2026-06-11; **CYCLE COMPLETE 2026-06-12: all three RETURNED + ASSESSED + MERGED — research high-water now REC-185**)

**Status 2026-06-12 (EOD — same-day full cycle):** W0 delta → briefs authored → Nick dispatched → all three returns assessed SERIALLY (fold order B→A→15), each quote-back HONORED, zero discards: **R14-B → A−** (`context/assessments/2026-06-12_Research_14B_PM_Assessment.md`; 10 M7-obligations, 2 M8, 2 FUTURE-AMD, 1 REJECT; W1–W8 crash-window vocabulary) · **R14-A → A−** (`...Research_14A_PM_Assessment.md`; ledger-gap dossier = M5-C flagship; 2 re-buckets vs Locked text) · **R15 → A−** (`...Research_15_PM_Assessment.md`; in-Project disposition re-run EXECUTED against strategy files 1–5; Apache-2.0 verdict CONFIRMED; REC-182 re-bucketed to M5-C via the R14-A pain-match). **Merged disposition: `context/planning/2026-06-12_M7-blueprint_merged-disposition.md`** (REC-31..40 + 141..185, 7 collisions adjudicated; AMD block = 6 families F1–F6; M7 holds at 3 pieces). Charter POPULATED same file. Raw returns archived → `homesynapse-core-docs/research/returns/2026-06-12_Research_14A/14B/15_*.md`; consumed briefs → `context/instructions/archive/`. **Next: PM drafts the M7 entry-gate AMD block (6 families) → DOCS review → ratification (with C8).** Connector-reachability process note: both R14 runs were DOCS-connector-blind (declared; embeds held — zero fabrications); fix venue or embed contract tables at quote granularity for future cycles.
...
- **Research 14-A — Automation Authoring & Operating UX: Failure Modes.** REC range **141–155**. Community-evidence register (Mom-Test): authoring failure catalog, the "why didn't it fire?" debuggability gap, reliability expectations (ledger differentiator evidence), definition churn, real concurrency footguns. → DOCS Project, web search required. Feeds M7/M8 charter + M5-C superiority material.
- **Research 14-B — Automation Engine Runtime: Robustness Prior Art.** REC range **156–170**. Engineering register: temporal-trigger correctness (DST/skew/catch-up vs AMD-25), cascade containment vs §3.7.1, intent-confirmation/ledger prior art vs §3.11.2, run persistence/REPLAY interaction (re-derive-never-re-execute), event-storm posture vs concurrency modes. → DOCS Project, web search required. Feeds the M7 AMD block + M7.x/M8.x instructions.
- **Research 15 — Ecosystem & Competitive Positioning Scan.** REC range **171–185**. Strategic register: Matter/Thread certification dynamics, platform trajectories, local-first positioning landscape, licensing/governance risk. **Generic deep research permitted for evidence-gathering; the disposition pass never is.** Feeds M5-C + `context/strategy/` ONLY — cannot mint code obligations (FUTURE parking otherwise).
- **W0 (PM-internal, precedes the returns):** Research-4 currency delta — REC-31..40 re-anchored against post-M4/M5/M6 source (REC-39 vs AMD-52 + the 55/24/36 pin discipline; REC-36 `RunContext`; permit arithmetic), DQ-1/2/3/5 escalation refresh, B2 C8 (`actorRef` stamps `AutomationId`) folded as an M7 contract dependency, P1 multi-piece charter sizing.
- **Guardrails:** Doc 07/AMD-25/sealed permits are gap-relative baselines, never open questions; no M6.3 creep; §0 quote-back gate + verbatim automation `module-info` embed at the dispatch sha in every R14 brief; briefs self-contained (no cross-references); serialized PM assessment.

---

## Addendum — the Research 16 cycle (added 2026-06-12; **ASSESSED 2026-06-12** — both returns folded, zero discards: 16-B grade **A**, 16-A grade **A−**; merged disposition `context/planning/2026-06-12_R16_output-contract_merged-disposition.md`; high-water now **REC-215**; the gate-free `scripts/dev/OUTPUT_CONVENTIONS.md` DRAFTED in core same session; assessments in `context/assessments/2026-06-12_Research_16{A,B}_PM_Assessment.md`)

**Object:** ONE output-contract philosophy serving three consumers — end-users, developers/operators, LLM agents — across Core logging AND the Pi script tooling (the `pi-health.sh` `READINESS:`/`[OK]`-idiom precedent, shipped `44bce4d`/`01841ba`). Strategic hook: B3 explainability is a named differentiator — logging UX is brand surface, not plumbing; the R14-A ledger-gap dossier + REC-145 no-ring-buffer attestation are its existing evidence base. First full exercise of the upgraded Project `research_mode` charters (the briefs cite the charter disciplines instead of restating them; quote-back anchors embedded per protocol). **Dispatch = Nick veto-or-default → DOCS Project, two SEPARATE fresh conversations; web search required in both.** Returns → `context/instructions/` → archive to docs `research/returns/` per the standing flow; serialized PM assessment (fold order B→A), then the merged disposition routes to: (a) the Script Output Standard — a new `scripts/dev/OUTPUT_CONVENTIONS.md`, gate-free, pi-health retrofitted as the reference implementation; (b) Doc 11 currency notes + the M12 observability evidence base; (c) M10/M13 UI inputs (parked, not drafted); (d) M5-C copy; (e) anti-requirements registered. Nothing in this cycle amends frozen contracts (the AMD-92 event vocabulary and Locked Doc 11 are inviolate baselines).

- **Research 16-A — Logging & Observability UX: Market and Human Factors.** REC range **186–200**. UX/market register: smart-home logging/debug pain catalog (installed workarounds > complaints > requests), best-in-class output-UX teardowns (Rust/Elm error grammar, the Tailscale/`gh`/`cargo` CLI class, severity/color + no-color-only accessibility, timestamp discipline, noise budgets and log-level abuse), the end-user plain-language-causality register (the B3 surface), the per-audience needs matrix (the cycle's integrating deliverable), anti-requirements (what demonstrably failed). Fence: no code obligations, no event-vocabulary changes, no Doc 11 re-litigation. Brief: `context/instructions/Research_16A_Logging_Observability_UX_Market_Human_Factors_Brief.md`.
- **Research 16-B — LLM-Legible Output Engineering: Structured Logging and Agent-Parseable Tooling.** REC range **201–215**. Engineering register: structured-logging prior art with exact citations (OTel log/trace semantic conventions, JSON Lines vs logfmt, systemd journal fields, SLF4J/MDC incl. virtual-thread propagation — under the no-collector constraint, cost weighed honestly), the agent-consumer contract (stable line-anchored keys, machine-parseable summary lines per the `READINESS:` precedent, dual-channel pretty-TTY vs `--json`/`--kv`, bounded output + tail-with-summary, deterministic ordering, no ANSI in non-TTY, exit-code semantics), correlation propagation (`CausalContext.correlationId` → log lines → trace assembly: one causal story, logging as its narration), a project-wide log-event naming-convention proposal (the Doc 07 §11.2 dotted-name precedent generalized), and the codified script-layer standard (the `OUTPUT_CONVENTIONS.md` rule-set, evidence-cited). Fence: proposals land as CONVENTION/STANDARD candidates + Doc 11/M12 inputs, never amendments; the scripts layer is gate-free. Brief: `context/instructions/Research_16B_LLM_Legible_Output_Engineering_Brief.md`.
- **Cycle-specific disposition buckets (both briefs):** SCRIPT-STANDARD / M12-INPUT / M10-M13-INPUT / DOC-11-CURRENCY / M5-C-COPY / FUTURE / REJECT. **Quote-back set (both):** the pi-health idiom block (severity helpers + `KEY:value` lines + `READINESS:` summary) + the Doc 07 §11.2 event-name list (all 14) + the DAS Register-C paragraph; 16-B adds the `CausalContext` signature + the `com.homesynapse.event` module-info verbatim. REC high-water at authoring = **185**; ranges partitioned, briefs self-contained.
