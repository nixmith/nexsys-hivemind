...
## 12. Remote-Cowork sessions (the device bridge) — verified 2026-07-08, the v25 arc

When the hub runs in REMOTE Cowork (cloud container + desktop bridge), §1's path duality changes shape: the container's file tools see only STAGED SNAPSHOTS of the mounted folder; the host is reached through bridge tools. Paid-for rules:
- **Reads:** `device_stage_files` = a host-side point-in-time snapshot. **A re-stage at the SAME device path serves the container a STALE cache** (the tool RESULT's `bytes`/`mtimeMs` reflect new host truth; the container path's content does not refresh). To re-read a changed file: copy it to a FRESH temp name on-device first (`git show HEAD:<path> > _scratch/x.HEAD.tmp.md` — git-object-sourced per §5(b)) and stage THAT.
- **Writes:** edit a hash-verified copy in the container → `SendUserFile` → `device_commit_files` (a true host-side write). Verify by size via `device_list_dir` (host) — a VM-mount `md5sum` immediately after is read-lag phantom-prone (§2 applies unchanged to `device_bash`).
