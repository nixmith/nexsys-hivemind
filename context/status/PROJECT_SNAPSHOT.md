...
**Open:** the word above · RATIFY (v1.1) · RATIFY-PLAN (September) · the Annex paste · O-2 (held card next boot, ~5 min). **Monday:** the NFCU call · THE PELTON WORD (OUTRANKS; reply = the conform addendum). **PKG-SEC-1 UN-HELD** (ships as written; C-1 = its own row). Docket adds: F-R4-1 (coding WU) · F-R4-2 (doctrine/docs) · C-1 (packaging).

**Fences:** the D-1 pair STAYS (narrow-mint pends the word) · `README.md:117` until W2-3 · no public brand use before G-2 · s31/nightly HANDS OFF until R-5 · the hub never implements · ONE COORDINATOR, ONE BOOT · no public sentence outside the register · orders never edited/grown · the §VIII(4) consent at the LICENSE flip · held card relabeled per packet §7 · bench floor `[PASS]` (pid 2347; next floor Tue 03:00 CT).

**Next:** Nick's word (MINT-NARROW | HOLD) → on MINT-NARROW the hub writes the register row. **PELTON → the word → the card.**
