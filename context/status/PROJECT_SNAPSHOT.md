...
**Last updated:** 2026-05-22 by PM (WUCP Phase 2 — M3.6e.2 closure)
...
M3.6 COMPLETE — all seven sub-WUs shipped (M3.6a through M3.6e.2). WUCP Phase 2 closeout COMPLETE for all M3.6 sub-WUs. Seventeen Claude Code work units total. Next: M3.7 scoping via research pipeline.
...
| M3.7 | NEXT | — | End-to-end integration tests |
...
- **Production Java files:** ~721+ across the JPMS-compiled production modules (post-M3.6e.2 addition of `MaterializedStateQueryService`, `ReadinessFilter`, `RestFilters` in state-store/rest-api, plus `DeploymentProfile` thread-pool fields, `HomeSynapseCore` Javalin bootstrap wiring, `StateQueryService.materialized()` static factory). Test files: **~1,575+ @Test methods** across ~158+ test files and ~35+ testFixtures files (M3.6e.1 added: `MaterializedStateQueryServiceTest` (10 tests), `ReadinessFilterTest` (8 tests), `DeploymentProfileTest` (4 tests), `HomeSynapseCoreTest` stateQueryService test, `StateQueryServiceTest` updated assertion).
...
- **Last build gate:** RESOLVED. Full `./gradlew check` GREEN at HEAD `76288af` on 2026-05-22 (post-M3.6e.2). Build confirmed by Nick. No deferred build gates open.
...
- **OR-M3-17 — NO_OP_DERIVATION placeholder (NEW 2026-05-22):** `HomeSynapseCore` step 5 uses `NO_OP_DERIVATION` (`Function.identity()`) as the derivation function in `StateProjection.create(...)`. Placeholder — real derivation logic lands when `state_changed` event derivation is wired per DEC-M3-10. Must be resolved before M3.7.
- **OR-M3-18 — NO_OP_ADVANCER placeholder (NEW 2026-05-22):** `HomeSynapseCore` step 5 uses `NO_OP_ADVANCER` (returns `AdvanceResult.empty()`) as the `ProjectionAdvancer`. Placeholder — real advancer lands when checkpoint advancement is wired. Must be resolved before M3.7.
...
1. **M3.7** — end-to-end integration tests. Estimated 6–8h Coder time. M3.7 scoping via research pipeline in progress.
