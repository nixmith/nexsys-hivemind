...
state-type: current
status: CURRENT
last-verified: 2026-06-12 (post-ratification) against core `e5ea76f` (substantive `7c73c91` = M6.2) — **AMD-88..93 + B2 C8/C9 RATIFIED 2026-06-12 (bundled DOCS review RATIFY-WITH-EDITS/AS-IS, all edits folded; return on disk). On-disk watermark NOW AMD-93; invariants 163/47 (§42–§47 registered); AMD-04 SUPERSEDED by AMD-91. M7 entry-gate rows 1–2 CLOSED — rows 3 (M6.3-vs-M7 ordering) + 4 (M5-C Increment 1) still gate M7.1. M6 3-of-4 COMPLETE (M6.3 triple-gated = the M6 critical path). projectionVersion 5; event pins 55/24/36 (M7 slices raise to 71/41/53 at implementation); build GREEN 147.** _Prior:_ 2026-06-07 against `7f44bed` — **M5-A COMPLETE** (platform-systemd `PlatformPaths`/`HealthReporter` impls + the **AMD-87 `Expectation` persisted codec** + `FloorId` registration; **M9 command-bearing-`CapabilityAdded` prerequisite CLEARED**) + **M5-B/B1 DONE** (**Doc 15 Cryptographic Architecture LOCKED**; AMD-86 + AMD-87 RATIFIED; watermark **AMD-87**; 135 invariants; build GREEN 147 tasks; `projectionVersion` 5). Prior spine state: `8ef9e9f` (M4 COMPLETE, watermark AMD-64).
-->
...
- **M6.3 — ⛔ GATED, 1-of-3 CLOSED (the last M6 piece):** ✅ **OQ-15-2 RESOLVED 2026-06-12** (microbench run in the guided Pi-evidence session — `encrypted_scopes` default CONFIRMED `[identity, presence_personal]`, no fallback; disposition `context/assessments/2026-06-12_OQ-15-2_disposition_encrypted-scopes-CONFIRMED.md`; Pi-5 NON-AUTHORITATIVE caveat accepted, optional Pi-4 confirmation in the advisory queue) + ⛔ the AMD-86 §3 energy/erasure interview signal (Nick-paced, the open gate) + OR-M6-NONCE (rides the M6.3 instruction authoring). The interview half also feeds the **W25/W26 M6.3-vs-M7 ordering call** (Nick's, at the boundary). The M6.3 instruction carries the resolved set verbatim + the cold-start cipher-warmup finding.

**M7 (Automation) — entry-gate 3-of-5 CLOSED (M7.1 issues only when ALL green):** (1) ✅ **AMD-88..93 RATIFIED 2026-06-12** (bundled DOCS review; edits folded; watermark 87→93; invariants §42–§47); (2) ✅ **B2 C8 + C9 RATIFIED 2026-06-12** (same bundle; automations stamp `AutomationId`; coupling verified); (3) ⛔ the M6.3-vs-M7 ordering call (Nick, W25/W26 boundary — the microbench half ✅ DELIVERED 2026-06-12 [OQ-15-2 RESOLVED]; the interview half still needed); (4) ⛔ **M5-C Increment 1** (the W25 Lane-4 structural gate — see M5-C below; NOW THE PRINCIPAL NON-EVIDENCE BLOCKER); (5) ✅ automation MODULE_CONTEXT currency (30-permit correction, `e5ea76f`). M7 = 3 first-class pieces (M7.1 trigger/condition · M7.2 run/action/dispatch · M7.3 Pending Command Ledger), F5 event slices distributed per piece.
