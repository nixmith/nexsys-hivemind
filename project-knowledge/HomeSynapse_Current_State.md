...
state-type: current
status: CURRENT
last-verified: 2026-06-05 against `8ef9e9f` — **M4 COMPLETE** (Workstreams A+B+C done; integration-api freeze AMD-54..64; watermark AMD-64; `projectionVersion` 5; build GREEN 145 tasks). Prior spine state: `98f705b` (M4.0b-3, watermark AMD-51).
-->

# HomeSynapse Core — Current State

Last updated: 2026-06-05 — **M4 COMPLETE** (`8ef9e9f`, watermark AMD-64, `projectionVersion` 5; build GREEN, 145 tasks). Workstream A (projection/derivation, typed end-to-end + event-time-deterministic) + B (device-model expansion incl. AMD-44 Floor/EntityRole) + C (integration-api interface freeze, AMD-54..64) all shipped. **AMD-65** (Expectation persisted codec) QUEUED — BLOCKING-for-M9. Next piece (recommended, awaiting Nick): **M5** Platform API + test-support paired with the **W3 website/docs** standup; **M6** Configuration follows. See `nexsys-hivemind/context/audits/2026-06-05_M4-retrospective.md` + `context/planning/2026-06-05_next-piece-recommendation.md`.
...
**Debt from M4.C:** **AMD-65** (the `Expectation` persisted sealed-type codec) is **QUEUED — BLOCKING-for-M9**: command-bearing `CapabilityAdded` does not round-trip yet (`WithinTolerance(double,double)` needs the AMD-52 bit-anchored-float treatment). A `@Disabled("AMD-65 pending")` onOff round-trip test is its executable acceptance spec; **M9 must not publish command-bearing `CapabilityAdded` until it lands.**
...
M4 is closed. **Recommended next piece (awaiting Nick's posture confirmation): M5 (Platform API + test-support) as a small Core lane, paired with a non-preemptable W3 website/docs standup; M6 (Configuration) follows M5; AMD-65 authored in the M5 window.** M5-first because it's dependency-free and its test-support module (`TestClock`/`InMemoryEventStore`/`SynchronousEventBus`/`NoRealIoExtension`) force-multiplies M6/M7/M9 testing. Strategic note: Core is healthy and ahead of its own axis — the launch risk is the three non-Core tracks (website/docs, Web UI, distribution) still at zero. M6 prerequisite: author/ratify AMD-66–71 (Research 5, A−); AMD-67's REC-41 blocker is now cleared by the M4.C config-schema-versioning freeze (AMD-54).
...
**Watermark = AMD-64.** M3: AMD-41/42/43 APPLIED. **M4 amendments:** **AMD-44** (Floor/EntityRole — IMPLEMENTED via B-S1/B-S2), **AMD-45** (atomic checkpoint — M4.0a), **AMD-47** (AttributeValue expansion — M4.B3), **AMD-50** (version-transition backfill — M4.0b-2), **AMD-51** (typed comparator — M4.0b-3; §2.6 erratum applied), **AMD-52** (typed `StateChangedEvent` payload + §11 relocation erratum — M4.0b-4a/4b), **AMD-53** (timestamp unifier — M4.0b-5), **AMD-54..64** (integration-api freeze block, 11 amendments — M4.C, all RATIFIED 2026-06-05 via one DOCS-Project block review that caught AMD-56's unimplementable route-(a) trigger + AMD-55's undetectable void-reauth). **AMD-65** QUEUED (Expectation codec, BLOCKING-for-M9). **Invariants:** AMD-54..64 registered **29 invariants** at `Architecture_Invariants_v1.md` **§24–§34** (index now 133 across 32 categories); AMD-51/52/53 at §21/§22/§23; AMD-47 at §20. **P2 RATIFIED** (AMD renumbering: device 46–49, projection 50–52 fixed — gaps unused at 46/48/49; integration assign-at-milestone). DEC-M3-14..17 + D-09..12 locked.

**Build:** GREEN at `8ef9e9f` (145 tasks). **New module since the last spine update: `core/value-model`** (`com.homesynapse.value`, leaf — holds the relocated `AttributeValue` hierarchy; 21 Gradle modules now). File/test counts last source-counted 2026-05-28 (**1,422** @Test / **724** Java files) + the M4 additions (value-model + the A/B/C surface) — not re-counted; a fresh source count is a low-priority follow-up logged in the backlog.
...
**M3 is COMPLETE** (eighteen Claude Code WUs, 2026-05-16–05-27). **M4 is COMPLETE** (2026-06-05, `8ef9e9f`) — Workstreams A (M4.0a–M4.0b-5), B (M4.B3/B-S1/B-S2), and C (M4.C) all shipped; watermark AMD-64; `projectionVersion` 5. Next: M5/M6 + the W3 website/docs standup (see the next-piece recommendation).
...
**CURRENT:** **M4 COMPLETE** (2026-06-05, `8ef9e9f`; M4.C / AMD-54..64 integration-api freeze, PM WUCP Phase 2 → APPROVE). Hivemind status: PASS (reconciled 2026-06-05 after a partial-M4.C-closeout STALE — see the M4 retrospective §5). Watermark AMD-64; `projectionVersion` 5. Next: M5/M6 + the W3 website/docs lane (next-piece recommendation, awaiting Nick's posture).
...
**M3 COMPLETE. M4 COMPLETE (2026-06-05, `8ef9e9f`)** — Workstreams A (M4.0a–M4.0b-5), B (M4.B3/B-S1/B-S2), and C (M4.C / AMD-54..64 integration-api freeze) all shipped; watermark AMD-64; `projectionVersion` 5. **Next forward = M5 (Platform API + test-support) + the W3 website/docs standup; then M6 (Configuration). AMD-65 (Expectation codec) QUEUED BLOCKING-for-M9.** See §1 + the M4 retrospective/next-piece recommendation in `nexsys-hivemind/context/`.
