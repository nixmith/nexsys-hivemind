...
state-type: current
status: CURRENT
last-verified: 2026-05-30 against M4.0b-3 commit `98f705b` (AMD-51 typed comparator COMPLETE + COMMITTED, build GREEN all gates; independently reviewed → APPROVE; `projectionVersion` 3; watermark AMD-51; Workstream A COMPLETE). Prior: `60b4185` M4.B3 (AMD-47), `7610296` M4.0b-2.
-->

# HomeSynapse Core — Current State

Last updated: 2026-05-30 (**M4.0b-3 COMMITTED `98f705b`** — AMD-51 typed `AttributeValue` change-detection comparator: external `AttributeValueComparator` (pkg-private `StructuralAttributeValueComparator`, exhaustive no-`default` 8-arm switch, total-form `1e-9` float/quantity epsilon + full IEEE-754 totality, order-sensitive array deep-compare, HA-mirrored Degraded) + `ComparisonPolicy` + `AttributeSchemaResolver` + `AttributeValueReconstructor` (symmetric schema-driven both-sides parse) in state-store; `StandardCapabilities` production catalogue in device-model **main** (DP-K — immutable injected snapshot; `TestCapabilityFactory` delegates). `ProductionDerivationRule` rewritten typed (reconstruct-both-sides → `comparator.changed`); **String `StateChangedEvent` payload preserved** (typed payload = AMD-52, staged behind OQ-05-08). `projectionVersion` **2→3** riding AMD-50's backfill unchanged. Build GREEN all gates; **independently reviewed against source → APPROVE**; WUCP Phase 2 closed. **On-disk amendment watermark raised AMD-50 → AMD-51.** AMD-51 §2.6 erratum applied (missing-schema → `StringValue` string-compare fallback — D-1 ACCEPT). **Workstream A COMPLETE.** **Next forward = the M4 next-part scope/plan conversation** (Workstream B device-model breadth + Workstream C integration-api freeze + the AMD-52 typed-payload design beat, OQ-05-08). Earlier: M4.B3 COMMITTED `60b4185` (AMD-47); AMD-51 RATIFIED; Research 4 FULLY RESOLVED (v4, unblocks M7/M8 automation); M4.0b-2 `7610296`.)

---

## 1. Current Milestone Status

**M4 — Foundation (projection/derivation + device-model expansion + integration-api freeze) — IN PROGRESS.** **Workstream A (projection/derivation) is COMPLETE**: **M4.0a** (atomic checkpoint coupling, AMD-45, `a441fdf`), **M4.0b-1** (production `ProductionDerivationRule` + `DispatchingProjectionAdvancer` REC-28, `cf1a97e`), **M4.0b-2** (AMD-50 backfill + `projectionVersion` 1→2, `7610296`), **M4.0b-3** (AMD-51 typed change-detection comparator + `projectionVersion` 2→3, `98f705b`, 2026-05-30 — independently reviewed → APPROVE). Typed state-change detection is LIVE. **Workstream B's typed-value contract is DONE**: **M4.B3** (device-model `AttributeValue` expansion, AMD-47, `60b4185`) shipped the 8-variant hierarchy + `AttributeValueUpcaster` SPI. **Next forward = the M4 next-part scope/plan** (Workstream B device-model breadth — AMD-44 Floor/EntityRole, Research 8 REC-23–30; Workstream C integration-api freeze — Research 6, **P3-gated**; and the AMD-52 typed-`StateChangedEvent` design beat, OQ-05-08). Workstream C gated on P3.
...
**M4.0b-3 (Typed `AttributeValue` Change-Detection Comparator, AMD-51) — COMPLETE** (committed 2026-05-30 `98f705b`, build GREEN all gates — `:core:device-model:check` + `:core:state-store:check` + `:lifecycle:lifecycle:check` + full `./gradlew check` 139 tasks). Replaces the string `Objects.equals` change-detect with a typed comparison. New in state-store: `AttributeValueComparator`(+`structural()`) / pkg-private `StructuralAttributeValueComparator` (exhaustive **no-`default`** 8-arm switch; total-form `|a−b| > max(absEps, relEps·max(|a|,|b|))` with `1e-9` defaults; full IEEE-754 totality incl. `Inf−Inf` handled before the arithmetic; order-sensitive array deep-compare; HA-mirrored Degraded), `ComparisonPolicy` (`FP_NOISE_DEFAULT 1e-9`), `AttributeSchemaResolver` (immutable `Map::get` snapshot), `AttributeValueReconstructor` (schema-driven **both-sides** parse — the prior `StringValue` is reconstructed too, AMD-51-INV-05). New in device-model **main**: `StandardCapabilities` (DP-K — `all()`/`attributeSchemas()`, lifted from `TestCapabilityFactory` which now delegates; the future `CapabilityRegistry` seed). `ProductionDerivationRule` rewritten typed; **String `StateChangedEvent` payload preserved** (DP-G; typed payload = AMD-52, staged). `DerivationRule.production(comparator,policy,schemas)` overload added; no-arg `production()` kept = string-fallback gateway. `HomeSynapseCore` wires it + bumps `projectionVersion` **2→3**. **No** `module-info`/Gradle/`CheckpointSerializer`/event-store change. **Independent Cowork review read every changed file vs AMD-51 (not the report) → APPROVE.** Deviations D-1/D-2/D-3 all `[REVIEW]` ACCEPT; **D-1 (missing-schema → `StringValue` string-compare fallback, not §2.6 Degraded) → AMD-51 §2.6 erratum applied** (2026-05-31). AMD-51-INV-01..05 upheld. **On-disk amendment watermark raised AMD-50 → AMD-51.** Twenty-first Claude Code WU. **Workstream A COMPLETE.**

**M4.B3 (device-model `AttributeValue` expansion, AMD-47) — COMPLETE** (committed 2026-05-30 `60b4185`, build GREEN both gates). Three records `QuantityValue(double value, String unit)` / `ArrayValue(List<AttributeValue>)` / `DegradedAttributeValue(originalTypeName, rawForm, failureReason)` + the `AttributeValueUpcaster` SPI (no `ServiceLoader`) + 3 `AttributeType` constants `QUANTITY`/`ARRAY`/`DEGRADED` + `AttributeValue` `permits` **5→8** + `AttributeSchema` DEGRADED-rejection guard (AMD-47-INV-04). `QuantityValue` canonicalises at construction (hand-rolled table, fail-closed; AMD-47-INV-03). **AMD-47 is now IMPLEMENTED in source** (the live `permits` clause enumerates all 8 variants). PM Phase 2 confirmed AMD-47 §2.1–§2.6 + INV-01..05 vs source. The DP-1 upcaster projection-path carry landed in M4.0b-3 (reconstruction inside `rule.evaluate`, both paths). Twentieth-equivalent CC WU.
...
- **A — Projection/derivation foundation — COMPLETE:** M4.0a (`AtomicCheckpointWriter`/AMD-45, `a441fdf`) ✓, M4.0b-1 (`DispatchingProjectionAdvancer` REC-28 + real `ProductionDerivationRule`, `cf1a97e`) ✓, M4.0b-2 (AMD-50 one-shot backfill on the `projectionVersion` 1→2 reconciliation replay, `7610296`) ✓, **M4.0b-3 (AMD-51 typed change-detection comparator + `projectionVersion` 2→3, `98f705b`, independently reviewed → APPROVE) ✓**. Typed state-change detection is LIVE.
- **B — Device-model expansion — typed-value contract DONE; breadth remaining:** **M4.B3** (`AttributeValue` expansion, **AMD-47 now IMPLEMENTED**, `60b4185`) shipped the 8-variant hierarchy + `AttributeValueUpcaster` SPI. Remaining B breadth: Research 8 REC-23–REC-30 + ratified-but-unbuilt **AMD-44** (Floor/EntityRole). (Doc 05's integration-api currency is a *separate* P3-gated half and gates Workstream C.)
...
**Next concrete action:** **the M4 next-part scope/plan conversation** (a fresh Cowork session) — formally scope the next M4 segment: Workstream B device-model breadth (AMD-44 Floor/EntityRole, Research 8 REC-23–30), Workstream C integration-api freeze (Research 6 REC-41–51, **P3-gated on Nick's NQ-1..6**), and the **AMD-52** typed-`StateChangedEvent` serializer/replay design beat (OQ-05-08 — run the beat + dispatch a deep-research brief on event-sourced payload evolution: Axon upcasters + peers). Do NOT author AMD-52 until OQ-05-08 closes. P3 (Research 6 NQ-1..6) can be resolved in parallel.

**Build:** GREEN at `98f705b` (M4.0b-3 — `./gradlew check` 139 tasks + `:core:device-model:check` + `:core:state-store:check` + `:lifecycle:lifecycle:check`; the `-PpiProfile=throttled` ITs are unaffected — non-catalog keys → string-fallback — and the 30-min `Pi4D1SpikeIT` soak is a long-running test, not a gate failure). Test/file counts last source-counted 2026-05-28 (**1,422** @Test methods / **724** Java files, 20 modules); M4.0a/M4.0b-1/M4.0b-2/M4.B3/M4.0b-3 added tests + types and modified files (not re-counted — no new modules).

**Active governance:** AMD-41/42/43 APPLIED (2026-05-16). **AMD-44 RATIFIED** (Floor/EntityRole — impl still pending in the M4.B breadth path). **AMD-45 RATIFIED + applied** (M4.0a, `a441fdf`). **AMD-50 RATIFIED + applied** (M4.0b-2, `7610296`). **AMD-47 RATIFIED + IMPLEMENTED** (M4.B3, `60b4185`) — device-model `AttributeValue` expansion: public records `QuantityValue`/`ArrayValue`/`DegradedAttributeValue`, the `AttributeValueUpcaster` SPI (no `ServiceLoader`), and `AttributeType` `QUANTITY`/`ARRAY`/`DEGRADED`; the 8-variant `permits` clause is **live in source**; AMD-47-INV-01..05 in `Architecture_Invariants_v1.md` §20; REC-93 hand-rolled `String` units (no JSR-385). **AMD-51 RATIFIED + IMPLEMENTED** (M4.0b-3, `98f705b`) — typed `AttributeValue` change-detection comparator (`AttributeValueComparator`/`ComparisonPolicy`/`AttributeSchemaResolver`/`AttributeValueReconstructor` in state-store; `StandardCapabilities` in device-model main), String `StateChangedEvent` payload preserved, `projectionVersion` 2→3; AMD-51-INV-01..05 in §21; **§2.6 erratum applied** (missing-schema → `StringValue` string-compare fallback, D-1). **On-disk amendment watermark = AMD-51** (raised AMD-50 → AMD-51 at AMD-51 ratification). **P2 RATIFIED** (AMD renumbering). DEC-M3-14 through DEC-M3-17 + D-09 through D-12 locked. **AMD-52** (typed `StateChangedEvent` payload) is the next projection amendment — **staged behind the OQ-05-08 serializer/replay design beat; not yet authored.**
...
**M3 is COMPLETE.** All seven sub-milestones shipped across nineteen Claude Code work units (2026-05-16 through 2026-05-27). **M4 Workstream A is now COMPLETE** (M4.0a/M4.0b-1/M4.0b-2/M4.0b-3, through 2026-05-30 `98f705b`); M4.B3 (AMD-47, `60b4185`) shipped Workstream B's typed-value contract. Next: the M4 next-part scope/plan (B breadth / C freeze / AMD-52 design beat).
...
**CURRENT:** M4.0b-3 closeout done (2026-05-30, `98f705b`; independently reviewed → APPROVE). Hivemind status: PASS (post-commit). **M4 Workstream A COMPLETE** (M4.0a/M4.0b-1/M4.0b-2/M4.0b-3); M4.B3 (AMD-47) shipped B's typed-value contract. All three repos committed (core `98f705b`, core-docs, hivemind). Next: the M4 next-part scope/plan conversation.
...
**M3 COMPLETE. M4 Workstream A COMPLETE (M4.0a/M4.0b-1/M4.0b-2/M4.0b-3, `98f705b`). M4.B3 (AMD-47, `60b4185`) shipped Workstream B's typed-value contract. Next forward = the M4 next-part scope/plan conversation (B breadth + C freeze + the AMD-52 design beat behind OQ-05-08).**
