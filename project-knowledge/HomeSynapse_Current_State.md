...
**Next: M6 (Configuration + secrets/crypto)** — the next Core critical-path milestone; entry-gate half-closed (crypto side via M5-B/B1). The config side (author + ratify **AMD-66–71** + the Doc 06 `SecretStore.setAll(Map)` currency amendment) + resolving **OQ-15-2** close it. M6 is **large** (config pipeline + secret store + per-scope key-manager + at-rest encryption + hot-reload) → chartered as multiple first-class pieces (P1), carrying the Doc 15 §3.8 `com.homesynapse.app` composition-root bridge + OR-M6-NONCE.
...
M4 is closed; the **M5 window is chartered + executing** (decisions D1–D8, 2026-06-06 — a blended, lane-tracked window). **M5-A COMPLETE** (`7f44bed`); **M5-B/B1** (the crypto entry-gate) **DONE** (Doc 15 LOCKED, AMD-86/87 RATIFIED); **M5-D** evidence authored + independently reviewed. **Next Core critical-path = M6 (Configuration + secrets/crypto).** M6 prerequisites: author/ratify **AMD-66–71** (Research 5, A−; AMD-67's REC-41 blocker now cleared by the M4.C config-schema-versioning freeze, AMD-54) + the Doc 06 `SecretStore.setAll(Map)` currency amendment; resolve **OQ-15-2** (the MVP encrypted-scope set, via the M5-D Pi-4 microbench) — then charter M6 as multiple first-class pieces (P1). Strategic note: Core is healthy and ahead of its own axis; the launch risk remains the non-Core tracks (website/docs, Web UI, distribution), now being addressed via the M5-C non-preemptable floor (P6/D3).
...
**CURRENT:** **M5 window IN PROGRESS — M5-A COMPLETE** (`7f44bed`; AMD-87 codec, PM WUCP Phase 2 → APPROVE, M9 prereq cleared) + **M5-B/B1 DONE** (Doc 15 Cryptographic Architecture LOCKED, AMD-86/87 RATIFIED). Watermark **AMD-87**; `projectionVersion` 5; build GREEN, 147 tasks. **Next = M6 (Configuration + secrets/crypto)** — entry-gate half-closed (crypto side done); AMD-66–71 + the Doc 06 `SecretStore.setAll(Map)` currency amendment + OQ-15-2 close it. W24 plan: `context/planning/weeks/2026-W24_jun08-jun14.md`.
...
**M3 COMPLETE. M4 COMPLETE (`8ef9e9f`). M5-A COMPLETE (`7f44bed`); M5-B/B1 DONE.** M5-A: platform-systemd `PlatformPaths`/`HealthReporter` impls + the **AMD-87 `Expectation` codec** (M9 command-bearing-`CapabilityAdded` prereq CLEARED) + `FloorId` registration. M5-B/B1: **Doc 15 Cryptographic Architecture LOCKED** + AMD-86/87 RATIFIED (watermark **AMD-87**). M5-D evidence authored + independently reviewed. **Next forward = M6 (Configuration + secrets/crypto)** — author/ratify AMD-66–71 + the Doc 06 `SecretStore.setAll(Map)` currency amendment, resolve OQ-15-2, then charter M6 multi-piece (P1). See §1 + the W24 plan in `nexsys-hivemind/context/`.
