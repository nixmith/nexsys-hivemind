...
- **M5-C — website/docs floor** (non-preemptable, P6/D3) — not yet started.

**Next: M6 (Configuration + secrets/crypto) — entry-gate CLOSED 2026-06-09.** The config side is ratified (**AMD-66/67/68/70/71 RATIFIED**, incl. AMD-68 = the Doc 06 `SecretStore.setAll(Map)` currency amendment; **AMD-69 DEFERRED** Tier-2/OQ-15-3); the crypto side was done via M5-B/B1. **M6.1 (config pipeline) is READY TO ISSUE.** M6 is **large** → chartered as four first-class pieces (P1: M6.1 pipeline / M6.2 secret store + key-mgmt / M6.3 at-rest write-path — still triple-gated on OQ-15-2 + the AMD-86 §3 interview signal + OR-M6-NONCE / M6.4 hot-reload swap), carrying the Doc 15 §3.8 `com.homesynapse.app` composition-root bridge + OR-M6-NONCE.
...
**M3 COMPLETE. M4 COMPLETE (`8ef9e9f`). M5-A COMPLETE (`7f44bed`); M5-B/B1 DONE; M6 entry-gate CLOSED (2026-06-09).** M5-A: platform-systemd `PlatformPaths`/`HealthReporter` impls + the **AMD-87 `Expectation` codec** (M9 command-bearing-`CapabilityAdded` prereq CLEARED) + `FloorId` registration. M5-B/B1: **Doc 15 Cryptographic Architecture LOCKED** + AMD-86/87 RATIFIED (watermark **AMD-87**). Config side: **AMD-66/67/68/70/71 RATIFIED 2026-06-09 + AMD-69 DEFERRED** (Tier-2/OQ-15-3); invariants §37–§41; watermark unchanged. **Next forward = issue M6.1 (config pipeline — READY TO ISSUE)**; M6.2 after M6.1 (E2 bridge + AMD-68 `setAll`); M6.3 triple-gated (OQ-15-2 + interviews + OR-M6-NONCE); M6.4 after M6.1. See §1 + the W24 plan in `nexsys-hivemind/context/`.
