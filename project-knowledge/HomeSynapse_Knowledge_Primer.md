...
status: CURRENT
last-verified: 2026-05-30 against M4.0b-3 commit `98f705b` (AMD-51 typed comparator COMPLETE + COMMITTED, build GREEN; `projectionVersion` 3; watermark AMD-51; Workstream A COMPLETE). Prior: `60b4185` M4.B3 (AMD-47), `7610296` M4.0b-2.
-->
...
**M4 — Foundation, Workstream A (projection/derivation) substantially COMPLETE (2026-05-29).** **M4.0a** (`a441fdf`): AMD-45 atomic subscriber+view checkpoint coupling via the `AtomicCheckpointSink` seam; reconciliation metadata populated (OR-M3-13 resolved). **M4.0b-1** (`cf1a97e`): the no-op derivation placeholders retired — shipped the production **`ProductionDerivationRule`** (package-private in `com.homesynapse.state`, string change-detect, publishes `state_changed` on LIVE → populates `attributes`) + REC-28 **`DispatchingProjectionAdvancer`** (package-private in `com.homesynapse.state`, constructor-injected `EnvelopeHandler` map, no `ServiceLoader`), both via DEC-M3-16 gateways; **`MinimalProjectionAdvancer` DELETED**; OR-M3-17/18 fully closed; `projectionVersion` stays 1. **M4.0b-2** (`7610296`): ratified AMD-50 version-transition backfill for the 1→2 transition — `projectionVersion` 1→2 trigger; `backfillActive` provenance gate; non-emitting one-shot backfill on both `onEvent`/`processBatch` via the narrow `applyBackfillAttribute`; supersession in `applyToState` (§2.2); `Clock` removed from `DerivationContext` (§2.4). AMD-44/45/50 RATIFIED; P2 RATIFIED. **AMD-47 RATIFIED + IMPLEMENTED** (M4.B3, `60b4185`, 2026-05-30) — device-model `AttributeValue` expansion: public records `QuantityValue`/`ArrayValue`/`DegradedAttributeValue` + `AttributeValueUpcaster` SPI (no `ServiceLoader`) + `AttributeType` `QUANTITY`/`ARRAY`/`DEGRADED`; the 8-variant `permits` clause is **live in source**; AMD-47-INV-01..05 in `Architecture_Invariants_v1.md` §20; REC-93 hand-rolled `String` units (no JSR-385). **AMD-51 RATIFIED + IMPLEMENTED** (M4.0b-3, `98f705b`, 2026-05-30) — typed `AttributeValue` change-detection comparator: `AttributeValueComparator`/`ComparisonPolicy`/`AttributeSchemaResolver`/`AttributeValueReconstructor` in state-store + `StandardCapabilities` in device-model main; **String `StateChangedEvent` payload preserved**; `projectionVersion` **2→3**; AMD-51-INV-01..05 in §21; §2.6 erratum applied (missing-schema → `StringValue` string-compare fallback, D-1). **On-disk amendment watermark = AMD-51.** **Workstream A COMPLETE.** Next: **the M4 next-part scope/plan** — Workstream B device-model breadth (AMD-44 Floor/EntityRole), Workstream C integration-api freeze (P3-gated), and the **AMD-52** typed-`StateChangedEvent` design beat (staged behind OQ-05-08 — do NOT author until that beat closes).
...
Physical and logical device model. One flat package (verified 2026-05-22). Entity (not Device) is the atomic addressable unit — a single physical device may produce multiple entities. Sealed Capability hierarchy, sealed AttributeValue hierarchy. Service interfaces for registries, validation, and discovery pipeline. **AMD-47 (RATIFIED + IMPLEMENTED, M4.B3 `60b4185`)** expanded the sealed `AttributeValue` hierarchy to **8 variants** — `QuantityValue`/`ArrayValue`/`DegradedAttributeValue` added to the existing five (`BooleanValue`/`IntValue`/`FloatValue`/`StringValue`/`EnumValue`) — plus the `AttributeValueUpcaster` SPI and `AttributeType` `QUANTITY`/`ARRAY`/`DEGRADED`. **Live in source** (the `permits` clause enumerates all 8; `QuantityValue` canonicalises at construction, AMD-47-INV-03). `StandardCapabilities` (device-model **main**, M4.0b-3 / DP-K) is the production catalogue of the 15 standard capabilities + their `AttributeSchema`s, lifted from `TestCapabilityFactory` (which now delegates) — the future `CapabilityRegistry` seed.
...
**Last verified against:** `homesynapse-core` M4.0b-2 closeout commit `7610296` (unchanged — P4r touched no code) on `2026-05-30`. M4 Workstream A substantially COMPLETE (M4.0a/M4.0b-1/M4.0b-2); derivation/advancer are real (`ProductionDerivationRule`/`DispatchingProjectionAdvancer` in state-store), `projectionVersion`=2, **AMD-44/45/47/50 ratified** (watermark AMD-50 — AMD-47 < 50, ceiling not raised; AMD-47-INV-01..05 registered §20), P2 ratified. Next: **M4.B3 (UNBLOCKED, not started)** → M4.0b-3.
