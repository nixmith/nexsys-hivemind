...
**M4 — Foundation, Workstream A (projection/derivation) substantially COMPLETE (2026-05-29).** **M4.0a** (`a441fdf`): AMD-45 atomic subscriber+view checkpoint coupling via the `AtomicCheckpointSink` seam; reconciliation metadata populated (OR-M3-13 resolved). **M4.0b-1** (`cf1a97e`): the no-op derivation placeholders retired — shipped the production **`ProductionDerivationRule`** (package-private in `com.homesynapse.state`, string change-detect, publishes `state_changed` on LIVE → populates `attributes`) + REC-28 **`DispatchingProjectionAdvancer`** (package-private in `com.homesynapse.state`, constructor-injected `EnvelopeHandler` map, no `ServiceLoader`), both via DEC-M3-16 gateways; **`MinimalProjectionAdvancer` DELETED**; OR-M3-17/18 fully closed; `projectionVersion` stays 1. **M4.0b-2** (`7610296`): ratified AMD-50 version-transition backfill for the 1→2 transition — `projectionVersion` 1→2 trigger; `backfillActive` provenance gate; non-emitting one-shot backfill on both `onEvent`/`processBatch` via the narrow `applyBackfillAttribute`; supersession in `applyToState` (§2.2); `Clock` removed from `DerivationContext` (§2.4). AMD-44/45/50 RATIFIED; on-disk watermark = **AMD-50**; P2 RATIFIED. **AMD-47 RATIFIED 2026-05-30** (P4r governance-mechanics session — device-model `AttributeValue` expansion: public records `QuantityValue`/`ArrayValue`/`DegradedAttributeValue` + `AttributeValueUpcaster` SPI (no `ServiceLoader`) + `AttributeType` `QUANTITY`/`ARRAY`/`DEGRADED`; fork resolved → `QUANTITY` added; AMD-47-INV-01..05 registered into `Architecture_Invariants_v1.md` §20; REC-93 hand-rolled `String` units supersede the deferred JSR-385 plan; Doc 02 §3.7/§8.2 folded current). Watermark stays **AMD-50** (47 < 50). Contract only — **no code touched**, HEAD stays `7610296`; the types + SPI are implemented by M4.B3. Next: **M4.B3** (device-model expansion implementing AMD-47 — now **UNBLOCKED** since both gates clear, but **not started**) → **M4.0b-3** (typed comparator/payload AMD-51/52; gated on M4.B3).
...
Physical and logical device model. One flat package (verified 2026-05-22). Entity (not Device) is the atomic addressable unit — a single physical device may produce multiple entities. Sealed Capability hierarchy, sealed AttributeValue hierarchy. Service interfaces for registries, validation, and discovery pipeline. **AMD-47 (RATIFIED 2026-05-30)** expands the sealed `AttributeValue` hierarchy to **8 variants** — adds `QuantityValue`/`ArrayValue`/`DegradedAttributeValue` to the source-verified existing five (`BooleanValue`/`IntValue`/`FloatValue`/`StringValue`/`EnumValue`) — plus the `AttributeValueUpcaster` SPI and `AttributeType` `QUANTITY`/`ARRAY`/`DEGRADED`. **Contract ratified; implementation is M4.B3 — not yet in source** (the live `permits` clause still enumerates the five existing variants until M4.B3 lands).
