...
**M3 (Event Distribution + State Materialization) is the current milestone.** M3.1 through M3.6d-b complete (fifteen Claude Code WUs). Next: M3.6e.1 (StateQueryService + REST gate — M3.6 capstone). 7 sub-milestones total (M3.1–M3.7). Remaining: M3.6e.1 → M3.6e.2 → M3.7 (~15–20h Coder time).
...
Process-level lifecycle orchestration. After M3.6d-b: 8 public + 2 package-private types. Phase 2 types (LifecyclePhase enum, SubsystemStatus enum, LifecycleEventType utility class, SubsystemState record, SystemHealthSnapshot record, SystemLifecycleManager interface) coexist with M3.6 composition-root types: `HomeSynapseConfig` (public record, 2 fields `PersistenceConfig persistence` + `EventBusConfig eventBus`, `HOME_DEFAULT` constant — M3.6d-a), `HomeSynapseCore` (public final, implements `ReadinessSource` — M3.6d-b; 12-step bootstrap: PersistenceFactory.start → BusMetrics.jfr → InProcessEventBus → DerivedWriteRateLimit → StateProjection.create → subscribeRuntime → healthSignalHandler → QueueSaturationHealthCheck → SharedScheduler → started=true; 4-arg constructor `(Path, HomeSynapseConfig, Clock, HomeId)`; `stateQueryService()` currently returns `ThrowingStateQueryService`, replaced by `MaterializedStateQueryService` in M3.6e.1; `stop()` tears down in reverse order), `SharedScheduler` (package-private final, 50 ms refill + 1 s tick, daemon thread `hs-sched-0`, `safelyInvoke` cadence defence against task-fault silencing — M3.6d-a), `ThrowingStateQueryService` (package-private final, all 5 methods throw `IllegalStateException("StateQueryService not yet wired — available after M3.6e")` — M3.6d-a). Module-info includes `requires transitive` for persistence, event.bus, state-store, plus non-transitive `requires org.slf4j` and `requires com.homesynapse.integration` (for `IntegrationEvents` aggregation). 10-phase sequential startup, no backward transitions. Fatal vs non-fatal subsystem classification. 30-second shutdown budget.
