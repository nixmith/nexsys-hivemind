...
- **Amendments:** AMD-03 (positional snapshot for condition eval), AMD-04 (cascade detection/depth limiting — **PROPOSED-superseded by AMD-91**, pending M7-block ratification), AMD-25 (temporal duration trigger modifier), AMD-31 (command execution order guarantees); **AMD-88..93 (M7 automation block, PROPOSED 2026-06-13)** — see the Amendments Quick Reference below for the six files
...
- **M7 blueprint chain:** `nexsys-hivemind/context/planning/2026-06-12_M7-blueprint_merged-disposition.md` (THE authority) · `2026-06-12_M7-M8-charter-skeleton.md` (entry-gate + pieces) · `context/assessments/2026-06-12_W0_Research_4_Currency_Delta.md` + `2026-06-12_Research_14B/14A/15_PM_Assessment.md` · `context/instructions/2026-06-13_AMD-88-93_C8-C9_Bundled_DOCS_Review_Prompt.md`
...
- **Amendments:** AMD-13 (migration framework), AMD-16 (secrets store backup), AMD-20 (YAML parser safety), AMD-24 (reload atomicity); **AMD-66/67/68/70/71 (M6 config block, RATIFIED 2026-06-09)** — listener seam, `(major,minor)` schema, `setAll(Map)`, observability events, hybrid layout; AMD-69 DEFERRED (Tier-2/OQ-15-3)
...
| **`research/returns/` (the research-pipeline archive, 2026-05-15 → 06-12)** | All raw research returns, numbered 02–15: entity modeling, automation engine (R4 — the M7 ground), configuration (R5), integration runtime (R6), REST/WS (R7 v1/v2), device model (R8), projection rebuild (R9), change detection (R10), typed payloads (R11), Zigbee derisk (R12), config market superiority (R13), **automation UX failure modes (R14-A) + runtime robustness (R14-B) + ecosystem positioning (R15) — the 2026-06-12 M7-blueprint cycle**. PM assessments live hivemind-side at `nexsys-hivemind/context/assessments/` |
| **The M7 blueprint authority chain (hivemind-side)** | `context/planning/2026-06-12_M7-blueprint_merged-disposition.md` (REC-31..40 + 141..185 merged; THE authority for the AMD block + M7.x/M8 scope + M5-C backlog + strategy refresh) → `context/planning/2026-06-12_M7-M8-charter-skeleton.md` (5-row entry-gate + 3 M7 pieces) → `design/amendments/AMD-88..93_*.md` (PROPOSED) |
...
|-----|---------------|------|
| 52 | Typed `StateChangedEvent` Payload (RATIFIED + IMPLEMENTED M4.0b-4a/4b; §11 relocation erratum) | `AMD-52_*.md` |
| 53 | Timestamp-Model Unifier (RATIFIED + IMPLEMENTED M4.0b-5; `projectionVersion` 5) | `AMD-53_*.md` |
| 54–64 | Integration-API Freeze Block (11 AMDs, RATIFIED 2026-06-05, M4.C `8ef9e9f`) | `AMD-54_*.md` … `AMD-64_*.md` |
...
| 66..71 | M6 Configuration Block (6 amendments) | **66/67/68/70/71 RATIFIED 2026-06-09; 69 DEFERRED (Tier-2/OQ-15-3, number reserved)** | Doc 06 — `ConfigurationChangeListener` (66), config-doc schema `(major,minor)` (67), `SecretStore.setAll(Map)` — the Doc 15 §7.3 currency amendment (68), config observability events + the E70-1 type-residency rule (70), hybrid `${config_dir}` layout + traversal guard + composition-root `Path` injection (71). 8 invariants **§37–§41**. Impl: M6.1 (66/67/70/71) / M6.2 (68) / M6.4 (66 swap, 70 `section_reloaded`). **Watermark UNCHANGED at AMD-87** (reserved slots filled). Review return: `nexsys-hivemind/context/audits/2026-06-09_AMD-66-71_DOCS_Review_Return.md`. |
...
- **AMD numbering — P2 RATIFIED (2026-05-29):** device 46–49, projection 50–52 fixed (gaps unused at 46/48/49); integration assigned-at-milestone → the Workstream C block took AMD-54..64 contiguously from the live watermark. M6 configuration amendments AMD-66–71: **authored 2026-06-08, ratified/deferred 2026-06-09** (66/67/68/70/71 RATIFIED; 69 DEFERRED Tier-2/OQ-15-3 — number reserved; watermark unchanged AMD-87).
...
**Last verified against:** `homesynapse-core` **`1eddd9a`** (M6.3 = substantive HEAD) on **2026-06-15** — **M6 COMPLETE 4-of-4 (M6.3 at-rest payload encryption landed `1eddd9a`)**; AMD-88..93 + B2 C8/C9 RATIFIED 2026-06-12; **on-disk watermark AMD-93**; event pins 55/24/36 (→71/41/53 at M7.x implementation); **163 invariants/47 categories**; AMD-04 SUPERSEDED by AMD-91. The 2026-06-15 converge synthesis is the current M7-forward map; M7.1 issue-gated (rows 3+4); C3 doc-fix + R-α research brief authored 2026-06-15. _Prior milestones (historical — superseded by the current line above):_ M4 + M5-A shipped; Doc 15 Cryptographic Architecture LOCKED (15th design doc); AMD-86/87 RATIFIED (M5 window); the M6 config block AMD-66/67/68/70/71 RATIFIED + AMD-69 DEFERRED (2026-06-09; watermark was then AMD-87 and the count 152/41 — both since raised to AMD-93 / 163-47 at the M7 block); `projectionVersion` 5 throughout.
