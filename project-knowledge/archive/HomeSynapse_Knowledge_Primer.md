...
status: CURRENT
last-verified: 2026-06-15 against core `1eddd9a` — **M6 COMPLETE 4-of-4: M6.1/M6.4/M6.2/M6.3 shipped — the config pipeline, hot-reload/write path, secret-store/key-management, AND at-rest payload encryption (`[identity, presence_personal]`) are PRODUCTION CODE now (M6.3 landed GREEN `1eddd9a`; the crypto is shipped + tested but runtime-INERT until app-bootstrap wires `payloadCipher` — `main()` is a stub). Event pins 55/24/36; automation = 53 types incl. 30 sealed permits (still Phase-2 interface-only — M7.1 writes the first impls). AMD-88..93 + B2 C8/C9 RATIFIED 2026-06-12 — watermark AMD-93; invariants 163/47; AMD-04 SUPERSEDED by AMD-91; the C8 four-kind actorRef convention governs from the next event written. The 2026-06-14/15 two-part core review + converge synthesis are the current M7-forward map (`context/audits/2026-06-15_core-review_CONVERGE_synthesis.md`): M7 READY, M7.1 issue-gated (rows 3+4); fix-firsts C3/C4; the two HIGHs (C1 auth, C2 decrypt) → app-bootstrap. _Prior: 2026-06-13 against `7c73c91` — M6 was 3-of-4, M6.3 triple-gated._** _Prior:_ 2026-06-09 against `6c6dd33` — **M6 ENTRY-GATE CLOSED: AMD-66/67/68/70/71 RATIFIED 2026-06-09 + AMD-69 DEFERRED (Tier-2/OQ-15-3); invariants §37–§41 (+8; index total regenerated 152/41); watermark UNCHANGED AMD-87 (reserved slots filled); M6.1 READY TO ISSUE.** Standing JPMS lesson (E70-1 type-residency, now in the P2 survey): a new type in a base module (event-model/value-model/platform-api) references only that module, a leaf, or `java.base` — config/domain types are *consumed* into flattened components, never *referenced* in event records. _Prior 2026-06-07 against `7f44bed`:_ **M5-A COMPLETE + M5-B/B1 DONE** (platform-systemd `PlatformPaths`/`HealthReporter` impls + the AMD-87 `Expectation` persisted codec; **Doc 15 Cryptographic Architecture LOCKED** — the 15th design doc; AMD-86 + AMD-87 RATIFIED; watermark **AMD-87**; `projectionVersion` 5). Prior spine state: `8ef9e9f` (M4 COMPLETE, watermark AMD-64).
-->
...
The source repo (`homesynapse-core`) contains the code. The companion repo (`homesynapse-core-docs`) contains design documents, governance, and research. Phase 2 (public API contracts — interfaces, records, enums, sealed hierarchies) is complete across all modules (~330+ public Java types). A new **`core/value-model`** leaf module was added in M4.0b-4a (holds the relocated `AttributeValue` hierarchy) — **22 Gradle subprojects** now (per `settings.gradle.kts`: ~16–17 JPMS-compiled production + 3 scaffold + `testing/integration-tests` + the `spike:wal-validation` throwaway).
...
**M3.1 (InProcessEventBus Core) completed 2026-05-16.** Production InProcessEventBus with 14 types (9 public + 5 package-private — see event-bus MODULE_CONTEXT for full inventory). Full subscriber lifecycle: Subscriber interface, SubscriberMode FSM, SubscriberSupervisor (backoff, circuit breaker), SubscriberDlq, ReplayWindowQueue, SubscriberRuntime. EventBusContractTest expanded from 18 to 44 methods (Tiers 1–8 active, Tiers 9–10 @Disabled). AMD-41/42/43 applied (state projection execution model, subscriber lifecycle/isolation, backpressure/observability). Architecture Invariants expanded to 94 (§19: BUS, PROJ, WRITER, SUB-ISO). **(Current register total: 152 invariants across 41 identifier categories — REGENERATED from the §17 table at the 2026-06-09 M6-block registration [§37–§41 added +8]; earlier running totals "133/32" and "135/34" were copied-forward under-counts — always re-derive from the §17 table, never propagate a stated total. AMD-47 §20, AMD-51 §21, AMD-52 §22, AMD-53 §23, AMD-54..64 §24–§34, AMD-86/87 §35/§36, AMD-66..71 §37–§41. The 11 AMD-88..93 candidates are PROPOSED, NOT yet registered.)** Total test suite: ~1,200+ @Test methods across ~600+ Java files. Full build: GREEN.
...
Rule engine. **One flat package** — **53 public types** in `com.homesynapse.automation` (count re-derived 2026-06-12 against source: 5 enums + 1 `RunId` wrapper + 4 sealed roots + **30 permits** [6 selectors / 9 triggers / 7 conditions / 8 actions, incl. Tier-2 empties] + 4 data records + 9 service interfaces; the prior "~52"/"27 permits" were stale copied-forward counts). No `dispatch`/`evaluator`/`policy` sub-packages exist (2026-05-22 DQ-4 resolution stands). Trigger → Condition → Action pipeline; 4 sealed hierarchies — the highest concentration of any module. Conditions evaluate against positional state snapshots (AMD-03). Cascade depth limiting (AMD-04 — **PROPOSED-superseded by AMD-91's `RunCausalChain`**, pending the M7-block ratification). Command execution ordering (AMD-31). **⚠ The M7 block (AMD-88..93, RATIFIED 2026-06-12) reshapes this module's contracts: triggers 9→12 permits + `triggerId`, selectors 6→7 + `includedRoles` (BREAKING), actions 8→9 + `confirmation()` + the `ActivateSceneAction`→`InvokeAutomationAction` rename, `RunContext.cascadeDepth`→`causalChain: RunCausalChain` (AMD-04 superseded). The RATIFIED contracts are the binding shapes for M7.x; SOURCE still carries the 53-type pre-block shape until the M7.x implementations land — cite the AMDs for contracts, source for current code.**

### configuration — `com.homesynapse.config`
YAML 1.2 config loading, JSON Schema validation, AES-256-GCM encrypted secrets. **PRODUCTION as of M6.1/M6.4/M6.2 (no longer Phase-2 stubs):** `YamlLoader` (CoreSchema-explicit — the engine default is JSON schema where `~` ≠ null; the M6.1 GF-1 lesson), `JsonSchemaCompositeValidator`, `StandardSchemaRegistry`, `StandardConfigurationService` (load/reload/write — reload = re-parse + `ConfigModelDiff` + AMD-66 listener classification + volatile atomic swap; write = optimistic concurrency + validate-mutated-copy + `AtomicYamlWriter` temp-fsync-rename + first-write backup; **write() REJECTS `!secret`/`!env`-bearing documents fail-closed** [M6.2 R-1; tag-preserving emitter = queued post-MVP AMD]), `StandardScopeKeyManager` (lazy 0400 `.root-key`, RFC-5869 HKDF-SHA256 scope KEKs, GCM-wrapped DEKs in `scope_keys.json`), `StandardSecretStore` (`secrets.enc` envelope under the `config_secrets` scope; AMD-68 `setAll` all-or-nothing; AMD-16 backup rotation). `ConfigModel` carries `(configSchemaMajor, configSchemaMinor)` (AMD-67). ConfigurationChangeListener fires synchronously before the reload event publish (AMD-66/AMD-70 ordering); module-info has 5 third-party non-transitive `requires` (snakeyaml-engine, networknt, etc. — the 2026-06-10 ruling).
...
- `EventTypes.CORE_PRODUCTION_EVENT_CLASSES` (**24 classes** as of M6.4 — 22 original + `ConfigValidationCompletedEvent` M6.1 + `ConfigSectionReloadedEvent` M6.4; `EventTypes` itself = **55 string constants**; `EventCategoryMapping` = **36 entries**) lives in `core/event-model`; `IntegrationEvents.LIFECYCLE_EVENT_CLASSES` (5 classes) lives in `integration/integration-api`. Aggregation at composition root via `Stream.concat(...).toList()`. Do NOT re-introduce per-call-site inline class imports (M3.6c removed 27 of them across AllEventClasses + IntegrationTestHarness).
...
**Last verified against:** `homesynapse-core` **`e5ea76f`** (substantive **`7c73c91`** = M6.2) on 2026-06-13. M4 + M5-A + **M6.1/M6.4/M6.2 shipped** (M6.3 triple-gated — the last M6 piece); **Doc 15 LOCKED**; AMD-86/87 + the AMD-66..71 block RATIFIED; watermark **AMD-87 on disk**; `projectionVersion` 5; event pins **55/24/36**; **152 invariants across 41 identifier categories** (regenerated 2026-06-09; 11 AMD-88..93 candidates PENDING). **AMD-88..93 (M7 automation block) + B2 C8/C9 RATIFIED 2026-06-12** (bundled review; edits folded; **watermark AMD-93**; invariants **163/47**). Entry-gate rows 1–2 ✅; M7.1 waits on row 3 (M6.3-vs-M7 ordering — microbench + interviews) + row 4 (M5-C Increment 1).
