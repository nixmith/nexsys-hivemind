...
status: CURRENT
last-verified: 2026-06-19 (currency pass at the AB-3 → AMD-94/Seam-1 arc — **M6 COMPLETE 4-of-4; M7.1 DONE (`beb4bc3`); AB-3 DONE (`60d50ce`) — the system is RUNNABLE** [`main()` boots `HomeSynapseCore`, which now **implements the Doc-12-Locked `SystemLifecycleManager`**; HTTP gated closed via a test-only `exposeHttpSurface()`; cipher inert]. **AB-1+AB-2 ISSUED** [Seam-1: opaque-bearer-token auth + the fail-closed read contract — the Coder builds next]; **AB-4 AUTHORED-GATED** on **AMD-94 PROPOSED** [Doc 15 §6: rotate-DEK-on-restore + the 1-byte envelope version tag → DOCS review → Nick ratifies, watermark 93→94, closes OR-M6-NONCE restore-half]. **Doc 16 DRAFT** [Superior Automation Layer] → independent DOCS review → Lock → gates **M7.2b**. M7.2 split: **M7.2a** [baseline run/dispatch, builds against the now-live composition root] / **M7.2b** [Lock-gated on Doc 16 + D2]. watermark AMD-93, invariants 163/47, 22 Gradle subprojects; substantive HEAD core `60d50ce` / docs `4a41494`. **AB-3-arc lessons (fold into every coding instruction):** the consumer/pin survey now includes **public-method-signature changes**; **pin the fail-closed SCOPE** (per-record / per-batch / per-subsystem-init) before issue; the FIX-07 `requires transitive`↔Gradle `api` two-gate **recurs on any new public seam type**; a green test encoding the wrong contract is worse than red — human WUCP Phase 2 catches compiles-but-wrong-contract that the deferred gate cannot. **A standing PM mission-control hub** now orchestrates the lane fan-out as the SINGLE spine-writer [`context/handoff/2026-06-19_PM-mission-control_orchestrator_session_prompt.md`] — every spawned session is write-isolated (the fix for the 2026-06-19 concurrent-write collisions). Prior baseline: 2026-06-18 against `1eddd9a`.
-->
