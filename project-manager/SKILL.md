...
status: CURRENT
last-verified: 2026-07-30 (SKILLS CURRENCY PASS 2, executed as a dedicated fresh lane per the full-bar-at-fresh-context law: **the v40 arc + the research-ambitions layer folded** — the `arc-disciplines` list +5 (items 24–28: filed-measurement-first re-pin · full-corpus sweep on any dialect finding · post-change fixture predictions · operator-block hygiene ×4 · instrument-first on a repeat failure), **rule-census-verified 23-in / 28-out with every prior name surviving**; **the strategy-layer pointer AMENDED to name BOTH documents** (the north star + `context/strategy/Substrate_Thesis_v0.md`, the market/paradigm layer) **with the language law riding it**. Same-pass siblings: `references/coding-instruction-format.md` #20–#21 · the coder masthead v40 line + two-document pointer · the playbook §8 addenda + the confirmation-window correction + the interim auth form · the FE §4 pointer line · the coder-preflight lock-free-porcelain line; census + honesty evidence in `context/audits/2026-07-30_skills-pass-2_return.md`.) Prior: 2026-07-28 (the post-v39 skills-currency + optimization pass, executed as a dedicated lane: **THE COMPRESSION** — the three arc blocks (v35 · v32 · bench) collapsed into ONE indexed `arc-disciplines` list, **rule-census-verified EQUAL at 17-in / 17-out** with every name surviving and every rule keeping its detail pointer (the four source pointers hoisted into the list header instead of repeated per rule); **+ the v39-arc disciplines** folded in as items 1–6 (never-author-on-an-unmeasured-hop · the build-graph seam class · preservation-fixture red carve-outs · returns-file-to-audits · the placeholder fill-in warning · the Check-9 remote-cache refinement); **+ the north-star pointer line** (the agent-substrate thesis; read when a brief touches strategy or the agent layer); durable-disciplines trimmed at three places where a sentence merely restated its own pointed-to carrier. Same-pass siblings: `references/freshness-preflight.md` Check 9 (+ the coder mirror) and `references/coding-instruction-format.md` #18–#19; census + trim evidence in `context/audits/2026-07-28_skills-pass_return.md`.) Prior: 2026-07-26 (v38 hub, beat 3 — the skills currency pass: Mode-1 doc count 14→18-re-derive; references/constraint-enforcement.md licensing principle RE-GROUNDED to the LOCKED Apache-2.0-forever strategy (the proprietary-distribution frame retired; on-disk headers stay proprietary only until the launch LICENSE flip); references/coding-instruction-format.md gains additions #16 ARCH-RULE-REACH + #17 SEAM-I/O (the CMD-API audit misses, pm-lessons 2026-07-24 ×2 folded at their operative carrier); CLAUDE.md doc-count corrected + Build-Verification reconciled to the standing targeted-set/deferred-gate discipline). Prior: 2026-07-19 (v35 hub, beat 6 — +the v35-arc disciplines line below: no-attribution-trailer commit messages [Nick's directive] · enumerate-file-shapes-at-source · subsumption-names-its-delta-class). Prior: 2026-07-18 (v32 hub, close-out — +the v32-arc disciplines line below: fixture-paired asserts standing; composed-behavior-under-state-history = a defect class; out-wait the instrument; deploy-state at the instrument; rename-readiness). Prior: 2026-07-11 (v29 hub, launch hygiene — +two durable record-integrity rules from the M9.6-RO correction arc: **a staged commit message asserts ONLY confirmed state** (write it after the delta exists, or hold the order) · **correction paste-blocks are HEADED “AUDIT CORRECTION — do not re-run the WU”** (never re-readable as a dispatch; receiving sessions verify-not-redo). Pointer, not copy — detail: pm-lessons 2026-07-10 ×2.) Prior: 2026-07-08 (v25 hub, beat 6 — +coding-instruction-format additions #14–15 [the exported-surface module-info check against the instruction's OWN DPs; crash-window rulings for non-atomic multi-event emission contracts; coder-self-review-fleet = layer-1 evidence, never a layer-2 substitute]; +two durable rules: **per-commit message files** — never reuse a staged commit message for a ride-along delta, its 'stages exactly N' clause goes stale against the second diff (the 6fd6ddc reconciliation, 2026-07-08); **remote-Cowork bridge model** — when the hub runs remote, host truth rides the device bridge per env-model §12 [stage-reads; SendUserFile→commit-writes; git-object sourcing for spine edits; a same-path re-stage serves a STALE container cache; local-MCP tool scope is VOLATILE — verify before relying]). Prior: 2026-07-07 (v23 hub, beat 7 — the M9.4 bench-arc discipline pointers; pm-lessons 2026-07-07).
arc-disciplines (ONE indexed list; each rule = its operative sentence + its pointer. DETAIL SOURCES, hoisted once: **v40** → pm-handoff v40 beats 1–5 + `context/audits/2026-07-28_B2_suite-port_return.md` §§12–13; **v39** → pm-handoff v39 beats 1–9 + the three 2026-07-27 returns in `context/audits/` (deploy-evening · WCAP-2 · devtools-chain-glance); **v35** → pm-handoff v35 beats 1–6; **v32** → `context/process/2026-07-18_compounding-testing-doctrine.md` (the operating charter) + pm-handoff v32 beats 1–3; **bench** → `context/process/bench-troubleshooting-playbook.md` — pointer-of-record + operator procedures, READ IT BEFORE authoring any bench-iterative WU):
 ...
 (20) **INSTRUMENT-FIRST** (bench) — an ambiguous outcome buys an observability arm (read-back INFO / log-only handler) in the NEXT WU; never theorize twice about the same silence.
 ...
 (28) **INSTRUMENT-FIRST ON A REPEAT FAILURE** (v40) — the same leg failing twice under two theories buys a bundle/evidence read BEFORE any third ruling, with per-hypothesis predictions stated before the read; never retune blind (F-12/F-13). Carrier: the B2 return §13.
strategy-layer (READ BOTH when a brief touches strategy, positioning, the company story, the agent layer, or research): (1) the north star — the TECHNICAL thesis, **the harness enforces; the model only proposes** — at `context/strategy/2026-07-27_homesynapse-technical-overview_north-star.md`; its filing frontmatter carries the verified-at-filing honesty state (one load-bearing claim is DESIGNED-FOR, not TRUE-TODAY — never quote it externally without that). (2) the Substrate Thesis — the MARKET/PARADIGM layer above it: the substrate bet (occupy the layer that compounds with model progress instead of being consumed by it), the L0–L3 layering, the ten-pattern library, §5/§9 as the evidence discipline — at `context/strategy/Substrate_Thesis_v0.md`; its own §11 precedence law governs (where it conflicts with source, governance artifacts, or direct system knowledge, THOSE win without argument). **The language law rides this pointer (D5, v40 beat 2):** every restatement of the enforcement position uses the layered form — the deterministic floor is MISSING from the field, not SUPERIOR; L2/L3 without L1 are unsound, L1 without L2 is insufficient — never restate it as deterministic-beats-model. **Gate sovereignty governs through Aug-16:** nothing there moves pre-freeze code; every build-out charters post-gate.
