...
status: CURRENT — W-SKILLS-9 (2026-09-13): §3 law 5 gains its mechanism (THE PROBE-EVERY-CAP RULE + the splice library's path); §3 gains law 15 (THE PRIOR-LEDGER GATE); §2 steps 7 and 9 and §6 gain the improvement register; nothing else touched; return context/audits/2026-09-13_W-SKILLS-9_return.md. Prior: v69 beat 1 (2026-09-10): §1's commit-boundary law re-cut on Nick's word (every repo is his hands; the hub never commits or pushes); §1's trailer sentence corrected to arc 7 (the 2026-09-03 harness sentence removed); §8 gains the trailer line. W-SKILLS-6 (2026-09-03): the token-shaped rewrite; census: every rule name of the 2026-08-29 masthead survives verbatim in references/laws-ledger.md. Return: context/research/2026-09-03_agent-skills_best-practices_hub-synthesis_W-SKILLS-6.md (§2–§3) + the v61 beat-9 spine line.
-->
...
7. **Audit the return at the bytes — two layers, always** (§3 law 1); rule the pushback on evidence (`references/review-and-quality.md` §3); file the intake audit under `context/audits/`. The intake's non-blocking findings land as rows of `context/planning/improvement-register.md` (an id, the surface, the finding, the instrument that would catch it), never as chat or as a WU the brief did not order.
...
9. **WUCP Phase 2** (`../context/protocols/work-unit-completion-protocol.md`): Update MODULE_CONTEXT.md where contracts changed, the deferred-gate audit, Open Risks, the drift check, Check 9. Phase 2 reads the improvement register for the WU's surface and retires the rows the landing closed.
...
5. **The clock law**: `date -u` first in every beat script; the stamp derives from it. **The guarded-splice law**: every assert and region-cap check runs BEFORE the first byte is written, and the commit step never runs on a failed splice; census lines anchor on beat-unique text. Its mechanism (THE PROBE-EVERY-CAP RULE, 2026-09-13): the script probes EVERY capped region it will write (chain · beat block · snapshot · brief) with the census slots at their widest plausible values, in one block that runs before PHASE 1 — the splice that minted it probed two regions of four and stood half-spliced until restored from HEAD (pm-lessons 2026-09-13); the library is `context/process/splice_lib_v1.py`, md5-asserted at load, versioned by file name (its next version carries the probe block as `precheck()`), and a script is committed under a name never used before in the session.
...
15. **The prior-ledger gate**: before a packet cut from a prior record goes LIVE, its cutter greps that record's deviations ledger and its findings card's asks for every reused command string and named witness, fixes or carries each hit as a named deviation, and lists the greps in the cutting beat's audit (`references/coding-instruction-format.md` #31; the WUCP Phase-2 rig-WU line).
...
| `../context/planning/improvement-register.md` | Read by grep (the surface's path or its `IR-` ids) when authoring on a surface, and at WUCP Phase 2 — the intake findings that are not yet WUs. Never read whole at boot. |
