...
status: CURRENT
last-verified: 2026-07-26 (v38 hub, beat 3 — the skills currency pass: Mode-1 doc count 14→18-re-derive; references/constraint-enforcement.md licensing principle RE-GROUNDED to the LOCKED Apache-2.0-forever strategy (the proprietary-distribution frame retired; on-disk headers stay proprietary only until the launch LICENSE flip); references/coding-instruction-format.md gains additions #16 ARCH-RULE-REACH + #17 SEAM-I/O (the CMD-API audit misses, pm-lessons 2026-07-24 ×2 folded at their operative carrier); CLAUDE.md doc-count corrected + Build-Verification reconciled to the standing targeted-set/deferred-gate discipline). Prior: 2026-07-19 (v35 hub, beat 6 — +the v35-arc disciplines line below: no-attribution-trailer commit messages [Nick's directive] · enumerate-file-shapes-at-source · subsumption-names-its-delta-class). Prior: 2026-07-18 (v32 hub, close-out — +the v32-arc disciplines line below: fixture-paired asserts standing; composed-behavior-under-state-history = a defect class; out-wait the instrument; deploy-state at the instrument; rename-readiness). Prior: 2026-07-11 (v29 hub, launch hygiene — +two durable record-integrity rules from the M9.6-RO correction arc: **a staged commit message asserts ONLY confirmed state** (write it after the delta exists, or hold the order) · **correction paste-blocks are HEADED “AUDIT CORRECTION — do not re-run the WU”** (never re-readable as a dispatch; receiving sessions verify-not-redo). Pointer, not copy — detail: pm-lessons 2026-07-10 ×2.) Prior: 2026-07-08 (v25 hub, beat 6 — +coding-instruction-format additions #14–15 [the exported-surface module-info check against the instruction's OWN DPs; crash-window rulings for non-atomic multi-event emission contracts; coder-self-review-fleet = layer-1 evidence, never a layer-2 substitute]; +two durable rules: **per-commit message files** — never reuse a staged commit message for a ride-along delta, its 'stages exactly N' clause goes stale against the second diff (the 6fd6ddc reconciliation, 2026-07-08); **remote-Cowork bridge model** — when the hub runs remote, host truth rides the device bridge per env-model §12 [stage-reads; SendUserFile→commit-writes; git-object sourcing for spine edits; a same-path re-stage serves a STALE container cache; local-MCP tool scope is VOLATILE — verify before relying]). Prior: 2026-07-07 (v23 hub, beat 7 — the M9.4 bench-arc discipline pointers; pm-lessons 2026-07-07).
v35-arc disciplines (2026-07-19, the G1-close / LEARN-PERSIST arc — detail: pm-handoff v35 beats 1–6): (1) **Commit messages carry NO attribution trailers** — Nick's standing directive 2026-07-19: never append Co-Authored-By / AI-attribution / session-link lines to any staged commit message (rule's home: env-model §9). (2) **Enumerate file shapes AT SOURCE at authoring** — test-method counts, construction-site locations, and every file-structure claim in an instruction or pre-verification come from reading the source file at authoring time, never carried from close-out prose or inferred from line numbers (two hub-owned [INFO] imprecisions in one WU: "two existing methods" vs the file's three; "adapter ctor" vs the actual initialize() sites). (3) **CI subsumption states its delta class** — a green descendant run subsumes an ancestor's per-commit confirm (the OR-GATE-M7.4 precedent) only with the delta named zero-impact for the gate in question (e.g. "zero-Java delta" for ci.yml).
v32-arc disciplines (2026-07-18, the I3a/F2/silent-skip arc — detail: `context/process/2026-07-18_compounding-testing-doctrine.md` [the operating charter] + pm-handoff v32 beats 1–3): (1) **Fixture-paired assert semantics are a STANDING RULE** — the hub prices instrument semantics before assertion; every new assert ships with fixtures proving its PASS and its false-verdict boundary (the v31 mint, field-proven at v32: first live rep caught the silent-skip class). (2) **Composed-behavior-under-state-history is a first-class defect class** — correct components can compose invisibly (the §3.9 skip × log-rehydrated availability: four days of lawful do-nothing runs rendering as success). Audits and scenario authoring must ask WHAT THE LOG ALREADY CARRIES; history-seeded scenarios (boot against a log bearing adversarial-but-lawful prior state) are the harness — the bench's seeded-log architecture exists for exactly this. (3) **The bench out-waits its instrument's resolution** — an experiment shorter than the evidence cadence measures nothing (availability resolves at ping-scale: minutes); state each instrument's resolution in the operator block. (4) **Deploy-state is re-derived at the instrument, never assumed from an ordered sequence** (the stale-REV2 hour); instruments self-identify — version banners are instrument semantics. (5) The terminal-materialization + type-filtered-checkpoint instrument classes are PAID rules (each cost a field misread twice) — price them in every runs-surface/checkpoint assertion. (6) Deploy hygiene rides every bench touch: exec-bit/`.gitattributes`/LF-only; never scp a CRLF working tree. (7) Operator-choreography-vs-sensor-physics is RULED — organic-runs-as-evidence; occupancy holds defeat scripted waves; write operator blocks to the sensor's physics. (8) **Rename-readiness:** the product/brand naming program is Nick-gated (R-1 at G-2) — every new work product stays token-parameterized; never couple artifacts, tokens, or file names to the current brand string beyond what already exists.
bench-arc disciplines (2026-07-07, the M9.4 silicon arc — DETAIL + operator procedures in `context/process/bench-troubleshooting-playbook.md`, the pointer-of-record and the seed of a future dedicated troubleshooting/integrations skill; read it BEFORE authoring any bench-iterative WU): (1) **the mechanism-without-driver sweep walks THROUGH the gate** — for every runbook-expected behavior verify emitter + caller chain + DRIVER to a production call site; "X is gated on Y" is not a sweep terminus (the class hit 5×: PJ/NCFG/ADP/RPT/DUR — every one green-build-invisible). (2) **Vacuous-VERIFY pairing** — every no-output-is-healthy check gets a paired positive-evidence line; instructions MANDATE an anti-vacuous success INFO on any silently-succeedable arm. (3) **Instrument-first** — an ambiguous bench outcome buys an observability arm in the NEXT WU (read-back INFO / log-only handler) so the following run is measurement, not theory; never theorize twice about the same silence. (4) **Timeline-consistent closures + the discriminator pattern** — closures must survive timestamp arithmetic against recorded operator testimony; contested closures get a cheap physical discriminator experiment with per-hypothesis predictions stated BEFORE the run. (5) Bench recipes must respect restart semantics (re-link arms are unreachable across restarts until rehydration lands — check the spine for DUR status). (6) **Operator handoffs follow the playbook §8 contract** — self-contained paste-blocks (shell state dies with the session), goal+done-when first, one physical act per line with the expected log token named, anti-actions stated explicitly, questions answerable in timestamps/counts, expected counts at every glance-point, ⏺ RECORD + paste-either-way.
...
durable-disciplines (stable content — the reason this masthead exists): CI/gate-of-record — an in-session LLM "verification: clean" is NOT a gate, and a claimed gate must be verified actually-green on the pushed commit; source-verify WUCP against the git DIFF, never the completion report; grounding-subagent-before-authoring + read-the-actual-source (not a top-level glob); parallelize independent pre-milestone inputs; staged-commit-on-word amendments + the `_scratch`/`git commit -F` protocol; record "V1 ships a subset of a Locked doc" explicitly; the record component/static-factory collision STOP-check; instruction-internal-consistency; reconcile-refinements-into-lanes-before-launch. The standing **PM mission-control hub** is the SINGLE spine-writer (the FIVE-repo model: core / docs / hivemind / skills / bench); every spawned session is write-isolated and returns to the hub. The bench (`nexsys-bench`) is the **test-and-truth engine**: a captured real device stream is a seeded event log → hardware-free regression tests + the measured `confirmed|unconfirmed` moat. **Lane-routing matrix (settled 2026-07-02/03, v14 beats 50–56):** host-side Claude Code for compile-loop backend M-WUs (targeted `./gradlew :*:compileJava :*:test` allow-listed in the CC session); fresh Cowork conversations for frontend/research/strategy lanes; in-conversation agents for micro-WUs and read-only grounding surveys; the hub never implements — it authors, dispatches, audits. **Restart-resume:** after any client drop, freshness-check before any spine write (env-model §11 — signatures + protocol live there). **Shared-tree commits:** selective staging with exact stated path counts (env-model §10). **Dispatch craft:** pm-lessons 2026-07-03 (the dispatch prompt is the quality gate — baseline-shift line, known-hazards line, evidence/decision artifact separation, re-derivable counts). **Post-green adversarial review (2026-07-04):** a full-`check`+CI-green arc still buys an INDEPENDENT adversarial-review lane at the arc boundary — the M9-arc precedent found 3 HIGH after every automated gate passed (deaf-radio / false-CONFIRMED / unreachable-TOLERANCE classes, all caught at desk price); the lane pattern: read-only, evidence-required findings (concrete trigger or it's a NOTE), settled-ground fenced with refutation-welcome BOTH directions, one return file, two-layer audited like any lane. **The two-layer audit extends to your OWN subagents:** grounding/verification agent returns get layer-2 spot-checks exactly like lane returns (the M9.3 "only 4 public types" refutation — an agent claim contradicting a MODULE_CONTEXT it itself quoted). **Audit-craft (2026-07-04 — POINTERS, detail in pm-lessons 2026-07-04 ×2):** audit-agent prompts QUOTE the instruction's own sentences verbatim for anything semantically subtle (supersession/expiry/ordering) — a paraphrased mandate becomes the agent's phantom truth and returns confident false DEFECTs; agent verdict LABELS are CLAIMS, agent QUOTES are EVIDENCE — layer-2 adjudicates every DEFECT/REFUTED against the PRIMARY text (instruction + ratified doc), never against the agent's restatement; any Files-table row placing a NEW test in a module the WU doesn't otherwise touch gets a build-file gating check at authoring (test-task `enabled`/profiles/tag filters — the R1 piProfile lesson: a test that never runs in the gate of record reads as coverage).
-->
