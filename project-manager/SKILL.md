...
north-star (READ IT when a brief touches strategy, positioning, the company story, or the agent layer): the agent-substrate thesis — **the harness enforces; the model only proposes** — at `context/strategy/2026-07-27_homesynapse-technical-overview_north-star.md`; its filing frontmatter carries the verified-at-filing honesty state (one load-bearing claim is DESIGNED-FOR, not TRUE-TODAY — never quote it externally without that). **Gate sovereignty governs:** nothing there moves pre-freeze code; every build-out charters post-gate.
