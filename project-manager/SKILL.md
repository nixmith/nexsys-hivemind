...
status: CURRENT
last-verified: 2026-06-28 (v11 mission-control hub — the M7.5a/b read-API + AB-4 cipher-activation point). **The read-API differentiator core is COMPLETE + the at-rest cipher is LIVE.** DONE + GREEN: M6 · M7.1 · AB-1/2/3 · M7.2a/b · M7.3 · M7.4a/b/c/d (M7.4 arc COMPLETE; D2 replay-purity CI-pinned) · **M7.5a + M7.5b** (both hero halves on the live log — "why fire / did confirm" + "why didn't fire" + the automation list) · **AB-4** (at-rest cipher LIVE for `[identity, presence_personal]`; F1 envelope-version `v1` + F3 nonce-construction fence + F13b fail-closed fsync). counts **71/41/53**; `projectionVersion` 5. core `eed477e`+AB-4 / docs `75d0345` (**Doc 16+17 LOCKED**; watermark **AMD-95**; invariants **170/50**). **Next Core slot = M9** (real Zigbee; the hero on real silicon, acceptance-tested vs the bench corpus) — or M7.5c (events+health) if re-sequenced. **The bench is reframed as the test-and-truth engine** (the 5th repo `nexsys-bench`; a captured real device stream is a seeded event log → hardware-free regression tests + the `confirmed|unconfirmed` moat measured) — the critical path + the common upstream of the two unbuilt mid-Aug go/no-go gates. **v6→v11 lessons folded into pm-lessons:** parallelize-independent-pre-milestone-inputs; source-verify-WUCP-against-the-diff + grounding-subagent-before-authoring + read-the-actual-source (not a top-level glob); staged-commit-on-word amendments + the `_scratch`/`git commit -F` protocol; record-V1-ships-a-subset-of-a-Locked-doc explicitly. Carried disciplines: CI/gate-of-record (an in-session LLM 'verification: clean' is NOT a gate); the record component/static-factory collision STOP-check; instruction-internal-consistency; reconcile-refinements-into-lanes-before-launch. The standing **PM mission-control hub** is the SINGLE spine-writer (now the FIVE-repo model: core / docs / hivemind / skills / **bench**); every spawned session write-isolated. **Check 9 STALE until Nick's mirror sync** (this currency fold). Prior baseline: 2026-06-22 (v5 hub — M7.2b-GREEN / action-model-FROZEN).
-->
...
**Phase 3 vocabulary:** Work units are called **Milestones** (M{major}.{minor}, e.g., M2.5). Each milestone is a single compile-and-commit unit with test coverage. The active backlog is `context/planning/phase-3-milestone-backlog.md`. Milestone groups: M1.x test-first prep, M2.x persistence, M3.x event-bus/state-projection/composition-root, M4 device-model expansion + projection/derivation + integration-api freeze, **M5-A** platform/codec, **M6** config/secrets/hot-reload/at-rest-crypto — **all COMPLETE**. **Current: the runnability arc is landing (2026-06-19).** M6 + **M7.1 DONE** (`beb4bc3`) + **AB-3 DONE** (`60d50ce`) — `main()` now boots `HomeSynapseCore`, which **implements the Doc-12-Locked `SystemLifecycleManager`**, so **the system is runnable for the first time** (HTTP gated closed via a test-only `exposeHttpSurface()`; the at-rest cipher inert — both activate at Seam-1). **AB-1+AB-2 ISSUED** (the Seam-1 go-live: auth + the fail-closed read contract — closes core-review **C1**/OR-APP-AUTH + **C2**/OR-RF-DECRYPT; the Coder builds next) with **AB-4 AUTHORED-GATED** on **AMD-94** (PROPOSED — Doc 15 §6 rotate-DEK-on-restore + the 1-byte envelope version tag; DOCS review → Nick ratifies → watermark 93→94). **Doc 16** (Superior Automation Layer) is in independent DOCS review → Lock, which gates **M7.2b**; **M7.2 is split** into **M7.2a** (baseline run/dispatch — builds against the now-live composition root) and **M7.2b** (Lock-gated on Doc 16 + D2). The pipeline (Coder lane is serial): `AB-1+AB-2 → AB-4 (post-AMD-94) → M7.2a → M7.2b → M7.3 → M9`, with the AMD-94 + Doc 16 reviews and research (R-γ, Key-Portability) running in parallel. A standing **PM mission-control hub** (`context/handoff/2026-06-19_PM-mission-control_orchestrator_session_prompt.md`) now orchestrates the fan-out as the SINGLE spine-writer (every spawned session write-isolated — the structural fix for the 2026-06-19 concurrent-write collisions). See `context/planning/weeks/2026-W26_jun22-jun28.md` + the snapshot 2026-06-19 masthead for live lanes.
