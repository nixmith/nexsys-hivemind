...
status: CURRENT
last-verified: 2026-06-22 (v5 mission-control hub — the M7.2b-GREEN / action-model-FROZEN point). **M7.2b GREEN** (core `1b0b6c9`, `./gradlew check` 149 tasks, first pass, zero gate-fix rounds) — **THE ACTION MODEL IS FROZEN** (minimal computed-param resolution + run-coupled fail-closed terminal + no-engine-retry AMD-90-INV-01). The Core engine now **ACTS end-to-end** (trigger → condition → run → action → dispatch → events): M6 (4-of-4) · M7.1 · AB-1/AB-2/AB-3 · M7.2a-1 · M7.2a-2 · M7.2b all DONE + GREEN. **Next Core slot = M7.3 (PendingCommandLedger)** — the confirmed/unconfirmed hero half (Option A, co-signed into V1) — then the thin causal-read-API → AB-4 → M9 Zigbee (the corrected sequence). docs `e47f01e` (**Doc 16 Superior Automation Layer LOCKED**; INV-SA-01..04; watermark **AMD-94**; invariants **169/49**); hivemind `04c0a54` + the v5-hub first-act hygiene fold (snapshot/pm-handoff rotation + these skill folds) on top — **Check 9 STALE until Nick's mirror sync**. **NEW LANE:** a pre-M9 **hardware-bench + device-characterization thread** (Nick-driven, parallel, write-isolated; Wave 1 ~2026-06-23; corpus `project-knowledge/device-corpus/`) overlays the fan-out. **v4-session lessons folded into pm-lessons + the trap list:** CI-as-gate-of-record (an in-session LLM 'verification: clean' is NOT a gate — M7.2a-2 shipped two record-collisions past it); the **record component/static-factory collision STOP-check** (added to coding-instruction-format); instruction-internal-consistency (an illustrative block must not contradict a binding pin); don't-refactor-working-code-without-cause; reconcile-research/contract-refinements-into-lanes-before-launch. The standing **PM mission-control v5 hub** is the SINGLE spine-writer; every spawned session write-isolated. Prior baseline: 2026-06-19 against `60d50ce` (AB-3/Seam-1 arc).
-->
