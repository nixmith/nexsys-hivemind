...
status: CURRENT
last-verified: 2026-09-13 (W-SKILLS-9 — TWO additions, zero retirements, everything else byte-unchanged: #30 the coder-session prompt checklist · #31 the prior-ledger gate. Return: `../../context/audits/2026-09-13_W-SKILLS-9_return.md`.) Prior: 2026-09-07 (W-SKILLS-8 — SEVEN additions, zero retirements, everything else byte-unchanged: #23 the separable-bytes law · #24 the diagnostic-readability check · #25 the measurement-shape law · #26 the operator-note law (the v64 laws) · #27 the instrument-beside-the-claim law · #28 the miss-script sweep · #29 the cap sentence (the v66 laws). Return: `../../context/audits/2026-09-07_W-SKILLS-8_return.md`.) Prior: 2026-08-18 (W-SKILLS-3 — the R-12 currency pass. ONE addition, zero retirements, everything else byte-unchanged: addition #22 THE LIVE-WIRE ACCEPTANCE BAR (H8/NEW-8b, v53 beat 4 — the third assigned carrier per the ratification). Census + token counts: `context/audits/2026-08-18_W-SKILLS-3_return.md`.) Prior: 2026-08-03 (W-SKILLS v44 — this file is now the **ONE HOME** of the §4c Arch-Rule Test-Clock paste-block, and **that block was carrying a live factual defect**: it asserted the ArchUnit rule scans non-whitelisted TEST classes, contradicting addition #6 in this same file and the 2026-06-13 M6.2 enforcement-reach correction. Corrected in place; the correction note rides the section. Additions #1–#21 and every other section byte-unchanged. Census: `context/audits/2026-08-03_W-SKILLS_v44-pass_return.md`.) Prior: 2026-06-07 against commit 8028337.
-->
...
## Additions (2026-07-26, from the CMD-API audit misses — pm-lessons 2026-07-24 ×2; folded at this operative carrier by the v38 skills pass)

16. **ARCH-RULE-REACH (MANDATORY P2-survey bullet).** Before any instruction mandates publishing from, wiring into, or otherwise crossing a module/layer boundary for a target package, grep `app/homesynapse-app/src/test/java/com/homesynapse/app/HomeSynapseArchRules.java` for every rule reaching that package and verify the mandate is jointly satisfiable with the instruction's own full-check success criterion. Origin (CMD-API R1): the instruction mandated an `app.post` publish from a package Rule 9 forbade — jointly unsatisfiable with its own gate; the Coder's scoped exemption realized the rule's documented intent, but the collision was authoring-detectable. Emit the result as named rules + line cites + an explicit zero-collisions (or collision-resolution) statement in the P2 survey (the WU-SKIP-VIS G-9/P12 shape).
17. **SEAM-I/O (first-of-its-class handler check).** When an instruction introduces the FIRST use of a verb/plane on an existing seam (the first POST on a GET-only endpoint family, the first body-consuming handler, the first multipart), verify AT AUTHORING that the seam interface can READ what the handler needs (body, headers, content type) — and where it cannot, the instruction specifies the seam growth explicitly. Origin: the CMD-API `EndpointContext` class — a POST mandate against a seam that could not read request bodies (`body()`/`requestHeader()` arrived as Coder-improvised seam growth, accepted at audit as hub authoring miss #2).
...
## Additions (2026-07-28, from the v39 arc — folded at the skills-currency pass; detail: pm-handoff v39 beats 1–9 + the three 2026-07-27 returns in `../../context/audits/`)

18. **Red-first predictions CARVE OUT preservation fixtures.** A test whose assertions ARE the behaviors HEAD already preserves cannot go red at HEAD *by construction*. When an instruction's test plan contains such a fixture, the red-first prediction sentence must name it and predict it **green-by-construction, disclosed** — never sweep it into a blanket "all new tests red at HEAD." Walk the test table before writing the prediction and sort each row into *red-at-HEAD* (it asserts the new behavior) or *green-by-construction* (it asserts a behavior being preserved); a row that cannot be sorted is a row whose purpose is not yet clear. Origin: DASH-SERVE I4 — the spec itself named the fixture "the preservation fixture" and then predicted all three e2e rows red; the Coder's honest green-by-construction disclosure was the lawful reading and the imprecision was owned as the hub's.
19. **The unmeasured-hop gate (instrument-first, AT AUTHORING).** An instruction premised on a **live-behavior claim** — what the deployed system actually serves, emits, renders, or stores — either (a) **cites the measurement**: a filed capture or return, by path, with the specific line the premise rests on, or (b) **orders the instrument FIRST** (an operator capture block, a dev-tools glance, a read-only probe) and is authored on its pastes. Theory never substitutes for measurement here, because a wrong premise ships a wrong P1 against the critical path. The gate applies to the premise, not the fix: a WU may be minted on measured evidence and still leave implementation open. Origin: CHAIN-HYDRATE — minted P1 on a plausible server-side correlation-join theory, then dissolved by two ten-minute capture blocks plus one dev-tools glance into a small FE null-guard fix (client-side crash) plus a P3 history-era note; the server-side premise was simply false. Companion: an instruction that cannot get its measurement before authoring says so explicitly and states what the premise would cost if wrong.

## Additions (2026-07-30, from the v40 B2 silicon arc — folded at skills currency pass 2; detail: pm-handoff v40 beats 1–5 + `../../context/audits/2026-07-28_B2_suite-port_return.md` §§12–13)

20. **The filed-measurement-first re-pin law (MANDATORY for any wire pin).** An instruction that pins a wire path or shape — an endpoint route, a payload nesting, a field dialect — either **cites the filed measurement** the pin rests on (a capture or return, by path, with the specific line) or **orders the measurement first** and pins on its pastes; the measure-then-pin pattern (the B2 S-2/S-3 probe blocks: the Pi probe ⏺s the wire, the desk pins same-session) is the compliant shape. A RE-pin additionally **checks the filed measurement corpus BEFORE any source read** — an endpoint-feed source pin never outranks a wire measurement, because the source you read may feed a DIFFERENT endpoint than the one the scenario reads. Origin: F-6 — the desk re-pinned `/state` to the `{t,v}` envelope from a serializer that feeds the ENTITY read, while the WCAP return had already measured `/state`'s live `{"value":…}` nesting on file; the pin missed a filed measurement and the audit passed it (hub co-owned). This is #19's sibling: #19 gates an UNMEASURED premise; #20 gates a pin against the measurements that ALREADY EXIST.
21. **The dialect-sweep obligation.** Any instruction fixing a wire-dialect defect carries the FULL-CORPUS sweep for the class as an in-scope act of the SAME beat — grep every scenario, fixture, and comment for the pattern and enumerate the occurrences in the instruction (or the fix's return); never fix only the files already open. Origin: the B2 return §12.7-A — the F-6 class had seven occurrences; the open-file four were fixed and occurrences six and seven survived to fail the first suite of record, decisively; a five-second corpus grep would have caught them in-session.

## Addition (2026-08-18, W-SKILLS-3 — the R-12 currency pass; carrier assigned per the v53 beat-4 ratification)

22. **The live-wire acceptance bar (H8 / NEW-8b, STANDING).** Any instruction touching an FE surface writes its success criterion to the live-wire rule: the surface earns "verified" only by ONE recorded real-wire exercise; green CI on a surface never touched by a real payload is a named false-confidence class; a mock that always populates a nullable field MANUFACTURES the false type that causes the defect. Until the exercise runs, the honest completion register is REPO-COMPLETE, LIVE-VERIFICATION PENDING (the H7 form). This is #19/#20's FE sibling — those gate the instruction's premise and pins; #22 gates its done-when. Exhibits + the plan shape: `../../context/audits/2026-08-17_FE-lane_NEW23_return.md` §5/§8.

## Additions (2026-09-07, W-SKILLS-8 — the v64 and v66 laws, each an operative sentence + its mechanism + its exhibit; detail: `laws-ledger.md` arcs (45) (47) (50) (53) (54) and `../../context/lessons/pm-lessons.md` THE v64 CLOSE MINTS; mapping: `../../context/audits/2026-09-07_W-SKILLS-8_return.md`)

23. **The separable-bytes law (MANDATORY when one lane delivers more than one commit).** A lane ordered to deliver N commits from ONE working tree names the DELIVERY SHAPE in its §0 contract — the tree at commit 1 plus a patch per further commit (`git format-patch`/`git diff` applied with `git apply -p1`, the round-trip proven by the lane), or one branch per commit — and the audit re-executes `git apply --check -p1` and `git apply --stat` against the declared per-commit census before any card is cut. "Two commits, one lane, stage nothing" without the shape is an authoring gap the lane must fill on its own; it did, once. Exhibit: `../../context/audits/2026-09-05_FIX-1_intake_two-layer-audit_v64.md` §7 R1 + `../../context/audits/2026-09-05_FIX-1b.patch`. Ledger twin: arc (47).
24. **The diagnostic-readability check (MANDATORY for every measurement row).** Every row of an instruction's measurement table names the ARTIFACT the value is read from (the JUnit XML's `<failure message>`, the gradle log, a stdout token) AND the FIELD; before the loops are ordered the hub reads the helper's throw / the emitter at source and asks whether that artifact CARRIES the diagnostic. A diagnostic no artifact carries is ordered as a STAMP (one paste, never a bound, never a wait/interval/target change) before the loops run. The exhibit: a row named "the resting checkpoint" as its discriminator while the helper's throw printed target and timeout only, and an SLF4J-free module's IT has an EMPTY `system-out` — the stamp went in by one paste and the mechanism was then located by a deterministic test. Exhibit: `../../context/audits/2026-09-05_v64-b1_boot-and-intake_audit.md` §3. Ledger twin: arc (45).
25. **The measurement-shape law.** A prediction in an instruction states the SHAPE it expects (which arm, which phase, which count) AND its direction, and the recipe carries the INVERSE arm as a first-class row — never as the arm the instruction has already called "collapses toward 0". v64's red came from the arm predicted to collapse (the pin WITHOUT load reproduced 2/36; the pin WITH load 0/40): a recipe that runs only the predicted arm cannot refute its own prediction. Mechanism: the §A-table gains one row per inverse arm (load/no-load; pinned/unpinned) before the instruction dispatches; the audit reads Predicted vs Measured row by row, mismatches first. Exhibit: `../../context/audits/2026-09-05_FIX-1_intake_two-layer-audit_v64.md` §2 (M-1) + coder-lessons 2026-09-05 (FAILCHAN-FIX-1). Ledger sibling: arc (44).
26. **The operator-note law.** A hub note handed to a RUNNING lane (a stamp, a re-run, a scope change) states in the note itself its expected effect on the lane's census (`FIX-1a → 10 = 8 M + 2 A`) and schedules the DELTA RE-AUDIT; no card is cut from the return while a note is outstanding, and a card already cut is RE-CUT at the delta or the note is withdrawn once the return lands. The guard on every card — `EXPECT: N` as its first line — stops the operator when the tree no longer matches. Exhibit: `../../context/audits/2026-09-05_FIX-1_intake_two-layer-audit_v64.md` §9 (§9-L); pm-handoff v64 beat 7. Ledger twin: arc (50).
27. **The instrument-beside-the-claim law (MANDATORY for every source-state claim).** Every claim about the tree in an instruction — "X does not exist", "the constant lives in Y", "the run built the artifact" — carries the COMMAND that made it in a parenthesis beside the claim (`(git grep -n EXPECTED_VERSION -- web-ui = 0 hits)`; `(gh run view <id> --json path filters …)`), so the lane re-runs the command and not the conclusion, and a wrong instrument is visible as an instrument. A grounding grep names its file-type scope explicitly (`-- '*.ts' '*.mjs' '*.js'`); a fence that names a constant is grepped across EVERY file type before dispatch. Exhibit: `../../context/audits/2026-09-06_FE-113_intake_two-layer-audit_v66-b5.md` §3 R1 (`EXPECTED_VERSION` grepped `.ts` only; it lives in `contract-check.mjs`); pm-handoff v66 beat 2 (guard 1 corrected at the workflow files). Ledger twin: arc (53).
28. **The miss-script sweep (MANDATORY P2-survey bullet when a WU appends a resolver or a step after an existing miss).** When a WU adds a resolution step AFTER a miss in an existing chain, the P2 survey sweeps the test HARNESS for the PRIOR surface's miss SCRIPT (the fixture that scripts the miss — `lookupStatus = 0x01` — not the log token or the reason string), because every scenario that scripts that miss changes meaning the moment a later surface can resolve it; the instruction lists each such scenario in Files to Modify with its new script (a status, or silence) and orders the hit test strengthened to pin the new surface's once-per-epoch bound. "Every existing test stays green" is a prediction the harness's default reply can refute. Exhibit: `../../context/audits/2026-09-06_F-R4-1b_intake_two-layer-audit_v66-b8.md` §1 R1; coder-lessons 2026-09-06 (F-R4-1b).
29. **The cap sentence (in every §0 lane contract).** "≤ N KB is a CEILING, not a target; the §0 card is what the hub reads first; a shorter return with the same receipts is a better return." A lane whose receipts genuinely need the space says so in its §0 and the cap was wrong. Exhibit: `../../context/planning/2026-09-06_v66_STATE-OF-THE-PROGRAM_assessment_trajectory_and_hygiene-program.md` §2.4. Ledger twin: arc (54).

## Additions (2026-09-13, W-SKILLS-9 — the v71 and v72 mints, each an operative sentence + its mechanism + its exhibit; detail: `../../context/lessons/pm-lessons.md` 2026-09-12 THE CODER-SESSION PROMPT CHECKLIST · 2026-09-13 THE PRIOR-LEDGER GATE; mapping: `../../context/audits/2026-09-13_W-SKILLS-9_return.md`)

30. **The coder-session prompt checklist (MANDATORY in every instruction's §0).** Four gaps in the prompt, not the WU, cost a clean 8-file lane four passes; the instruction closes them before dispatch: (a) the return cap is ARITHMETIC — 3 KB + ~1 KB per Files-table row, rounded up — never a round number; (b) the gate lines are VERBATIM task paths read from `settings.gradle.kts` (`:app:homesynapse-app:test`, the `--tests` line for the ITs), never a guessed `:app:test`; (c) "scripts over ~15 lines are written with the Write tool and run by path — no long heredocs" (the host-side harness cannot parse a long quoted heredoc; short ones work); (d) when a test's capture point moves (a parent logger for a leaf class's logger), the instruction says what the old point could not see. For the hub: a lane's transcript goes to `_scratch/<window>/<lane>_transcript.md` with a one-line report, never pasted. Exhibit: `../../context/audits/2026-09-12_v71-b2_FIX-2b-ii-i_intake_two-layer-audit.md` §5. Lane twin: `../../coder/SKILL.md` §1.
31. **The prior-ledger gate (MANDATORY before a packet cut from a prior record goes LIVE).** Before a packet cut from a prior record goes LIVE, its cutter greps that record's deviations ledger and its findings card's asks for every command string and every named witness the packet reuses, and fixes each hit in the packet or carries it as a named deviation with the reason; the greps are listed in the cutting beat's audit. "Banked as ten asks" in the spine is a count, not a checklist — R-4c inherited four of R-4b's eleven deviations from one unread ledger. Exhibit: `../../context/audits/2026-09-13_v72-b6_R-4c_intake_two-layer-audit.md` §4; the record's §9. Twins: `../SKILL.md` §3 law 15; the WUCP Phase-2 checklist's rig-WU line (`../../context/protocols/work-unit-completion-protocol.md`).
