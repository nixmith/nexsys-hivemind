...
Report the count as `Check 9: N/N identical at the bytes` (or the STALE/CONFLICTED lines verbatim). The `diff -rq` block above is this same comparison against a HOST mirror, run only if one exists. Exhibit: `../../context/audits/2026-09-05_v65-b1_boot-and-intake_audit.md` §3 (the preflight line as executed). Exhibit of a STALE run adjudicated (2026-09-13, W-SKILLS-9): `../../context/audits/2026-09-13_v73-b1_boot-and-intake_audit.md` §4 check 9 — 27/28 identical, one SOURCE file ahead of its synced copy by a hub re-cut; the lag is Nick's sync, authoring rides SOURCE, forward work lawful.
...
- **The exclusion list as run (2026-09-13, W-SKILLS-9):** the list is re-derived from the newest beat at every run, never carried forward; a lane the spine records as LIVE or DISPATCH-READY is excluded by that record, and a brief whose cadence trigger passed with no paste and no spine beat is STALE → reconciled in the same beat by setting its `status:` to FILED with the prior status kept after `Was:`, the body untouched. Exhibit: `nexsys-hivemind/context/audits/2026-09-13_v73-b1_boot-and-intake_audit.md` §4 check 12.
