...
### The adjudication order — a refutation with line cites is the best outcome of instrument-first working (added 2026-09-07, W-SKILLS-8; the v64 law)

**The standard:** a return whose `[REVIEW]` refutes the instruction's MECHANISM THEORY with line cites is not a defect in the return — it is instrument-first working as designed, and the instruction's theory was a PREDICTION (arc (48)). The hub therefore audits in this order, before a single card line is written: **(1) mismatches FIRST** — the instruction's Predicted column read against the return's Measured column row by row; **(2) each miss owned in writing** — what the prediction assumed, which assumption the measurement refutes, what the fix's branch becomes — never explained away; **(3) the lane's mechanism re-executed at source** (the cited lines read by the hub, 2–3 checks minimum, the non-re-executions disclosed); **(4) only then the verdict and the cards.** The exhibit: the hub's `:302` lost-unpark / phase-2 theory was refuted at source by the lane (`drainAndPromote :83–:150` wrote no checkpoint; phase 1), ruled ACCEPT with the miss owned — `../../context/audits/2026-09-05_FIX-1_intake_two-layer-audit_v64.md` §2 + §7 R2. The pre-filed rules that make this fast: `laws-ledger.md` D18 addendum (rules pre-filed before the return exists).
...
## §HELD-BY-THE-HUB — re-printed EVERY beat: running lanes under the cap (which two, and why in one line) · the operator's own hands' acts, in order · the owed lines · the open words, each with the fact it needs, where that fact comes from, and when it is due · the given words
## §DONE — what landed this beat, one line each (a lane's `RETURNED <path> <bytes>` line is the whole report)
...
### The correction-before-freeze rule — and the correction-before-landing pattern (added 2026-09-13, W-SKILLS-9; the v71 b5 mint + HERO-1c D3)

**THE CORRECTION-BEFORE-FREEZE RULE.** A `[REVIEW]` that names a self-contradiction between two keys of a frozen wire — an additive key individually correct and still contradicting its sibling (v1.1.4's `settledAt` null for a superseded DISPATCHED action whose `settled` is true) — or a user-visible wrong sentence (HERO-1c: a null verb rendered as "act"; feedback primitives labelled with the hero's `explain.*` rows instead of the app's `ui.*` pair) is corrected BEFORE the landing, never after: the hub issues an AUDIT CORRECTION paste (arc (30): headed "AUDIT CORRECTION — do not re-run the WU") to a fresh lane, inside the same census, and the card is cut from the corrected tree. The mechanism behind it: the CG-123 pattern freezes the wire at the landing, so a contradiction that lands is a contract and its fix is a bump; a wrong sentence that lands is what the household reads. Its authoring half: every additive-key instruction states, per new key, its relation to the keys it sits beside (`settledAt != null ⇔ settled`; `confirmedAt` only when CONFIRMED) — D8 applied to the wire. Exhibits: `../../context/audits/2026-09-12_v71-b5_EXPLAIN-114a_intake_two-layer-audit.md` §3 R3 + `../../context/instructions/2026-09-12_EXPLAIN-114a_AUDIT-CORRECTION_R3-settledAt.md`; `../../context/audits/2026-09-13_v73-b5_R-5A_and_HERO-1c_intake_two-layer-audit.md` §3 (D2 + D3) + `../../context/instructions/2026-09-13_HERO-1c_AUDIT-CORRECTION_D3-generic-labels_D2-two-rows.md`.
